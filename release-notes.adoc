...
Release {release-version} is a fifty-seventh midPoint release, code-named _Curie_.
...
** Improved shopping cart summary - list of users in tooltip now contains name. See MID-9762[].
...
See bug:MID-9621[] and xref:_behavior_changes_since_4_8_3[].
* Fix for getting current login users on another node. See bug:MID-9746[].
* Fix for redirecting from the Preview Changes panel to the Details panel when the panel configuration doesn't contain panelType.
See bug:MID-9699[].
* Fix for missing value in lookup table for lifecycle state displayed in LifecycleStatePanel. See bug:MID-9637[].
* Disable full-text search for the Construction Step panel of the Role Wizard. See bug:MID-9693[].
* Fix for saved search state of assignment panel on pages of different assignment holder type. See bug:MID-9457[].
* Fix for memory leak in MidpointAutowiredBeanFactoryObjectPostProcessor caused by undestroyed unused authentication filters.
See bug:MID-9580[] and bug:MID-9834[].
* Fix for full-text search panel for assignments panels that used a custom collection filter. See bug:MID-9692[].
...
* Fix for resolving container value id when adding two new assignments generated from scripts. See bug:MID-9743[].
...
* Widgets which use filters supported only in native repository now displays warning with reason instead of empty state. See bug:MID-9854[].
...
* Upgrade DatabaseTable Connector to 1.5.2.0 version.
* Upgrade DatabaseTable Connector to 2.8 version.
* Upgrade LDAP and AD Connector to 3.7.2 version.
...
* New version (3.7.2) of LDAP connector bundle (including xref:/connectors/connectors/com.evolveum.polygon.connector.ldap.LdapConnector/[LDAP Connector] and xref:/connectors/connectors/com.evolveum.polygon.connector.ldap.ad.AdLdapConnector/[Active Directory Connector]) was released and bundled with midPoint.
