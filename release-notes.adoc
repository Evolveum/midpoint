...
** Added xref:/midpoint/reference/admin-gui/resource-wizard/#association-type-configuration[wizard] support for association configuration in resources.
...
** Added support for xref:/midpoint/reference/roles-policies/mining/#advanced-options[attribute group by/clustering rule].
** Added support for xref:/midpoint/reference/roles-policies/mining/#advanced-options[analyze attribute functionality].
** Added xref:/midpoint/reference/roles-policies/mining/#role-mining-presets[predefined role mining modes].
** Added support for xref:/midpoint/reference/roles-policies/mining/#advanced-options[assignment filters].
...
* xref:/midpoint/reference/roles-policies/outlier-detection/[Outlier Detection]
** Introduces a feature that helps uncover potential security risks by identifying users with unusual access rights.
** For more information, see the xref:/midpoint/reference/roles-policies/outlier-detection/[Outlier Detection documentation].
...
** Midpoint automatically partition shadow tables based on resource and object class of shadow.
Partitioning is enabled by default on new deployments and needs to be explicitly enabled on existing deployments.
See xref:/midpoint/reference/repository/native-postgresql/shadow-partitioning/[Repository -> Native -> Shadow Partitioning] for details.
...
The `synchronize/membership` container was added to the object operation policy object, present in xref:/midpoint/reference/concepts/mark/[object marks] (like the `Protected` one).
