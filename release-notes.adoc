---
layout: release
release-version: '4.10.6'
visibility: hidden
---
= MidPoint 4.10 "Braille" - Update 6
:release-version: 4.10.6
:page-visibility: hidden
:page-liquid:
:page-toc: top

Release {release-version} is an eighty-fifth midPoint release, code-named _Braille_.
The {release-version} release brings security, stability and miscellaneous bug fixes.
...
== Changes With Respect To Version 4.10.5
* Fixed unbounded growth of task operation results during iterative item processing. See bug:MID-12197[]

== Changes With Respect To Version 4.10.4

* Updated multiple libraries, mainly spring-boot, tomcat, postgresql, multiple NPM dependencies.
...
* Fixed validity options translation in Shooping cart. See bug:MID-11142[].

== Changes With Respect To Version 4.10.3
...
* Fixed the merging of the resource capabilities when no id attribute present. See bug:MID-11218[]
* Fixed defaultOperationPolicy merging in objectType of the resource object. See bug:MID-11412[]
...
== Changes With Respect To Version 4.10.2

* Improved login and self-service dashboard loading speed. See bug:MID-10972[]
* Localized archetype label now correctly displays in the assignment picker and summary panel. See bug:MID-11177[]
* Fixed 10k limit in certification queries using iterative search.
    Campaigns with 10k+ cases now properly transition through their stages.
    See bug:MID-11043[]
* Improved user list load speed when listing users with `orgRelation` authorization for users managing many organizations. See bug:MID-11179[]
* Fixed duplicated "No response" option for campaign. See bug:MID-11166[]
* Fixed recipient converter while sending notification.
    All users returned by the `recipientExpression` filter now receive the notification.
    See bug:MID-11038[]
* Improved large dataset export by implementing streaming export. See bug:MID-10990[]
* Updated saved filters merging strategy to ensure multiple custom and pre-defined filters display as expected. See bug:MID-11181[]
* Upgraded libraries (Spring Boot, Wicket, Tomcat, LDAP connector, and others).
* Added option to relax email address validation on send. See bug:MID-11048[]
* Added default sort order to `AccessCertificationWorkItem` list for stable ordering between page loads. See bug:MID-11047[]
* Fixed CSV export filename missing the `.csv` extension. See bug:MID-11046[]
* Fixed authentication exception handling during the user log-in process. See bug:MID-10953[]
* Updated handling of String -> byte[]. See bug:MID-11015[]
* Implemented support for `notifyChange` REST call without explicitly providing the shadow OID. See bug:MID-11230[]
* Fixed GUI issue when requesting access to a role which the user already has assigned under a different relation. See bug:MID-10557[]
...
== Changes With Respect To Version 4.10.1

* Updated libraries, mainly Spring Boot, wicket, NPM dependencies.
* Improved accessibility and usability (WCAG 2.2 AA compliance) of the self-service user interface.
* Fixed concurrency issue in iterativeScripting. See bug:MID-11058[].
* Page storage fixed for dashboard widget collection. See bug:MID-11055[].
* Fixed skip activation outbounds for unmanaged shadows. See bug:MID-11029[].
* Removed redundant ProvisioningService#getObject calls during context load. See bug:MID-11115[].
* Improve certification progress bar visibility. See bug:MID-10930[].
* Fixed admin gui configuration merging. See bug:MID-11045[]

== Changes with Respect To Version 4.10

* Fixed multiple issues connected to WCAG 2.2 requirements.
* Further fix for the Comment column behavior in the Certification items table. See bug:MID-10912[].
* Fixed session storage for Task Errors panel. See bug:MID-10975[].
* Fixed conflicts resolving in Request Access flow. See bug:MID-11001[].
* Fixed Duplicate action for certification definitions. See bug:MID-10456[].
* Fixed translation of the orig value of the poly string in case it is defined in the DisplayType label, help, tooltip etc. See bug:MID-10996[].
* Fixed export of the object/taget display name columns in the Certification items table. See bug:MID-10873[].
* Fixed translation of the left menu items for collection views. See bug:MID-10998[].
* Fixed SchemaType objects loading to prism context. See bug:MID-10983[].
* Fixed visibility of the create report button.
* Fixed report data object name and export file name to be unique. See bug:MID-10978[].
* Fixed searching by stage deadline in the Certification cases table. See bug:MID-11023[].
* Fixed ConcurrentModificationException in RelationUtil. See bug:MID-10940[].
* Fixed an issue where submitting a certification decision was possible after the stage deadline had passed. See bug:MID-11028[].
* Fixed the import report file name to be a generated UUID. See bug:MID-11014[].
* Fixed certification case stage outcome information. See bug:MID-11026[].
* Fixed searching for changed item path. See bug:MID-10985[].
* Fixed rendering of the delta when changed item is a container. See bug:MID-10984[].
* Fixed shadow attributes/id parsing exception. See bug:MID-10944[].
* Updated spring boot to 3.5.8.
...
* Self-service user interface is now very close to WCAG 2.2 AA compliance.
* Implemented multiple improvements related to background tasks:
...
** Better xrefv:/midpoint/reference/master/tasks/task-errors/[search for errors] in task run history. See bug:MID-10411[].
...
** Added xrefv:/midpoint/reference/master/expressions/expressions/filter/[Filter expression evaluator]. See bug:MID-10060[].
** Improved `safe` expression profile in default system configuration.
** Recommended xref:/midpoint/reference/expressions/expressions/security/[_semi-safe_ configuration of expression profiles] to provide reasonable level of xref:/midpoint/reference/expressions/expressions/security/[expression security] for some delegated administration scenarios.
* Improved ways to report audit delta in GUI and reports using xrefv:/midpoint/reference/master/security/audit/#improving-search-capabilities-for-changed-item-paths-in-audit-records[changed item paths].
Improved indexing of audit records with "add object" deltas.
See bug:MID-10413[].
* Added xrefv:/midpoint/reference/support-4.10/resources/resource-configuration/capabilities/#last-login-timestamp-capability[`lastLoginTimestamp` capability]. See bug:MID-10147[].
* Implemented xrefv:/midpoint/reference/master/admin-gui/admin-gui-config/#_default_gui_object_list_panel_configuration/[Default List View Panel Configuration] which brings a possibility to set the common paging options for list panels. See bug:MID-10414[].

=== Other Improvements

* Summary panel for roles now uses display name and identifier (same as for organizations and services). See bug:MID-10270[].
* Task execution constraints added to "advanced options" tab. See bug:MID-10218[].
* Removed max password length constraint from default password policy. See bug:MID-10305[].
* Added support for exclusion of metadata in `get`/`search` rest APIs. See bug:MID-10216[].
...
* Updated various libraries, mainly: Spring Boot, Tomcat, PostgresSQL JDBC driver.
...
* Date time picker now can use `adminGuiConfiguration/displayFormats/longDateTimeFormat`setting if available.
...
* bug:MID-10496[] Fixed issue of task not starting at next scheduled time.
...
=== Upgrade from MidPoint 4.10.x

MidPoint {release-version} data model is completely backwards compatible with midPoint 4.10.
...
* There are minor changes in the initial objects, system configuration localization keys.
