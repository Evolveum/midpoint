...
Release {release-version} is an eighty-third midPoint release, code-named _Curie_.
...
== Changes with Respect To Version 4.8.13

* Upgraded multiple dependencies, mainly spring-boot, postgresql and multiple NPM packages.
* Removed Xerces dependency. See bug:MID-12034[].
...
* Form multipart limit (adminGuiConfiguration/formMaxMultiparts) now defaults to the Tomcat server.tomcat.max-part-count value instead of a fixed 300.
* Fixed read-only fields presentation. See bug:MID-11853[]
...
* Improved styles - label size and form breaks on generated forms. See bug:MID-11628[].

== Changes with Respect To Version 4.8.12

* Hided conflict warning on Shopping cart panel of the Request Access wizard in case of prune action. See bug:MID-11250[] and xref:#_behavior_changes_since_4_8_12[behavior changes since 4.8.12].
* Updated multiple libraries, mainly spring boot to 4.1 and hibernate to 7.4.1, cxf to 4.2.2. See bug:MID-11547[], bug:MID-11002[], bug:MID-11948[].
...
* Fixed sending notification to multiple recipients. See bug:MID-11038[]
* Fixed reference value update in search panel. See bug:MID-11400[]
* Fixed custom relation translation. See bug:MID-11523[]
* Fixed model loading for unassign action popup. See bug:MID-11584[]
...
* Fixed search authorization denial for queries with type filters when read access is item-limited, and fixed the Roles of teammates panel in the Request access wizard to list role, service, organization and application assignments only. See bug:MID-11221[].
* Fixed localization keys being rendered verbatim in several GUI areas. See bug:MID-11269[].
* Fixed missing `.csv` extension in the name of the exported CSV file. See bug:MID-11046[].
* Fixed CSV export on the Tasks list page. See bug:MID-11652[].
* Fixed automatic setting of `archetypeRef` when creating a new object from an archetyped collection view. See bug:MID-11914[].
* Fixed logging initialization, `midPoint.log` is now written from the very start of the system startup. See bug:MID-11861[].
* Fixed closing of a task whose dependent or parent task no longer exists, and improved detection of stale bucketing workers. See bug:MID-10913[].
* Fixed memory leak caused by authentication filters. See bug:MID-9794[].
* Added `#readLog` model authorization action, so the log can be read via REST without superuser privileges.

== Changes with Respect To Version 4.8.11

* Fixed the usage of the multiple `ProtectedStringType` fields in the same GUI panel. See bug:MID-11072[]
...
* Fixed the relation dependent behavior and an item selection GUI issue while adding an item into shopping cart. See bug:MID-10851[], bug:MID-10557[].
* Fixed ability to hide Campaigns scheduling menu item by visibility configuration. See bug:MID-11176[]
* Fixed axiom query parsing for saved filters to ensure complex filters work as expected after they are saved for later use. See bug:MID-11183[]
* Upgraded libraries (Spring Boot, Wicket, Tomcat, and others).
* Fixed concurrency issue in multi-threaded `iterativeScripting` tasks. See bug:MID-11058[]
* Fixed dashboard widget showing "Unknown" for `ShadowType` collections instead of counting the shadows. See bug:MID-11102[]
...
== Changes with Respect To Version 4.8.10

* Added Slovenian language
* Fixed shopping cart icon number while resolving conflicts in Request access wizard. See bug:MID-10925[].
* Fixed report preview. See bug:MID-10929[].
* Fixed container duplication while adding new container value in GUI. See bug:MID-10926[].
* Fixed marking of the propagated pending delete operations as completed. See bug:MID-10870[].
* Fixed displaying and creating of the enforcement policy action. See bug:MID-10928[].
* Fixed Unassignment action on the Governance panel. See bug:MID-10955[].
* Fixed object types configuration on the members panel. See bug:MID-10896[].
* Fixed search item expression filter configuration. See bug:MID-10977[].
...
* Extended the list of the searchable items in GUI for focus type objects. See bug:MID-11012[].
...
* Improved search authorization dealing w/ multiple obj. types, bug:MID-10206[].
This change slightly affects authorization evaluation is such cases.
For more information see commit message https://github.com/Evolveum/midpoint/commit/28ab1c8196a66caba9a7aeb76b6ad29e890bd5f6[28ab1c81].
...
For more information see xrefv:/midpoint/reference/support-4.8/upgrade/upgrade-with-ninja/#initial-objects-update[here].
...
[#_behavior_changes_since_4_8_12]
=== Behavior Changes Since 4.8.12
