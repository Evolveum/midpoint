...
---
= MidPoint 4.9 "Verne" - Update 9
:release-version: 4.9.9
:page-liquid:
:page-toc: top

Release {release-version} is an eighty-fourth release midPoint release, code-named _Verne_.
...
== Changes with Respect To Version 4.9.8
...
== Changes with Respect To Version 4.9.7
...
* Fixed the multiple click on Submit request button in Role catalog. See bug:MID-11939[]
...
* Fixed GUI error occurring when opening the Subtasks panel in some reconciliation tasks. See bug:MID-11107[].
* Fixed misleading correlation case behavior in GUI with cases seemingly closing on a resolution error. See bug:MID-11094[]
* Fixed missing error details in the Error panel of a task where errors occurred. See bug:MID-10682[].
* Fixed an issue with redundant role request overriding assignment metadata.
    Improved GUI feedback to the role requester when the request is redundant because the target role is already assigned.
    See bug:MID-10979[].
* Upgraded libraries (Spring Boot, Wicket, Tomcat, LDAP connector, webpack, and others).
...
* Fixed displaying of the non-necessary warning messages on the shopping cart page. See bug:MID-10924[].
* Fixed member panel above user-permission table headers within Role mining GUI. See bug:MID-10526[].
* Fixed NPE while admin GUI configuration merging. See bug:MID-10970[].
* Fixed Duplicate action for certification definitions. See bug:MID-10456[].
* Fixed validation check for mandatory items in GUI forms. See bug:MID-10974[].
* Improved error logging in SqaleTableMapping. See bug:MID-11018[].
...
The preferred option to start simple midPoint instance is via docker compose. For more information see xref:/midpoint/get-started/install/containers/docker/[here].
...
For more information see xrefv:/midpoint/reference/support-4.9/upgrade/upgrade-with-ninja/#initial-objects-update[here].
