...
public class ConnectorInstanceIcfImpl implements ConnectorInstance {
		...
		try {
			...
			if (LOGGER.isTraceEnabled()) {
				...
				for (String propName : apiConfig.getConfigurationProperties().getPropertyNames()) {
					LOGGER.trace("P: {} = {}", propName, apiConfig.getConfigurationProperties().getProperty(propName)
							.getValue());
				}
			}
	...
	public Schema generateConnectorSchema() {
		...
		configurationContainerDef.createPropertyDefinition(ConnectorFactoryIcfImpl.CONNECTOR_SCHEMA_TIMEOUTS_ELEMENT,
	...
	public Schema getResourceSchema(OperationResult parentResult) throws CommunicationException,
		...
		// Result type for this operation
		OperationResult result = parentResult.createSubresult(ConnectorInstance.class.getName() + ".getResourceSchema");
		result.addContext("connector", connectorType);
	...
	private void parseResourceSchema(org.identityconnectors.framework.common.objects.Schema icfSchema) {
		...
		for (ObjectClassInfo objectClassInfo : objectClassInfoSet) {
			...
			for (AttributeInfo attributeInfo : attributeInfoSet) {
				...
				ResourceObjectAttributeDefinition roaDefinition = roDefinition.createAttributeDefinition(attrXsdName,
						attrXsdType);
	...
	public Set<Object> getCapabilities(OperationResult parentResult) throws CommunicationException,
		...
		// Result type for this operation
		OperationResult result = parentResult.createSubresult(ConnectorInstance.class.getName() + ".getCapabilities");
		result.addContext("connector", connectorType);
	...
	public Set<ResourceObjectAttribute> addObject(ResourceObject object, Set<Operation> additionalOperations,
		...
		try {
			...
			throw new SchemaException("Error while converting resource object attributes. Reason: " + ex.getMessage(),
					ex);
		}
		if (attributes == null) {
	...
	public List<Change> fetchChanges(ResourceObjectDefinition objectClass, Property lastToken, OperationResult parentResult)
		...
		try {
			icfConnectorFacade.sync(icfObjectClass, syncToken, syncHandler, new OperationOptionsBuilder().build());
			icfResult.recordSuccess();
		} catch (Exception ex) {
			Exception midpointEx = processIcfException(ex, icfResult);
	...
	public void test(OperationResult parentResult) {
		...
		OperationResult connectionResult = parentResult.createSubresult(ConnectorTestOperation.CONNECTOR_CONNECTION
				.getOperation());
	...
	private ObjectClass objectClassToIcf(ResourceObjectDefinition objectClass) {
		...
		if (ACCOUNT_OBJECTCLASS_LOCALNAME.equals(lname)) {
			...
			throw new IllegalArgumentException("Cannot recognize objectclass QName " + qnameObjectClass + " for "
	...
	private void convertFromActivation(Set<Attribute> attributes, ActivationChangeOperation activationChangeOperation) {

		attributes
				.add(AttributeBuilder.build(OperationalAttributes.ENABLE_NAME, activationChangeOperation.isEnabled()));

	}

	private void convertFromPassword(Set<Attribute> attributes, PasswordChangeOperation passwordChangeOperation) {
	...
	private List<Change> getChangesFromSyncDelta(Set<SyncDelta> result, Schema schema, OperationResult parentResult)
	...
	private ObjectChangeModificationType createModificationChange(SyncDelta delta, ResourceObject resourceObject)
		...
		for (ResourceObjectAttribute attr : resourceObject.getAttributes()) {
			...
			try {
				...
				throw new SchemaException("An error occured while serializing resource object properties to DOM. "
						+ ex.getMessage(), ex);
			}
						...
						if (configElement.getNamespaceURI() == null
							...
							if (type.isArray()) {
								...
								while (i + 1 < configurationNodelist.getLength()
										...
										&& ((Element) (configurationNodelist.item(i + 1))).getLocalName().equals(
												propertyName)) {
									i++;
				...
				for (int i = 0; i < childNodes.getLength(); i++) {
					...
					if (item.getNodeType() == Node.ELEMENT_NODE) {
						...
						if (subelement.getNamespaceURI() != null
								&& subelement.getNamespaceURI().equals(ConnectorFactoryIcfImpl.NS_ICF_CONFIGURATION)) {
