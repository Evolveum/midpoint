...
public class ProvisioningService implements ProvisioningPortType {
	...
	@Override
	public java.lang.String addObject(ObjectContainerType objectContainer, ScriptsType scripts,
			Holder<OperationalResultType> result) throws FaultMessage {
		...
		if (object instanceof ResourceObjectShadowType) {
			try {
				...
				// TODO: fist add object on resource by calling identity
				// connector
				...
				if (valueWriter.validateBeforeCreate(resourceObject)) {
					...
					// TODO: The search should be executed here. The get only
					// works if the system does not use GUID
					ResourceObject robj = null;// rai.get(result.value,
												// resourceObject);
					if (robj == null) {
						processScripts(result.value, rai, scripts, ScriptOrderType.BEFORE,
								OperationTypeType.ADD);
						// This should return with the entire object, like the
						// update
						updatedObject = rai.add(result.value, resourceObject, shadow);
						processScripts(result.value, rai, scripts, ScriptOrderType.AFTER,
								OperationTypeType.ADD);
					} else {
						// FIXME
						throw new UnsupportedOperationException();
						// updatedObject = rai.modify(result.value,
						// resourceObject, null);
					}
				}
				...
				// TODO: update the attributes and keep the ones we need to
				// store
				...
				logger.error(
						"### PROVISIONING # Fault addObject(..): Error on add object to the repository: {}",
						ex.getMessage());
				...
				logger.error("### PROVISIONING # Fault addObject(..): Object validation error: {}",
						ex.getMessage());
				throw createFaultMessage("Object validation error: " + ex.getMessage(),
						SchemaViolationFaultType.class, false, ex, result.value);
			} catch (Exception ex) {
				logger.error(
						"### PROVISIONING # Fault addObject(..): General error in the provisioning (addObject): {}",
						ex.getMessage());
				logger.debug("General error in the provisioning (addObject).", ex);
				throw createFaultMessage("General error in the provisioning (addObject): " + ex.getMessage(),
						SystemFaultType.class, false, ex, result.value);
			}
			...
			// TODO: check if this provides correct SOAP faults. I have some
			// doubts.
			logger.error("### PROVISIONING # Fault addObject(..): Unsupported object type : {}", object
					.getClass().getName());
			throw new FaultMessage("Unsupported object type " + object.getClass().getName(),
					new IllegalArgumentFaultType());
	...
	public ObjectContainerType getObject(java.lang.String oid, PropertyReferenceListType resolve,
		...
		try {
			...
			// Tis operation needs to support getting both ResourceObjectShadow
			// (and subclasses)
			...
			if (object instanceof ResourceType) {
				...
				logger.error("### PROVISIONING # Fault getObject(..) : Unsupported object type : {}", object
						.getClass().getName());
				throw createFaultMessage("Unsupported object type " + object.getClass().getName() + " (OID: "
						+ oid + ")", IllegalArgumentFaultType.class, false, null, result.value);

			}

		} catch (com.evolveum.midpoint.xml.ns._public.repository.repository_1.FaultMessage ex) {
			logger.error("### PROVISIONING # Fault getObject(..) : Repository error : {}", ex.getMessage());
			throw createFaultMessage("Repository invocation failed (getObject)", ex.getFaultInfo(), ex,
					result.value);
		} catch (RuntimeException ex) {
			logger.error("### PROVISIONING # Fault getObject(..) : Unknown error : {}", ex.getMessage());
			throw createFaultMessage("Error on getting object", SystemFaultType.class, false, ex,
					result.value);
		}
	...
	public ObjectListType searchObjects(QueryType query, PagingType paging,
		...
		try {
			...
			logger.error("### PROVISIONING # Fault searchObjects(..) : Repository error : {}",
					ex.getMessage());
	...
	public void modifyObject(ObjectModificationType objectChange, ScriptsType scripts,
		...
		try {
			...
			if (ot instanceof ResourceObjectShadowType) {
				...
				ResourceObjectDefinition objectDefinition = schema.getObjectDefinition(shadow
						.getObjectClass());
				...
				getResourceAccessInterface(shadow).modify(result.value, identification, objectDefinition,
						changes);
				...
				logger.error(
						"### PROVISIONING # Fault modifyObject(..): Unsupported object type {} (OID: {})", ot
								.getClass().getName(), oid);
				throw new FaultMessage("Unsupported object type " + ot.getClass().getName() + " (OID: " + oid
						+ ")", new IllegalArgumentFaultType());
			}
			...
			// TODO: It really should not happen that inner code will throw WSDL
			// fault!
	...
	@Override
	public void deleteObject(java.lang.String oid, ScriptsType scripts, Holder<OperationalResultType> result)
			throws FaultMessage {
		...
		try {
			...
			if (c instanceof ResourceObjectShadowType) {
				...
				try {
					ResourceAccessInterface rai = getResourceAccessInterface(shadow);
					ResourceObject resourceObject = valueWriter.buildResourceObject(shadow, rai
							.getConnector().getSchema());
					processScripts(result.value, rai, scripts, ScriptOrderType.BEFORE,
							OperationTypeType.DELETE);
					rai.delete(result.value, resourceObject);
					processScripts(result.value, rai, scripts, ScriptOrderType.AFTER,
							OperationTypeType.DELETE);
				} catch (MidPointException ex) {
					// TODO !!!
					logger.error("### PROVISIONING # Fault deleteObject(..): Unknown error : {}",
							ex.getMessage());
				...
				logger.error(
						"### PROVISIONING # Fault deleteObject(..): Unsupported object type {} (OID: {})", c
								.getClass().getName(), oid);
				throw new FaultMessage("Unsupported object type " + c.getClass().getName() + " (OID: " + oid
						+ ")", new IllegalArgumentFaultType());
			}
	...
	public PropertyAvailableValuesListType getPropertyAvailableValues(java.lang.String oid,
			PropertyReferenceListType properties, Holder<OperationalResultType> result) throws FaultMessage {
		...
		try {
			...
			logger.error("### PROVISIONING # Fault getPropertyAvailableValues(..): Unknown error : {}",
					ex.getMessage());
			logger.debug("Unknown error occured.", ex);
	...
	@Override
	public ObjectContainerType addResourceObject(String resourceOid, ObjectContainerType objectContainer,
			Holder<OperationalResultType> result) throws FaultMessage {
	...
	@Override
	public ObjectContainerType getResourceObject(String resourceOid,
			ResourceObjectIdentificationType identification, Holder<OperationalResultType> result)
			throws FaultMessage {

		ResourceType resource = null;
		try {
			resource = (ResourceType) getRepository().getObject(resourceOid, new PropertyReferenceListType())
					.getObject();
		} catch (com.evolveum.midpoint.xml.ns._public.repository.repository_1.FaultMessage ex) {
			logger.error("Error on getting resource (oid: " + resourceOid + ") from repository");
			throw new FaultMessage("Error on getting resource (oid: " + resourceOid + ") from repository",
					ex.getFaultInfo(), ex);
		}
		ResourceObjectShadowType shadow = null;
		// try {
		// ResourceObject object = getResourceAccessInterface(resourceOid,
		// resource, null).operationGET(result.value, identification, null);
		//
		// } catch (MidPointException ex) {
		// }
	...
	public ObjectListType listResourceObjects(String resourceOid, String objectType, PagingType paging,
		...
		try {
			...
			ResourceObjectDefinition def = schema.getObjectDefinition(new QName(
					schema.getResourceNamespace(), objectType));
			if (null != def) {
	...
	@Override
	public void modifyResourceObject(String resourceOid, ObjectModificationType objectChange,
			Holder<OperationalResultType> result) throws FaultMessage {
		throw new UnsupportedOperationException("Not supported yet.");
	}

	@Override
	public void deleteResourceObject(String resourceOid, ResourceObjectIdentificationType identification,
			Holder<OperationalResultType> result) throws FaultMessage {
		throw new UnsupportedOperationException("Not supported yet.");
	}

	/**
	 * Fills the required __PASSWORD__ attribute based credentials in the
	 * accountshadow
	 * 
	 ...
	 * @throws DOMException
	 */
	private void processAccountCredentials(ResourceObject resourceObject, AccountShadowType shadow)
			throws DOMException {
		...
		pwd.setTextContent(new String(Base64.decodeBase64(((Element) shadow.getCredentials().getPassword()
				.getAny()).getTextContent())));
	 ...
	 * @return
	 */
	protected ResourceAccessInterface getResourceAccessInterface(String resourceOID,
			ResourceType inputResource) throws FaultMessage {
		BaseResourceIntegration connector = null;
		ResourceAccessConfigurationType resourceAccessConfiguration = null;
		try {
			ResourceType savedResource = (ResourceType) getRepository().getObject(resourceOID,
					new PropertyReferenceListType()).getObject();
			// resourceAccessConfiguration =
			// getResourceAccessConfiguration(savedResource);
			// TODO: Merge the two resources instead of overwrite. This way the
			// resource can configured from business process similar: Complete
			// Resource
			...
			throw new FaultMessage("Error on resolve ObjectShadow.resourceRef from repository.",
					ex.getFaultInfo(), ex);
		}
		try {
			ResourceAccessInterface rif;
			rif = connectorFactory.checkout(ResourceAccessAspect.class, connector,
					resourceAccessConfiguration);
			return rif;
		} catch (InitialisationException ex) {
			logger.error("Error on ResourceAccessInterface Initialisation");
			throw new FaultMessage("Error on ResourceAccessInterface Initialisation, reason: "
					+ ex.getMessage(), null, ex);
		}
	}

	protected ResourceAccessInterface getResourceAccessInterface(ResourceObjectShadowType shadow)
			throws FaultMessage {
		if (shadow.getResource() == null) {
			if (shadow.getResourceRef() == null) {
				throw new FaultMessage("Resource and resourceRef are undefined at object(oid="
						+ shadow.getOid() + "). One of them should be set.", new SchemaViolationFaultType());
			}
	...
	private ResourceAccessConfigurationType getResourceAccessConfiguration(ResourceType resource) {
		...
		try {
			...
			return (ResourceAccessConfigurationType) getRepository().getObject(oid,
					new PropertyReferenceListType()).getObject();
		} catch (com.evolveum.midpoint.xml.ns._public.repository.repository_1.FaultMessage ex) {
			logger.error(
					"Error on resolve ResourceType.resourceAccessConfigurationRef from repository. ResourceType="
							+ resource.getOid() + " , ResourceAccessConfigurationType=" + oid, ex);
		}
	 ...
	 * 
	 * This should be independed from identity connectors concepts or any other
	 * specific provisioning implementation.
	 * 
	 * @param shadow
	 *            Resource obeject shadow from the repository.
	 * @throws FaultMessage
	 */
	private void completeResourceObjectShadow(ResourceObjectShadowType shadow) throws FaultMessage,
			MidPointException, com.evolveum.midpoint.xml.ns._public.repository.repository_1.FaultMessage {
		...
		if (resultObject == null) {
			throw new FaultMessage("The object " + shadow.getOid() + " could not be found",
					new SystemFaultType());
		}
	...
	protected void resolveResource(ResourceObjectShadowType shadow)
			throws com.evolveum.midpoint.xml.ns._public.repository.repository_1.FaultMessage {
		if (shadow.getResource() == null) {
			if (shadow.getResourceRef() == null) {
				throw new com.evolveum.midpoint.xml.ns._public.repository.repository_1.FaultMessage(
						"Resource and resourceRef are undefined at object(oid=" + shadow.getOid()
								+ "). One of them should be set.", new SchemaViolationFaultType());
			}
	...
	public Set<AttributeChange> processObjectChanges(ResourceSchema schema,
			ResourceObjectDefinition objectDefinition, ObjectModificationType objectChange)
			...
			if (any != null && !any.isEmpty()) {
				...
				if (segments.isEmpty()) {
					...
					if (SchemaConstants.I_ATTRIBUTES.equals(segments.get(0).getQName())) {
						if (segments.size() > 1) {
							throw new IllegalRequestException("Hierarchy in " + segments.get(0).getQName()
				...
				ResourceAttributeDefinition attributeDefinition = objectDefinition
						.getAttributeDefinition(attrQName);
				ResourceAttribute resourceAttribute = new ResourceAttribute(attributeDefinition);
				...
				logger.warn("modifyObject(): Received empty attribute value element for oid {}",
						objectChange.getOid());
			}
		}
		return attrChanges;
	...
	private QName getAttrNameFromXPath(ResourceObjectDefinition definition, XPathType xpath)
			throws IllegalRequestException {

		List<XPathSegment> segments = xpath.toSegments();

		if (!SchemaConstants.I_ATTRIBUTES.equals(segments.get(0).getQName())) {
			throw new IllegalRequestException("Expected XPath starting with " + SchemaConstants.I_ATTRIBUTES
					+ ", got " + xpath);
		}
		...
		throw new IllegalRequestException("Attribute definition not found for " + attrQName + " (xpath "
				+ xpath + " )");
	}
		...
		try {
			...
			if (c instanceof ResourceType) {
				...
				try {
					...
					ResourceStateType.SynchronizationState syncToken = state == null ? null : state
							.getSynchronizationState();

					ResourceSchema schema = rai.getConnector().getSchema();

					// TODO: The hardcoded "Account" value should be removed,
					// this must be somehow dynamic
					ResourceObjectDefinition rad = rai.getConnector().getSchema()
							.getObjectDefinition(new QName(schema.getResourceNamespace(), "Account"));
					...
					for (SynchronizationResult.Change changeWrapper : changeCollection.getChanges()) {
						...
						// FIXME: This should be debug, not info
						logger.info("Synchronization (OID:{}) detected change: {}", resource.getOid(),
								DebugUtil.prettyPrint(change));
						...
						if (changeWrapper.getChange() instanceof ObjectChangeDeletionType) {
							ObjectChangeDeletionType ocd = (ObjectChangeDeletionType) changeWrapper
									.getChange();
							originalShadow = findOrCreateShadow(null, ocd.getOid(), resource);
							if (originalShadow == null) {
								// This shouldn't be an error. We are going
								// delete something that was
								// already deleted, so what the heck? Someone
								// already did that work ...
								// (this may well happen in case of sync message
								// re-delivery).

								// So just log a warning and pretend success.
								logger.warn(
										"Got delete sync message for already deleted shadow {}, resource {}. ignoring it",
										ocd.getOid(), resource.getName());
							...
							((ObjectChangeModificationType) change).getObjectModification().setOid(
									originalShadow.getOid());
							getRepository().modifyObject(
									((ObjectChangeModificationType) change).getObjectModification());
							ObjectContainerType object = getRepository().getObject(
									((ObjectChangeModificationType) change).getObjectModification().getOid(),
						...
						if (!(originalShadow instanceof ResourceObjectShadowType)) {
							logger.error(
									"### PROVISIONING # Fault synchronize(..): Invalid change object. The changed object is not a resourceObject. OID={}",
									originalShadow.getOid());
							// TODO: Is SystemFault appropriate here?
							throw new FaultMessage(
									"Invalid change object. The changed object is not a resourceObject. OID="
											+ originalShadow.getOid(), new SystemFaultType());
						}
					...
					logger.error("### PROVISIONING # Fault synchronize(..): Unknown error : {}",
							ex.getMessage());
					logger.debug("Unknown error " + ex);
				...
				logger.error("### PROVISIONING # Fault syncronize(..): Unsupported object type {} (OID: {})",
						c.getClass().getName(), oid);
				throw new FaultMessage("Unsupported object type " + c.getClass().getName() + " (OID: " + oid
						+ ")", new IllegalArgumentFaultType());
			}
		} catch (com.evolveum.midpoint.xml.ns._public.repository.repository_1.FaultMessage ex) {
	...
	public ResourceTestResultType testResource(String resourceOid) throws FaultMessage {
		...
		try {
			...
			JAXBElement<DiagnosticsMessageType> element = new JAXBElement<DiagnosticsMessageType>(
					SchemaConstants.I_DIAGNOSTICS_MESSAGE_ERROR, DiagnosticsMessageType.class, message);
	...
	@Override
	public synchronized EmptyType launchImportFromResource(String resourceOid, String objectClass)
			throws FaultMessage {
		...
		try {
			...
			logger.error("### PROVISIONING # Fault launchImportFromResource(..) : Repository error : {}",
					ex.getMessage());
			throw createFaultMessage("Repository invocation failed (getObject)", ex.getFaultInfo(), ex, null);
		} catch (ClassCastException ex) {
			logger.error(
					"### PROVISIONING # Fault launchImportFromResource(..): Object with OID {} is not a resource",
					resourceOid);
			throw new FaultMessage("Object with OID " + resourceOid + " is not a resource",
					new SystemFaultType());
		}
		...
		if (task != null && task.isAlive()) {
			logger.error(
					"### PROVISIONING # Fault launchImportFromResource({},{}): An import task is already running",
		...
		try {
			...
			objectDefinition = schema.getObjectDefinition(new QName(schema.getResourceNamespace(),
					objectClass));
		} catch (MidPointException ex) {
			logger.error(
					"### PROVISIONING # Fault launchImportFromResource(..): Definition of object class {} not found in resource schema",
					objectClass);
			throw new FaultMessage("Definition of object class " + objectClass
					+ " not found in resource schema", new SystemFaultType());
		}
		...
		logger.info("Staring import task, importing from resource {} (OID: {})", resource.getName(),
				resource.getOid());
		task.start();
	...
	public TaskStatusType getImportStatus(String resourceOid) throws FaultMessage {
		...
		try {
			...
			logger.error("### PROVISIONING # Fault getImportStatus(..) : Repository error : {}",
					ex.getMessage());
			throw createFaultMessage("Repository invocation failed (getObject)", ex.getFaultInfo(), ex, null);
		} catch (ClassCastException ex) {
			logger.error(
					"### PROVISIONING # Fault getImportStatus(..): Object with OID {} is not a resource, reason: {}",
	...
	private ResourceStateType getResourceState(String oid)
			throws com.evolveum.midpoint.xml.ns._public.repository.repository_1.FaultMessage {
	...
	// Returning newly created state object is necessary if the input state was
	// null
	private ResourceStateType refreshToken(ResourceStateType state, String oid, String resourceName,
			SynchronizationState syncToken)
			throws com.evolveum.midpoint.xml.ns._public.repository.repository_1.FaultMessage {
	...
	private ObjectType findOrCreateShadow(ResourceObject identifier, String acctOid, ResourceType resource)
			throws JAXBException, com.evolveum.midpoint.xml.ns._public.repository.repository_1.FaultMessage,
			FaultMessage {
		...
		if (results == null || results.getObject().size() != 1) {
			...
			if (identifier == null) {
				...
				// This code really should not log an error. Threfore changing
				// this
	...
	protected QueryType createSearchStateQuery(String oid) {
		...
		Element resourceRef = doc.createElementNS(SchemaConstants.I_RESOURCE_REF.getNamespaceURI(),
				SchemaConstants.I_RESOURCE_REF.getLocalPart());
	...
	// not finished yet.
	public static final String SEARCH_SHADOW_BY_UUID = "<c:query  xmlns:xsi='http://www.w3.org/2001/XMLSchema-instance'"
			...
			+ "     <c:and>"
			+ "         <c:type uri=\"http://midpoint.evolveum.com/xml/ns/public/common/common-1.xsd#AccountShadowType\"/>"
			+ "          <c:equal>"
			+ "             <c:path>c:attributes</c:path>"
			+ "             <c:value>"
			+ "                 <dj:__UID__>cn=foobar,uo=people,dc=nlight,dc=eu</dj:__UID__>"
			+ "             </c:value>" + "          </c:equal>" + "     </c:and>" + " </c:query>";
	public static final String SEARCH_BY_UUID = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>"
			+ "          <c:query  xmlns:xsi='http://www.w3.org/2001/XMLSchema-instance'"
			+ "   xmlns:c='http://midpoint.evolveum.com/xml/ns/public/common/common-1.xsd'"
			...
			+ "   xmlns:foo=\"http://foo.com/\">"
			+ "    <c:and>"
			+ "        <c:type uri=\"http://midpoint.evolveum.com/xml/ns/public/common/common-1.xsd#ResourceStateType\"/>"
			+ "        <c:equal>" + "            <c:value>" + "                <i:resourceRef oid=\"1234\"/>"
			+ "            </c:value>" + "        </c:equal>" + "    </c:and>" + " </c:query>";

	private void processScripts(OperationalResultType result, ResourceAccessInterface rai,
			ScriptsType scripts, ScriptOrderType order, OperationTypeType type) {
