...
class ShadowSearchLikeOperation {
    ...
    Integer executeCount(OperationResult result)
        ...
        if (shouldDoRepoSearch()) {
            return localBeans.shadowManager.countShadows(ctx, query, options, result);
        } else {
    ...
    private SearchResultMetadata executeIterativeSearchInRepository(
        ...
        try {
            return localBeans.shadowManager.searchShadowsIterative(
                    ctx,
                    query,
                    options,
    ...
    private @NotNull SearchResultList<PrismObject<ShadowType>> executeNonIterativeSearchInRepository(OperationResult result)
            throws SchemaException, ExpressionEvaluationException, CommunicationException, SecurityViolationException,
            ConfigurationException, ObjectNotFoundException {
        SearchResultList<PrismObject<ShadowType>> shadows = localBeans.shadowManager.searchShadows(ctx, query, options, result);
    ...
    private void processRepoShadow(PrismObject<ShadowType> shadow, OperationResult result)
        ...
        ProvisioningUtil.setProtectedFlag(
                ctx,
                shadow,
                SchemaService.get().matchingRuleRegistry(),
                SchemaService.get().relationRegistry(),
                localBeans.expressionFactory,
                result);
