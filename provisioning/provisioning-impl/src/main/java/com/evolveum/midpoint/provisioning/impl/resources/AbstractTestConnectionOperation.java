 ...
 */
abstract class AbstractTestConnectionOperation {

    private static final Trace LOGGER = TraceManager.getTrace(AbstractTestConnectionOperation.class);

    @NotNull protected final PrismObject<ResourceType> resource;
    @NotNull protected final Task task;
    @NotNull protected final CommonBeans beans;

    AbstractTestConnectionOperation(@NotNull PrismObject<ResourceType> resource, @NotNull Task task, @NotNull CommonBeans beans) {
    ...
    public void execute(OperationResult result)
        ...
        for (ConnectorSpec connectorSpec : allConnectorSpecs) {
            ...
            testConnector(connectorSpec, capabilityMap, connectorTestResult);
        ...
        try {
            ...
            return;
        } catch (GenericFrameworkException | ConfigurationException | ObjectNotFoundException | SchemaException |
                RuntimeException e) {
            String statusChangeReason = operationDesc + " failed while fetching schema: " + e.getMessage();
            if (LOGGER.isTraceEnabled()) {
        ...
        try {
            completedResource = completeResource(rawSchema, capabilityMap, schemaResult);
        ...
        // TODO: connector sanity (e.g. refined schema, at least one account type, identifiers in schema, etc.)
    }

    private PrismObject<ResourceType> completeResource(
            ResourceSchema rawSchema, Map<String, Collection<Object>> capabilityMap, OperationResult schemaResult) throws SchemaException, ExpressionEvaluationException, ConfigurationException, ObjectNotFoundException {
        PrismObject<ResourceType> resourceToComplete = getResourceToComplete(schemaResult);
        return new ResourceCompletionOperation(resourceToComplete, null, rawSchema, true, capabilityMap, task, beans)
                .execute(schemaResult);
    }

    protected abstract PrismObject<ResourceType> getResourceToComplete(OperationResult schemaResult) throws SchemaException, ObjectNotFoundException;

    protected abstract String createOperationDescription();
    ...
    protected abstract void setResourceAvailabilityStatus(AvailabilityStatusType status, String statusChangeReason, OperationResult result)
            throws ObjectNotFoundException;
    ...
    protected void testConnector(
        ...
        testConnection(connector, parentResult);

        parentResult.computeStatus();
        if (!parentResult.isAcceptable()) {
            operationCtx += ". " + getTestName() + " failed: " + parentResult.getMessage();
            markResourceDown(operationCtx, parentResult);
            ...
            operationCtx += ". " + getTestName() + " successful.";
    ...
    protected OperationResult createSubresultForTest(ConnectorSpec connectorSpec, OperationResult result) {
