...
/**
 * (This supporting class here is a version taken from the 4.7-M1, to facilitate back-porting MID-7918.
 * It should not present any problem, as the functionality was only added, not removed.)
 *
 ...
 * @author semancik
 */
@SuppressWarnings("unused") // Not all new methods are currently used in old (4.4.x) code
public abstract class ConnectorSpec {
     ...
     */
    public static @NotNull List<ConnectorSpec> all(@NotNull ResourceType resource) throws SchemaException {
    ...
    private static class Additional extends ConnectorSpec {
        ...
        private Additional(@NotNull ResourceType resource, @NotNull ConnectorInstanceSpecificationType definitionBean)
            ...
            schemaCheck(StringUtils.isNotBlank(name), "No connector name in additional connector in %s", resource);
