 ...
 */
public final class AccessCertificationNextStageRun
        extends LocalActivityRun
        <AccessCertificationNextStageWorkDefinition,
                AccessCertificationNextStageActivityHandler,
                AbstractActivityWorkStateType> {

    private static final Trace LOGGER = TraceManager.getTrace(AccessCertificationNextStageRun.class);

    private static final String CLASS_DOT = AccessCertificationNextStageRun.class.getName() + ".";

    AccessCertificationNextStageRun(
    ...
    protected @NotNull ActivityRunResult runLocally(OperationResult result) throws ActivityRunException, CommonException {
        @NotNull AccessCertificationNextStageActivityHandler handler = getActivityHandler();

        LOGGER.trace("Task run starting");

        OperationResult runResult = result.createSubresult(CLASS_DOT + "run");

        String definitionOid = null;//getWorkDefinition().getCertificationCampaignDefRef().getOid();
