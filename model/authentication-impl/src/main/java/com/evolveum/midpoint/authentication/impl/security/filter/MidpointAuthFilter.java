...
import com.evolveum.midpoint.authentication.impl.security.MidpointAuthenticationManager;
...
public class MidpointAuthFilter extends GenericFilterBean {
    ...
    private void doFilterInternal(ServletRequest request, ServletResponse response,
        ...
        if (AuthSequenceUtil.isPermitAll(httpRequest) && !AuthSequenceUtil.isLoginPage(httpRequest)) {
        ...
        if (AuthSequenceUtil.isIgnoredLocalPath(authenticationsPolicy, httpRequest)) {
            chain.doFilter(request, response);
            return;
        }
        ...
        if (sequence == null) {
            ...
            if (securityPolicy != null) {
                message += "' in authentication of Security Policy with oid " + securityPolicy.getOid();
            } else {
        ...
        try {
            ...
            if (AuthSequenceUtil.isSpecificSequence(httpRequest)) {
                if (authModulesOfSpecificSequences.containsKey(sequence.getName())) {
            ...
            VirtualFilterChain vfc = new VirtualFilterChain(
                    chain, ((AuthModuleImpl)authModules.get(indexOfProcessingModule)).getSecurityFilterChain().getFilters());
    ...
    private AuthenticationSequenceType getAuthenticationSequence(MidpointAuthentication mpAuthentication, HttpServletRequest httpRequest, AuthenticationsPolicyType authenticationsPolicy) {
        ...
        if (mpAuthentication != null && AuthSequenceUtil.isLoginPage(httpRequest)) {
            ...
            sequence = AuthSequenceUtil.getSequenceByPath(httpRequest, authenticationsPolicy, taskManager.getLocalNodeGroups());
        }
