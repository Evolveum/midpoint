...
/**
 * Result handler for the validity scanner.
 */
public class FocusValidityScannerResultHandler
    ...
    private static final Trace LOGGER = TraceManager.getTrace(FocusValidityScannerResultHandler.class);

    public FocusValidityScannerResultHandler(FocusValidityScannerTaskPartExecution taskExecution) {
        super(taskExecution);
    }
    ...
    protected boolean handleObject(PrismObject<FocusType> object, RunningTask workerTask, OperationResult result)
        ...
        if (taskExecution.oidAlreadySeen(object.getOid())) {
