...
public class SchemaHandling extends XPathUtil {
    ...
    private void evaluateAndApplyOutboundExpressionValues(UserType user, AccountShadowType accountShadow, ResourceType resource) throws SchemaHandlingException {
        TRACE.debug("SCHEMA HANDLING: Account before processing of schema handling: {}", DebugUtil.prettyPrint(accountShadow));
        TRACE.trace("Account before processing of schema handling: {}", JAXBUtil.silentMarshalWrap(accountShadow, SchemaConstants.I_ACCOUNT_TYPE));
        ...
        for (AttributeDescriptionType attributeHandling : attributesHandling) {
            ...
            if (null != attributeDefinition) {
                ...
                TRACE.warn("Attribute {} defined in schema handling is not defined in the resource schema (oid={}). Attribute was not processed", attributeHandling.getRef(), resource.getOid());
            }
        }

        TRACE.debug("SCHEMA HANDLING: Account with applied schema handling: {}", DebugUtil.prettyPrint(accountShadow));
        TRACE.trace("Account with applied schema handling: {}", JAXBUtil.silentMarshalWrap(accountShadow, SchemaConstants.I_ACCOUNT_TYPE));
    ...
    private UserType evaluateAndApplyInboundExpressionValues(UserType user, ResourceObjectShadowType accountShadow) throws SchemaHandlingException {
        TRACE.debug("INBOUND: User before processing of inbound expressions: {}", DebugUtil.prettyPrint(user,true));
        TRACE.trace("User before processing of inbound expressions: {}", JAXBUtil.silentMarshalWrap(user, SchemaConstants.I_USER_TYPE));
        ...
        TRACE.trace("User with applied inbound expressions: {}", JAXBUtil.silentMarshalWrap(user, SchemaConstants.I_USER_TYPE));
    ...
    private void applyValue(AccountShadowType account, QName accountAttributeName, String accountAttributeValue) {
        ...
        for (Element attribute : attributes) {
            TRACE.trace("attribute: namespaceURI = '{}', localName = '{}'", attribute.getNamespaceURI(), attribute.getLocalName());
            if (localName.equals(attribute.getLocalName()) && namespace.equals(attribute.getNamespaceURI())) {
                ...
                TRACE.trace("Changed account's attribute '{}' value to '{}'", accountAttributeName, accountAttributeValue);
                return;
            }
        }
        ...
        TRACE.trace("Created account's attribute '{}' with value '{}'", accountAttributeName, accountAttributeValue);
...
//                variables.put(SchemaConstants.I_USER, new Variable(userEl, false));
    ...
    private ResourceObjectDefinition getResourceObjectDefinition(ResourceType resource) throws SchemaHandlingException {
        ...
        if (null == objectDefinition) {
            TRACE.error("Object definition with QName {} not found in the resource schema (oid={})", objectDefinitionQName, resource.getOid());
    ...
    private void processOutboundSection(AttributeDescriptionType attributeHandling, Map<QName, Variable> variables, AccountShadowType accountShadow) throws SchemaHandlingException {
        ...
        TRACE.trace("Start outbound processing of attribute handling for attribute '{}'", accountAttributeName);
        ...
        if (isApplicablePropertyConstruction(attributeHandling.getOutbound().isDefault(), xpathType, variables, accountShadow)) {
            if (null != expressionHolder && !StringUtils.isEmpty(expressionHolder.getExpressionAsString())) {
                ...
                if (TRACE.isDebugEnabled()) {
                    TRACE.debug("OUTBOUND: expression '{}' for attribute {}, resource {}, was evaluated to '{}'", new Object[]{expressionHolder.getExpressionAsString(), accountAttributeName, DebugUtil.resourceFromShadow(accountShadow), accountAttributeValue});
                    if (variables != null) {
        ...
        TRACE.trace("Finished outbound processing of attribute handling for attribute '{}'", accountAttributeName);
    }
    ...
    private Node processInboundSection(AttributeDescriptionType attributeHandling, Map<QName, Variable> variables, ResourceObjectShadowType accountShadow, Node domUser, UserType user) throws SchemaHandlingException {
        ...
        if (null == inbounds || inbounds.size() == 0) {
            TRACE.trace("Finished inbound processing of attribute handling for attribute '{}' - nothing to process", accountAttributeName);
            return domUser;
        }
        ...
        for (ValueAssignmentHolder inbound : inbounds) {
            ...
            if (matchedNodes.getLength() == 0) {
                ...
                TRACE.debug("INBOUND: expression [attribute:{}] =(new)=> [user:{},path:{}] {}",new Object[]{ attributeHandling.getName(), DebugUtil.prettyPrint(user), xpathType, DebugUtil.prettyPrint(newNodes) });
                ...
                //IV. set modified user
                domUser = parentNode.getOwnerDocument().getFirstChild();
                TRACE.debug("INBOUND: expression [attribute:{}] =(replace)=> [user:{},path:{}] {}",new Object[]{ attributeHandling.getName(), DebugUtil.prettyPrint(user), xpathType, DebugUtil.prettyPrint(newNodes) });
                Validate.notNull(domUser, "null value assigned to domUser");
            }

        }
        TRACE.trace("Finished inbound processing of attribute handling for attribute '{}'", accountAttributeName);
//        if (null != domUser) {
//            domUser.normalize();
//        }
    ...
    public UserType applyUserTemplate(UserType user, UserTemplateType userTemplate) throws PatchException, SchemaHandlingException {
        ...
        TRACE.debug("USER TEMPLATE: Processing template {} for user {}",DebugUtil.prettyPrint(userTemplate),DebugUtil.prettyPrint(user));
        ...
        for (AccountConstructionType accountConstr : userTemplate.getAccountConstruction()) {
            ...
            if (accountConstr.getResource()!=null) {
                ...
                TRACE.error("Account construction in user template (OID:{}) must have resource or resourceRef dedined.",userTemplate.getOid());
            ...
            if (UserTypeUtil.findAccountRef(user, resourceOid)!=null) {
                TRACE.debug("USER TEMPLATE: Account on resource {} already exists for user {}, skipping",resourceOid,user.getOid());
                continue;
            }
            ...
            if (object == null) {
                ...
                if (objectType != null) {
                    ...
                    try {
                        ...
                        TRACE.warn("Variable '{}' couldn't be serialized to XML, skipping. Error: {}", varDef.getName(), ex.getMessage());
                        continue;
                    }
                }
            }
