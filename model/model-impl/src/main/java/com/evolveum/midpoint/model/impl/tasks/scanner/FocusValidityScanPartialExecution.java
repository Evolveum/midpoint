...
public class FocusValidityScanPartialExecution
        extends AbstractScanActivityExecution<FocusType, FocusValidityScanWorkDefinition, FocusValidityScanActivityHandler> {
    ...
    private TimeValidityPolicyConstraintType getValidityConstraint() {
        return activity.getWorkDefinition().getValidityConstraint();
    }
    ...
    private ObjectFilter createFilterForItemTimestamp(ItemPath path,
        ...
        if (lowerBound == null) {
            return getPrismContext().queryFor(getObjectType())
    ...
    @Override
    protected @NotNull ItemProcessor<PrismObject<FocusType>> createItemProcessor(OperationResult opResult) {
        return createDefaultItemProcessor(
                (object, request, workerTask, result) -> recomputeFocus(object, workerTask, result));
    }


    private boolean recomputeFocus(PrismObject<FocusType> focus, RunningTask workerTask, OperationResult result)
    ...
    private LensContext<FocusType> createLensContext(PrismObject<FocusType> focus, RunningTask workerTask, OperationResult result)
        ...
        ModelExecuteOptions options = new ModelExecuteOptions(getPrismContext()).reconcile();
