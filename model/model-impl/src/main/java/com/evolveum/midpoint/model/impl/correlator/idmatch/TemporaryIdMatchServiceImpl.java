...
/**
 * TEMPORARY IMPLEMENTATION - until real {@link IdMatchServiceImpl} is ready.
 *
 * A dummy implementation of {@link IdMatchService} used to test IdMatchCorrelator without actual ID Match Service.
 */
public class TemporaryIdMatchServiceImpl implements IdMatchService {

    public static final IdMatchService INSTANCE = new TemporaryIdMatchServiceImpl();

    /** Instantiated when this URL is provided. */
    public static final String URL = "dummy";
    ...
    public @NotNull MatchingResult executeMatch(@NotNull ShadowAttributesType attributes, @NotNull OperationResult result) {
        ...
        if (!fullMatch.isEmpty()) {
            stateCheck(fullMatch.size() == 1, "Fully matching reference IDs is not of size 1: %s", fullMatch);
