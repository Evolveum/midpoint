...
public class ShadowIntegrityCheckActivityExecution
    ...
    protected void initializeExecution(OperationResult opResult)
        ...
        ExecutionModeType executionMode = activity.getDefinition().getExecutionMode();
        configuration = new ShadowCheckConfiguration(LOGGER, activity.getWorkDefinition(), executionMode);
    ...
    protected @NotNull ItemProcessor<PrismObject<ShadowType>> createItemProcessor(OperationResult opResult)
        ...
        return createDefaultItemProcessor(
                new ShadowIntegrityCheckItemProcessor(this));
    }
    ...
    private void deleteShadows(DuplicateShadowsTreatmentInstruction instruction, StringBuilder sb, Task task, OperationResult result) {
        ...
        for (PrismObject<ShadowType> shadowToDelete : shadowsToDelete) {
            ...
            for (PrismObject<FocusType> owner : owners) {
                ...
                ReferenceDelta deleteDelta = getPrismContext().deltaFactory().reference().createModificationDelete(FocusType.F_LINK_REF, owner.getDefinition(),
                        getPrismContext().itemFactory().createReferenceValue(oid, ShadowType.COMPLEX_TYPE));
                modifications.add(deleteDelta);
                if (shadowOidToReplaceDeleted != null) {
                    ReferenceDelta addDelta = getPrismContext().deltaFactory().reference().createModificationAdd(FocusType.F_LINK_REF, owner.getDefinition(),
                            getPrismContext().itemFactory().createReferenceValue(shadowOidToReplaceDeleted, ShadowType.COMPLEX_TYPE));
    ...
    List<PrismObject<FocusType>> searchOwners(PrismObject<ShadowType> shadow, OperationResult result) {
        try {
            ObjectQuery ownerQuery = beans.prismContext.queryFor(FocusType.class)
