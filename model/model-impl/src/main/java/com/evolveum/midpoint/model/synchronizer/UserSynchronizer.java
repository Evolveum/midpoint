...
public class UserSynchronizer {
	...
	public HookOperationMode synchronizeUser(SyncContext context, Task task, OperationResult result) throws SchemaException,
        ...
        // via inbound, e.g. identifiers generated by the resource, DNs and similar things.
        ...
        while (context.getWave() < maxWaves) {
        	if (attempt > maxAttempt) {
        		throw new PolicyViolationException("Too many attempts to synchronize user (wave "+context.getWave()+")");
        	}
	        ...
	        for (AccountSyncContext accountContext: context.getAccountContexts()) {
	        	if (accountContext.getWave() != context.getWave()) {
	        	...
	        	context.recomputeNew();
		        SynchronizerUtil.traceContext("values computation", context, false);
		        if (consistenceChecks) context.checkConsistence();
