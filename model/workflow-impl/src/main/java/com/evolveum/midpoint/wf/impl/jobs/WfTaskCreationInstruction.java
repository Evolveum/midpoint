 ...
 * @author mederly
 */
public class WfTaskCreationInstruction<PRI extends ProcessorInstruction, PCI extends ProcessInstruction> implements DebugDumpable {
	...
	protected final PRI processorInstruction;
	private final PCI processInstruction;
    ...
    //region Constructors
    protected WfTaskCreationInstruction(ChangeProcessor changeProcessor, PRI processorInstruction, PCI processInstruction) {
        Validate.notNull(changeProcessor);
        this.changeProcessor = changeProcessor;
		this.processorInstruction = processorInstruction;
    ...
    public static WfTaskCreationInstruction createWfOnly(ChangeProcessor changeProcessor,
			ProcessorInstruction processorInstruction, ProcessInstruction processInstruction) {
        WfTaskCreationInstruction instruction = new WfTaskCreationInstruction(changeProcessor, processorInstruction, processInstruction);
        return instruction;
    }
    ...
    public String debugDump(int indent) {
		...
		DebugUtil.debugDumpWithLabel(sb, "Processor instruction", String.valueOf(processorInstruction), indent+1);
		DebugUtil.debugDumpWithLabel(sb, "Process instruction", String.valueOf(processInstruction), indent+1);
	...
	public Task createTask(WfTaskController taskController, Task parentTask) throws SchemaException {
		...
		if (processorInstruction != null) {
			wfContext.setProcessorSpecificState(processorInstruction.createProcessorSpecificState());
		}
		if (processInstruction != null) {
			wfContext.setProcessSpecificState(processInstruction.createProcessSpecificState());
		}
	...
	public Map<String, Object> getAllProcessVariables() throws SchemaException {
		...
		if (processorInstruction != null) {
			processorInstruction.createProcessVariables(map, getPrismContext());
		}
		if (processInstruction != null) {
			processInstruction.createProcessVariables(map, getPrismContext());
		}
		return map;
	}
