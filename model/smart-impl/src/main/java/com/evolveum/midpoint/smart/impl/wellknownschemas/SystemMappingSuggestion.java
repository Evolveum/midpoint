                ...
                .expressionEvaluator(
                        new ObjectFactory().createScript(
                                new ScriptExpressionEvaluatorType()
                                        .language("mel")
                                        .code(script)));
        // A trust descriptor is necessary to execute the script.
        // We choose built-in safeScriptingProfile, as it is safe enough to run code generated by an LLM.
        expression.setTrustDescriptor(
                MidPointTrustDescriptor.explicit(ExpressionProfile.mappingsQualityAssessment()));
        return new SystemMappingSuggestion(
                ItemPath.create(ShadowType.F_ATTRIBUTES, ItemName.from(SchemaConstants.NS_RI, shadowAttrName)),
                focusPropertyPath,
                expression,
