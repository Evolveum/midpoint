...
@Component
public class TrimLowerCaseAndStripDiacriticsHeuristic implements HeuristicRule {

    @Override
    public String getName() {
        return "trimLowerCaseAndStripDiacritics";
    }

    @Override
    public String getDescription() {
        return "Trim whitespace, convert to lowercase, and strip diacritical marks";
    }
    ...
    public boolean isApplicable(ValuesPairSample<?, ?> sample) {
        return sample.pairs().stream()
                ...
                .anyMatch(str -> str != null && 
                        (hasDiacritics(str) || !str.equals(str.trim()) || !str.equals(str.toLowerCase())));
    }
    ...
    public ExpressionType inboundExpression(MappingExpressionFactory factory) {
        return factory.createScriptExpression(
                "basic.toAscii(input?.trim())?.toLowerCase()",
    ...
    public ExpressionType outboundExpression(String focusPropertyName, MappingExpressionFactory factory) {
        return factory.createScriptExpression(
                "basic.toAscii(" + focusPropertyName + "?.trim())?.toLowerCase()",
                "Trim, convert to lowercase, and strip diacritical marks");
    }
}
