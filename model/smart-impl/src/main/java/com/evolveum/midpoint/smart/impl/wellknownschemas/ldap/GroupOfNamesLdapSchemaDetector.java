...
@Component
public class GroupOfNamesLdapSchemaDetector implements WellKnownSchemaDetector {
    ...
    private static final Set<String> LDAP_GROUP_OBJECT_CLASS_NAMES = Set.of(
            "groupofnames",
            "groupofuniquenames"
    );

    private static final Set<String> LDAP_GROUP_ATTRIBUTES_LOWERCASE = Set.of(
            "cn",
            "dn",
            "description"
    );
    ...
    public Optional<WellKnownSchemaType> detectSchemaType(ResourceType resource, ResourceObjectTypeDefinition typeDefinition) {
        ...
        if (connectorRef != null && connectorRef.getOid() != null) {
            ...
            if (LDAP_CONNECTOR_TYPES.stream().anyMatch(type -> connectorType.contains(type))) {
                return Optional.of(WellKnownSchemaType.LDAP_GROUP_OF_NAMES);
            }
        }
        ...
        if (attributeNames.containsAll(LDAP_GROUP_ATTRIBUTES_LOWERCASE)) {
            return Optional.of(WellKnownSchemaType.LDAP_GROUP_OF_NAMES);
        }
