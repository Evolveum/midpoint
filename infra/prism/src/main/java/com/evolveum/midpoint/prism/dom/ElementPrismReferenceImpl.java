...
public class ElementPrismReferenceImpl<T> extends ElementPrismAbstractImpl {
	
	private NodeList valueNodeList;
	
	public ElementPrismReferenceImpl(PrismReferenceValue value) {
		super(value);
		this.valueNodeList = new NodeListEmptyImpl();
	}
	
	protected PrismReferenceValue getValue() {
		return (PrismReferenceValue)super.getValue();
	}
	
	protected PrismReference getProperty() {
		return (PrismReference)getItem();
	}
	...
	protected Map<String,String> getAttributeMap() {
		...
		map.put(PrismConstants.ATTRIBUTE_OID_LOCAL_NAME, getValue().getOid());
		// TODO
//		if (getValue().getTargetType() != null) {
//			map.put(PrismConstants.ATTRIBUTE_REF_TYPE_LOCAL_NAME, getValue().getTargetType());
//		}
		return map;
	}
