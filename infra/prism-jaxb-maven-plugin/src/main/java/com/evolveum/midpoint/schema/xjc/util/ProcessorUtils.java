...
public final class ProcessorUtils {
    ...
    public static void createQName(JCodeModel codeModel, JDefinedClass targetClass, String targetField, QName qname, JFieldVar namespaceField, boolean namespaceFieldIsLocal, boolean createPath) {
        JExpression namespaceArgument;
        if (namespaceField != null) {
            if (namespaceFieldIsLocal) {
                namespaceArgument = namespaceField;
            } else {
                JClass schemaClass = codeModel._getClass(StepSchemaConstants.SCHEMA_CONSTANTS_GENERATED_CLASS_NAME);
                namespaceArgument = schemaClass.staticRef(namespaceField);
            }
        } else {
            namespaceArgument = JExpr.lit(qname.getNamespaceURI());
        }
        createNameConstruction(codeModel, targetClass, targetField, qname, namespaceArgument, createPath ? ItemName.class : QName.class);
    }

    private static void createNameConstruction(JCodeModel codeModel, JDefinedClass definedClass, String fieldName,
