...
public class RefinedObjectClassDefinitionImpl implements RefinedObjectClassDefinition {
    ...
    private static RefinedObjectClassDefinition parseRefinedObjectClass(ResourceObjectTypeDefinitionType schemaHandlingObjDefType,
        ...
        if (rOcDef.schemaHandlingObjectTypeDefinitionType.isDefault() != null) {
            defaultValue = rOcDef.schemaHandlingObjectTypeDefinitionType.isDefault();
            LOGGER.trace("Object class definition has defined defaultness with value of {}", defaultValue);
        } else {
            defaultValue = objectClassDef.isDefaultInAKind();
            LOGGER.trace("Object class definition is not explicitly marked as default, so defaultness may be calculated based on schema: {}", defaultValue);
        }
        rOcDef.setDefault(defaultValue);
        ...
        LOGGER.trace("Object class definition parsed and refined is: {}", rOcDef);
        return rOcDef;
    }
