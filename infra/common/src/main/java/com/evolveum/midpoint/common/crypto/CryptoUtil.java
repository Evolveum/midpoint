...
public class CryptoUtil {
	...
	private static void applyToRealValue(Object value, String propertyName, ProtectedStringProcessor processor)
		...
		if (value instanceof ProtectedStringType) {
			...
			} else if (raw.getXnode() != null && raw.getXnode().getTypeQName() != null) {
				try {
		...
		} else if (value instanceof NotificationConfigurationType) {
			NotificationConfigurationType cfg = (NotificationConfigurationType) value;
			applyToRealValue(cfg.getMail(), propertyName+"/mail", processor);
			applyToRealValue(cfg.getSms(), propertyName+"/sms", processor);
