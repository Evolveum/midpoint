...
public class RelationshipsConnectorDevPartItem extends AbstractWizardPartItem<ConnectorDevelopmentType, ConnectorDevelopmentDetailsModel> {

//    private static final Trace LOGGER = TraceManager.getTrace(NextConnectorDevStatusItem.class);

    protected RelationshipsConnectorDevPartItem(WizardPanelHelper<? extends Containerable, ConnectorDevelopmentDetailsModel> helper) {
        super(helper);
    }
    ...
    protected List<WizardParentStep> createWizardSteps() {
        return List.of(new RelationshipsConnectorStepPanel(getHelper()));
    }

    @Override
    public Enum<?> getIdentifierForWizardStatus() {
        return ConnectorDevelopmentStatusType.RELATIONSHIPS;
    }
}
