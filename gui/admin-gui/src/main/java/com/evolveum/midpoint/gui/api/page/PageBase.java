...
public abstract class PageBase extends WebPage implements ModelServiceLocator {
	...
	// No need for this to store in session. It is used only during single init and
	// render.
	...
	private void initializeModel() {
		workItemCountModel = new LoadableModel<Integer>() {
			...
			protected Integer load() {
				...
				} catch (SchemaException | SecurityViolationException | ExpressionEvaluationException
		...
		certWorkItemCountModel = new LoadableModel<Integer>() {
			...
			protected Integer load() {
				try {
					...
					return acs.countOpenWorkItems(getPrismContext().queryFactory().createQuery(), true, null, task,
							result);
				} catch (SchemaException | SecurityViolationException | ObjectNotFoundException | ConfigurationException
	...
	public CompiledUserProfile getCompiledUserProfile() {
		// TODO: may need to always go to ModelInteractionService to make sure the
		...
		if (compiledUserProfile == null) {
			...
			try {
				...
				if (InternalsConfig.nonCriticalExceptionsAreFatal()) {
					...
					// Just return empty admin GUI config, so the GUI can go on (and the problem may
					// get fixed)
	...
	public <O extends ObjectType, T extends ObjectType> boolean isAuthorized(String operationUrl,
			...
			OwnerResolver ownerResolver) throws SchemaException, ObjectNotFoundException, ExpressionEvaluationException,
			CommunicationException, ConfigurationException, SecurityViolationException {
		Task task = getPageTask();
		AuthorizationParameters<O, T> params = new AuthorizationParameters.Builder<O, T>().oldObject(object)
				.delta(delta).target(target).build();
		boolean isAuthorized = getSecurityEnforcer().isAuthorized(operationUrl, phase, params, ownerResolver, task,
				task.getResult());
		if (!isAuthorized && (ModelAuthorizationAction.GET.getUrl().equals(operationUrl)
				|| ModelAuthorizationAction.SEARCH.getUrl().equals(operationUrl))) {
			isAuthorized = getSecurityEnforcer().isAuthorized(ModelAuthorizationAction.READ.getUrl(), phase, params,
					ownerResolver, task, task.getResult());
		}
		return isAuthorized;
	}
	...
	public <O extends ObjectType, T extends ObjectType> void authorize(String operationUrl,
			AuthorizationPhaseType phase, PrismObject<O> object, ObjectDelta<O> delta, PrismObject<T> target,
			OwnerResolver ownerResolver, OperationResult result) throws SecurityViolationException, SchemaException,
	...
	public <O extends ObjectType, T extends ObjectType> void authorize(String operationUrl,
			AuthorizationPhaseType phase, PrismObject<O> object, ObjectDelta<O> delta, PrismObject<T> target,
			OwnerResolver ownerResolver) throws SecurityViolationException, SchemaException, ObjectNotFoundException,
	...
	public Task createSimpleTask(String operation, String channel) {
		...
		return WebModelServiceUtils.createSimpleTask(operation, channel, user.getUser().asPrismObject(),
				getTaskManager());
	}
	...
	public void renderHead(IHeaderResponse response) {
		...
		String skinCssPath = String.format("../../../../../../webjars/adminlte/2.3.11/dist/css/skins/%s.min.css",
				skinCssString);
		response.render(CssHeaderItem.forReference(new CssResourceReference(PageBase.class, skinCssPath)));
	...
	private void initLayout() {
		...
		customLogoImgSrc.add(new VisibleBehaviour(
				() -> logoModel.getObject() != null && StringUtils.isEmpty(logoModel.getObject().getCssClass())));
		...
		customLogoImgCss.add(new VisibleBehaviour(
				() -> logoModel.getObject() != null && StringUtils.isNotEmpty(logoModel.getObject().getCssClass())));
		...
		mainHeader.add(new AttributeAppender("style", new IModel<String>() {
			...
			@Override
			public String getObject() {
				return logoModel.getObject() != null
		...
		SideBarMenuPanel sidebarMenu = new SideBarMenuPanel(ID_SIDEBAR_MENU,
				new LoadableModel<List<SideBarMenuItem>>(false) {
		...
		version.add(new VisibleBehaviour(() -> isFooterVisible()
...
//                }
	...
	private OperationResult executeResultScriptHook(OperationResult result) {
		...
		try {
			...
			PrismPropertyDefinition<OperationResultType> outputDefinition = getPrismContext().definitionFactory()
					.createPropertyDefinition(ExpressionConstants.OUTPUT_ELEMENT_NAME,
							OperationResultType.COMPLEX_TYPE);
			Expression<PrismPropertyValue<OperationResultType>, PrismPropertyDefinition<OperationResultType>> expression = factory
					.makeExpression(expressionType, outputDefinition, contextDesc, task, topResult);
			...
			ExpressionEvaluationContext context = new ExpressionEvaluationContext(null, variables, contextDesc, task,
					topResult);
			PrismValueDeltaSetTriple<PrismPropertyValue<OperationResultType>> outputTriple = expression
					.evaluate(context);
	...
	/**
	 * It's here only because of some IDEs - it's not properly filtering resources
	 * during maven build. "describe" variable is not replaced.
	 *
	...
	public <T> void parseObject(String lexicalRepresentation, final Holder<T> objectHolder, String language,
			boolean validateSchema, boolean skipChecks, Class<T> clazz, OperationResult result) {

		boolean isListOfObjects = List.class.isAssignableFrom(clazz);
		boolean isObjectable = Objectable.class.isAssignableFrom(clazz);
		if (skipChecks || language == null || PrismContext.LANG_JSON.equals(language)
				|| PrismContext.LANG_YAML.equals(language) || (!isObjectable && !isListOfObjects)) {
			T object;
			try {
				if (isListOfObjects) {
					List<PrismObject<? extends Objectable>> prismObjects = getPrismContext()
							.parserFor(lexicalRepresentation).language(language).parseObjects();
					if (!skipChecks) {
					...
					PrismObject<ObjectType> prismObject = getPrismContext().parserFor(lexicalRepresentation)
					...
					object = getPrismContext().parserFor(lexicalRepresentation).language(language).type(clazz)
							.parseRealValue();
				}
				objectHolder.setValue(object);
			} catch (RuntimeException | SchemaException e) {
				result.recordFatalError(
						createStringResource("PageBase.message.parseObject.fatalError", e.getMessage()).getString(), e);
			}
			return;
		}
...
//        }
		...
		if (WebComponentUtil.isAuthorized(AuthorizationConstants.AUTZ_UI_MY_WORK_ITEMS_URL,
				...
				AuthorizationConstants.AUTZ_UI_CASE_WORK_ITEMS_ALLOCATED_TO_ME_URL,
				AuthorizationConstants.AUTZ_UI_CASE_WORK_ITEMS_ALL_URL,
	...
	private void createConfigurationMenu(SideBarMenuItem item) {
		...
		MainMenuItem debugs = addMainMenuItem(item, "fa fa-file-text",
				"PageAdmin.menu.top.configuration.repositoryObjects", null);
		...
		if (WebModelServiceUtils.isEnableExperimentalFeature(this)) {
			addMainMenuItem(item, "fa fa-cog", "PageAdmin.menu.top.configuration.evaluateMapping",
	...
	private MainMenuItem addMainMenuItem(SideBarMenuItem item, String icon, String key,
			Class<? extends PageBase> page) {
	...
	private MainMenuItem createWorkItemsItems() {
		...
		addMenuItem(item, "PageAdmin.menu.top.workItems.listProcessInstancesRequestedBy",
				PageProcessInstancesRequestedBy.class);
		addMenuItem(item, "PageAdmin.menu.top.workItems.listProcessInstancesRequestedFor",
...
//        addCollectionsMenuItems(item.getItems(), TaskType.COMPLEX_TYPE);
	...
	private MainMenuItem createValuePolicieItems() {
		MainMenuItem item = new MainMenuItem("fa fa-asterisk", createStringResource("PageAdmin.menu.top.valuePolicies"),
				null);
	...
	private void createSelfServiceMenu(SideBarMenuItem menu) {
		...
		addMainMenuItem(menu, GuiStyleConstants.CLASS_ICON_PROFILE, "PageAdmin.menu.profile", PageSelfProfile.class);
...
//                PageSelfConsents.class);
			...
			for (RichHyperlinkType link : menuList) {
				if (link.getTargetUrl() != null && !link.getTargetUrl().trim().equals("")) {
					AdditionalMenuItem item = new AdditionalMenuItem(
							...
							new Model<String>(link.getLabel()), link.getTargetUrl(),
	...
	private MenuItem getDashboardMenuItem(IModel<String> nameModel, Class<? extends PageBase> pageClass) {
		MenuItem menu = new MenuItem(nameModel, PageDashboardInfo.class) {
			private static final long serialVersionUID = 1L;
	...
	private MainMenuItem createUsersItems() {
		...
		createFocusPageNewEditMenu(item.getItems(), "PageAdmin.menu.top.users.new", "PageAdmin.menu.top.users.edit",
				PageUser.class, true);
	...
	private boolean isMenuItemAuthorized(Class<? extends PageAdmin> newPageClass) {
		try {
			...
			return isAuthorized(ModelAuthorizationAction.ADD.getUrl(), AuthorizationPhaseType.REQUEST,
					object == null ? null : object.asPrismObject(), null, null, null);
			...
			LoggingUtils.logUnexpectedException(LOGGER, "Couldn't solve authorization for New organization menu item",
					ex);
		}
		return false;
	}
	...
	private MainMenuItem createOrganizationsMenu() {
		...
		createFocusPageNewEditMenu(item.getItems(), "PageAdmin.menu.top.users.org.new",
				"PageAdmin.menu.top.users.org.edit", PageOrgUnit.class, true);
...
//        addCollectionsMenuItems(item.getItems(), OrgType.COMPLEX_TYPE);
	...
	private MainMenuItem createServicesItems() {
		...
		createFocusPageNewEditMenu(item.getItems(), "PageAdmin.menu.top.services.new",
				"PageAdmin.menu.top.services.edit", PageService.class, true);
	...
	private void addCollectionsMenuItems(List<MenuItem> menu, QName type,
			Class<? extends PageAdminObjectList> redirectToPage) {
		List<CompiledObjectCollectionView> objectViews = getCompiledUserProfile()
				.findAllApplicableObjectCollectionViews(type);
		if (objectViews == null) {
			return;
		}
		objectViews.forEach(objectView -> {
			// objectlistView.getType() might be null - from documentation:
			// It may not be present in case that the type is defined in a referenced object
			// colleciton.
			...
			PrismObject<? extends ObjectType> collectionObject = WebModelServiceUtils
					.resolveReferenceNoFetch(collectionRef, this, task, result);
			...
			MenuItem userViewMenu = new MenuItem(
					viewDisplayType != null && PolyStringUtils.isNotEmpty(viewDisplayType.getLabel())
							? createStringResource(viewDisplayType.getLabel())
							: createStringResource("MenuItem.noName"),
					redirectToPage, pageParameters, null) {
				private static final long serialVersionUID = 1L;
	...
	public DeploymentInformationType loadDeploymentInformationType() {
		...
		try {
			...
			result.recordFatalError(
					createStringResource("PageBase.message.loadDeploymentInformationType.fatalError").getString(), ex);
		}
	...
	public PrismObject<UserType> getAdministratorPrivileged(OperationResult parentResult) throws CommonException {
		...
		try {
			return securityContextManager.runPrivilegedChecked(() -> {
				...
				return getModelService().getObject(UserType.class, SystemObjectsType.USER_ADMINISTRATOR.value(), null,
						task, result);
			});
		} catch (Throwable t) {
			result.recordFatalError(
					createStringResource("PageBase.message.getAdministratorPrivileged.fatalError", t.getMessage())
							.getString(),
					t);
			throw t;
		} finally {
	...
	public void addBreadcrumb(Breadcrumb breadcrumb) {
		...
		Breadcrumb last = getBreadcrumbs().isEmpty() ? null : getBreadcrumbs().get(getBreadcrumbs().size() - 1);
