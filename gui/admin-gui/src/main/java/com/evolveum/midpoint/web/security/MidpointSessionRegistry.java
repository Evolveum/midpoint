...
public class MidpointSessionRegistry extends SessionRegistryImpl {
    ...
    @Override
    public void onApplicationEvent(SessionDestroyedEvent sessionEvent) {
        super.onApplicationEvent(sessionEvent);
        for (SecurityContext context : sessionEvent.getSecurityContexts()) {
            if (context != null && context.getAuthentication() instanceof MidpointAuthentication) {
