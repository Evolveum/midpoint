...
@PageDescriptor(
        urls = {
                @Url(mountUrl = "/admin/config/action", matchUrlForSecurity = "/admin/config/action")
        },
        action = {
        ...
        @AuthorizationAction(actionUri = AuthorizationConstants.AUTZ_UI_BULK_ACTION_URL,
                label = "PageAction.auth.action.label", description = "PageAction.auth.action.description")
})
public class PageAction extends PageAdminConfiguration {

    private static final Trace LOGGER = TraceManager.getTrace(PageAction.class);
    ...
    private void initLayout() {
        ...
        AjaxSubmitButton start = new AjaxSubmitButton(ID_START, createStringResource("PageAction.button.start")) {
        ...
        try {
            ...
            if (parsed == null) {
                result.recordFatalError(
                        createStringResource("PageAction.message.startPerformed.fatalError.provided").getString());
            } else if (!(parsed instanceof ExecuteScriptType) && !(parsed instanceof ScriptingExpressionType)) {
                result.recordFatalError(
                        createStringResource(
                                "PageAction.message.startPerformed.fatalError.notBulkAction",
            ...
            if (bulkActionDto.isAsync()) {
                try {
                    ...
                    result.recordStatus(
                            ...
                            createStringResource(
                                    "PageAction.message.startPerformed.inProgress",
                ...
                try {
                    ...
                    result.recordStatus(
                            ...
                            createStringResource(
                                    "PageAction.message.startPerformed.success",
