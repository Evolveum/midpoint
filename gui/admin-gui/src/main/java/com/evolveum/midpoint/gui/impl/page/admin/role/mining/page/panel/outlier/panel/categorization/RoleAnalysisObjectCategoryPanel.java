...
public class RoleAnalysisObjectCategoryPanel extends AbstractObjectMainPanel<RoleAnalysisSessionType, ObjectDetailsModels<RoleAnalysisSessionType>> {
    ...
    //TODO move it to a separate class part of ProgressValueModel
    private @NotNull WebMarkupContainer initDistributionRolePanel(@NotNull RoleAnalysisSessionType session, String panelId, Class<?> typeClass) {
        ...
        int total = unPopular + abovePopular + noiseExclusive + noise + anomalyExclusive + anomaly + outlier + excluded;

        List<ProgressBar> progressBars = new ArrayList<>();
        progressBars.add(new ProgressBar(calculatePercentage(unPopular, total), ProgressBar.State.INFO));
        progressBars.add(new ProgressBar(calculatePercentage(abovePopular, total), ProgressBar.State.SECONDARY));
        progressBars.add(new ProgressBar(calculatePercentage(noiseExclusive, total), ProgressBar.State.PRIMARY));
        progressBars.add(new ProgressBar(calculatePercentage(noise, total), ProgressBar.State.WARNING));
        progressBars.add(new ProgressBar(calculatePercentage(anomalyExclusive, total), ProgressBar.State.LIGHT));
        progressBars.add(new ProgressBar(calculatePercentage(anomaly, total), ProgressBar.State.SUCCESS));
        progressBars.add(new ProgressBar(calculatePercentage(outlier, total), ProgressBar.State.DARK));
        progressBars.add(new ProgressBar(calculatePercentage(excluded, total), ProgressBar.State.DANGER));
        progressBars.sort(Comparator.comparing(ProgressBar::getValue).reversed());

        List<ProgressValueModel> progressValues = new ArrayList<>();
        progressValues.add(new ProgressValueModel(
                unPopular, calculatePercentage(unPopular, total), "Unpopular", "text-info"));
        progressValues.add(new ProgressValueModel(
                abovePopular, calculatePercentage(abovePopular, total), "Above popular", "text-secondary"));
        progressValues.add(new ProgressValueModel(
                noiseExclusive, calculatePercentage(noiseExclusive, total), "Noise exclusive", "text-primary"));
        progressValues.add(new ProgressValueModel(
                noise, calculatePercentage(noise, total), "Noise", "text-warning"));
        progressValues.add(new ProgressValueModel(
                anomalyExclusive, calculatePercentage(
                anomalyExclusive, total), "Anomaly exclusive", "text-light"));
        progressValues.add(new ProgressValueModel(
                anomaly, calculatePercentage(anomaly, total), "Anomaly", "text-success"));
        progressValues.add(new ProgressValueModel(
                outlier, calculatePercentage(outlier, total), "Outlier", "text-dark"));
        progressValues.add(new ProgressValueModel(
                excluded, calculatePercentage(excluded, total), "Excluded", "text-danger"));
            ...
            protected @NotNull Component getPanelComponent(String id) {
                RoleAnalysisDistributionProgressPanel<?> panel = new RoleAnalysisDistributionProgressPanel<>(id) {
                    ...
                    protected @NotNull Component getLegendComponent(String id) {
                        ...
                        for (ProgressValueModel valueModel : progressValues) {
    ...
    static class ProgressValueModel implements Serializable {
        ...
        public ProgressValueModel(int integerValue, double doubleValue, String title, String textClass) {
