...
//		};
	...
	private <V> DropDownChoice createDropDown(String id, IModel defaultModel, final List<V> values,
			IChoiceRenderer renderer) {
		DropDownChoice listSelect = new DropDownChoice(id, defaultModel,
	...
	// BasicSearchPanel<RoleMemberSearchDto> basicSearch = new
	// BasicSearchPanel<RoleMemberSearchDto>(
	...
	// DropDownChoice typeSelect = createDropDown(ID_OBJECT_TYPE,
	// RoleMemberSearchDto.F_TYPE,
	// WebComponentUtil.createFocusTypeList(), new QNameChoiceRenderer());
	// add(typeSelect);
	//
	// DropDownChoice tenant = createDropDown(ID_TENANT,
	// RoleMemberSearchDto.F_TENANT, createTenantList(), new
	// ObjectTypeChoiceRenderer<OrgType>());
	// add(tenant);
	//
	// DropDownChoice project = createDropDown(ID_PROJECT,
	// RoleMemberSearchDto.F_PROJECT,
	...
	// private void clearFilterAccordingToNamePerformed(AjaxRequestTarget
	// target) {
	...
	//
	// FocusBrowserPanel<T> focusBrowser = new
	// FocusBrowserPanel<T>(pageBase.getMainPopupBodyId(),
	// getClassFromType(), WebComponentUtil.createFocusTypeList(), true,
	// pageBase) {
	//
	// @Override
	// protected void addPerformed(AjaxRequestTarget target, QName type, List<T>
	// selected) {
	...
	// };
	//
	// pageBase.showMainPopup(focusBrowser, new Model<String>("Choose member"),
	// target, 900, 500);
	//
	// }
	...
	// NotFilter filter =
	// NotFilter.createNot(RefFilter.createReferenceEqual(FocusType.F_ROLE_MEMBERSHIP_REF,
	// UserType.class, getPrismContext(), roleId));
	// return ObjectQuery.createObjectQuery(AndFilter.createAnd(q.getFilter(),
	// filter));
	// }
	...
	// QName typeName = searchModel.getObject().getType();
	// return
	// getPrismContext().getSchemaRegistry().getCompileTimeClass(typeName);
	// }

	// private TablePanel initRoleTable() {
	// ObjectDataProvider provider = new
	// ObjectDataProvider(RoleMemberPanel.this, RoleType.class) {
	//
	// @Override
	// protected void saveProviderPaging(ObjectQuery query, ObjectPaging paging)
	// {
	// RoleMembersStorage storage =
	// getSession().getSessionStorage().getRoleMembers();
	...
	// ObjectDataProvider provider = new
	// ObjectDataProvider(RoleMemberPanel.this, OrgType.class) {
	//
	// @Override
	// protected void saveProviderPaging(ObjectQuery query, ObjectPaging paging)
	// {
	...
	// ObjectDataProvider<UserListItemDto, UserType> provider = new
	// ObjectDataProvider<UserListItemDto, UserType>(
	...
	private ObjectQuery createQuery(boolean useNameFilter, boolean isNot) {
		...
		try {
			...
			roleRef.setOid(getModelObject().getOid());
	...
	//
	// column = new
	// LinkColumn<SelectableBean<RoleType>>(createStringResource("ObjectType.name"),
	// "name",
	// "value.name") {
	//
	// @Override
	...
	// columns.add(column);
	//
	// column = new PropertyColumn(createStringResource("OrgType.displayName"),
	// "value.displayName");
	// columns.add(column);
	//
	// column = new PropertyColumn(createStringResource("OrgType.identifier"),
	// "value.identifier");
	// columns.add(column);
	//
	// column = new
	// PropertyColumn(createStringResource("ObjectType.description"),
	// "value.description");
	// columns.add(column);
	//
	...
	//
	// column = new
	// LinkColumn<SelectableBean<OrgType>>(createStringResource("ObjectType.name"),
	// "name",
	// "value.name") {
	//
	// @Override
	// public void onClick(AjaxRequestTarget target,
	// IModel<SelectableBean<OrgType>> rowModel) {
	...
	// List<IColumn<UserListItemDto, String>> columns = new
	// ArrayList<IColumn<UserListItemDto, String>>();
	//
	// columns.add(new CheckBoxHeaderColumn());
	// columns.add(new IconColumn<UserListItemDto>(null) {
	//
	// @Override
	// protected IModel<String> createIconModel(final IModel<UserListItemDto>
	// rowModel) {
	...
	//
	// @Override
	// protected IModel<String> createTitleModel(final IModel<UserListItemDto>
	// rowModel) {
	// return new AbstractReadOnlyModel<String>() {
	//
	// @Override
	...
	//
	// IColumn column = new
	// LinkColumn<UserListItemDto>(createStringResource("ObjectType.name"),
	// UserType.F_NAME.getLocalPart(), UserListItemDto.F_NAME) {
	//
	// @Override
	// public void onClick(AjaxRequestTarget target, IModel<UserListItemDto>
	// rowModel) {
	...
	//
	// column = new
	// PropertyColumn(createStringResource("AssignmentType.tenant"), null,
	// UserListItemDto.F_TENANT);
	// columns.add(column);
	//
	// column = new
	// PropertyColumn(createStringResource("AssignmentType.project"), null,
	// UserListItemDto.F_PROJECT);
	// columns.add(column);
	//
	// column = new
	// PropertyColumn(createStringResource("UserType.emailAddress"), null,
	...
	// InlineMenuItem(createStringResource("roleMemberPanel.menu.add"), true,
	...
	//
	// headerMenuItems.add(new
	// InlineMenuItem(createStringResource("roleMemberPanel.menu.remove"), true,
	// new HeaderMenuAction(this) {
	//
	// @Override
	// public void onSubmit(AjaxRequestTarget target, Form<?> form) {
	...
	//
	// headerMenuItems.add(new
	// InlineMenuItem(createStringResource("roleMemberPanel.menu.recompute"),
	// true,
	// new HeaderMenuAction(this) {
	//
	// @Override
	...
	//
	// headerMenuItems.add(new
	// InlineMenuItem(createStringResource("roleMemberPanel.menu.removeAll"),
	// true,
	// new HeaderMenuAction(this) {
	//
	// @Override
	...
	//
	// headerMenuItems.add(new
	// InlineMenuItem(createStringResource("roleMemberPanel.menu.recomputeAll"),
	...
	//
	// PrismContainer<AssignmentType> assignments =
	// obj.findContainer(FocusType.F_ASSIGNMENT);
	// StringBuilder tenantBuilder = new StringBuilder();
	// StringBuilder orgBuilder = new StringBuilder();
	// for (PrismContainerValue<AssignmentType> assignment :
	// assignments.getValues()) {
	// if (assignment != null && assignment.asContainerable() != null) {
	// ObjectReferenceType ref = assignment.asContainerable().getTenantRef();
	// if (ref != null) {
	// tenantBuilder.append(WebComponentUtil.getOrigStringFromPoly(ref.getTargetName()))
	// .append("\n");
	// }
	// ObjectReferenceType orgRef = assignment.asContainerable().getOrgRef();
	// if (orgRef != null) {
	// orgBuilder.append(WebComponentUtil.getOrigStringFromPoly(orgRef.getTargetName()))
	// .append("\n");
	// }
	// }
	//
	...
	// private List<UserListItemDto> getSelectedUsers(AjaxRequestTarget target,
	// UserListItemDto selectedUser) {
	...
	// private void addMembersPerformed(AjaxRequestTarget target, QueryScope
	// scope) {
	// initDialog(target);
	...
	// private void addMembers(QName type, List selected, AjaxRequestTarget
	// target) {
	// // OperationResult parentResult = new OperationResult("Add members");
	// Task operationalTask = getP.createSimpleTask("Add members");
	//
	// try {
	// ObjectDelta delta =
	// ObjectDelta.createModificationAddContainer(UserType.class, "fakeOid",
	// FocusType.F_ASSIGNMENT, getPrismContext(), createAssignmentToModify());
	//
	// TaskType task = WebComponentUtil.createSingleRecurenceTask("Add
	// member(s)", type,
	// getActionQuery(QueryScope.TO_ADD, selected), delta,
	// TaskCategory.EXECUTE_CHANGES,
	// getPageBase());
	// WebModelServiceUtils.runTask(task, operationalTask, parentResult,
	// pageBase);
	// // execute("Add member(s)", getActionQuery(QueryScope.TO_ADD,
	// // selected), delta, parentResult, target);
	// // parentResult.setBackgroundTaskOid(taskOid);
	// } catch (SchemaException e) {
	// parentResult.recordFatalError("Failed to add members " + e.getMessage(),
	// e);
	// LoggingUtils.logException(LOGGER, "Failed to remove members", e);
	// pageBase.showResult(parentResult);
	// }
	// // parentResult.recordInProgress(); // TODO don't do this in case of
	// // error
	// // pageBase.showResult(parentResult);
	// target.add(getFeedbackPanel());
	//
	...
	// private void removeMembersPerformed(AjaxRequestTarget target, QueryScope
	// scope) {
	...
	// try {
	// ObjectDelta delta =
	// ObjectDelta.createModificationDeleteContainer(UserType.class, "fakeOid",
	// FocusType.F_ASSIGNMENT, getPrismContext(), createAssignmentToModify());
	// TaskType task = WebComponentUtil.createSingleRecurenceTask("Remove
	// member(s)",
	// searchModel.getObject().getType(), getActionQuery(scope, null), delta,
	// TaskCategory.EXECUTE_CHANGES, getPageBase());
	// WebModelServiceUtils.runTask(task, operationalTask, parentResult,
	// pageBase);
	// // execute("Remove member(s)", getActionQuery(scope, null), delta,
	// // parentResult, target);
	// // String taskOid = execute("Remove member(s)",
	// // getActionQuery(scope, null), delta, parentResult, target);
	// // parentResult.setBackgroundTaskOid(taskOid);
	// } catch (SchemaException e) {
	...
	// QueryScope scope) {
	// Task operationalTask = pageBase.createSimpleTask("Recompute all
	// members");
	...
	// member(s)",
	// searchModel.getObject().getType(), getActionQuery(scope, null), null,
	...
	// ObjectDelta.createAddDelta(task.asPrismObject());
	// pageBase.getPrismContext().adopt(delta);
	// pageBase.getModelService().executeChanges(WebComponentUtil.createDeltaCollection(delta),
	// null,
	...
	// private void execute(String taskName, ObjectQuery query, ObjectDelta
	// deltaToExecute,
	// OperationResult parentResult, AjaxRequestTarget target) {
	// Task operationalTask = pageBase.createSimpleTask("Execute changes");

	// TaskType task = WebComponentUtil.createSingleRecurenceTask(taskName,
	// searchModel.getObject().getType(), query, deltaToExecute,
	// TaskCategory.EXECUTE_CHANGES, getPageBase());
	// WebModelServiceUtils.runTask(task, operationalTask, parentResult,
	// pageBase);
	// target.add(getFeedbackPanel());
	...
	// private TaskType createTask(String taskName, ObjectQuery query,
	// ObjectDelta delta, String category,
	...
	// ItemPath path = new ItemPath(TaskType.F_EXTENSION,
	// SchemaConstants.MODEL_EXTENSION_OBJECT_QUERY);
	// PrismProperty objectQuery = prismTask.findOrCreateProperty(path);
	// QueryType queryType = QueryJaxbConvertor.createQueryType(query,
	// getPrismContext());
	// objectQuery.addRealValue(queryType);
	//
	// path = new ItemPath(TaskType.F_EXTENSION,
	// SchemaConstants.MODEL_EXTENSION_OBJECT_TYPE);
