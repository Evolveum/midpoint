 ...
 */
public class WizardGuideTilePanel<T extends Tile<O>, O extends Serializable> extends TilePanel<T, O> {

    private static final String ID_BADGE = "badge";
    private static final String ID_LOCK = "lock";

    public WizardGuideTilePanel(String id, IModel<T> model) {
        super(id, model);
    }
    ...
    protected void initLayout() {
        ...
        add(AttributeAppender.append("style", "min-height: 220px;"));
    ...
    private void applyTooltip() {
        IModel<String> tooltipModel = getDescriptionTooltipModel();
        if (tooltipModel != null && tooltipModel.getObject() != null) {
            add(new TooltipBehavior());
            add(AttributeAppender.replace("title", tooltipModel));
            add(AttributeAppender.replace("aria-label", tooltipModel));
        }
    }
