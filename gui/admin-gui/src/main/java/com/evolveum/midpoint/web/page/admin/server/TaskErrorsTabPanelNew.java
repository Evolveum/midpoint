...
import com.evolveum.midpoint.web.page.admin.server.dto.TaskErrorDto2;
 ...
 * Created by honchar.
 */
public class TaskErrorsTabPanelNew extends BasePanel<PrismObjectWrapper<TaskType>> implements TaskTabPanel {
    private static final long serialVersionUID = 1L;

    private static final String ID_TASK_ERRORS = "taskErrors";

    public TaskErrorsTabPanelNew(String id, LoadableModel<PrismObjectWrapper<TaskType>> taskWrapperModel) {
    ...
    private void initLayout() {
        ...
        SelectableBeanObjectDataProvider<? extends ObjectType> provider = new SelectableBeanObjectDataProvider<ObjectType>(this, ObjectType.class, null) {
            ...
            public SelectableBean<ObjectType> createDataObjectWrapper(ObjectType obj) {
                return new TaskErrorDto2<>(obj, getModelObject().getOid());
            }
        ...
        BoxedTablePanel<TaskErrorDto2<ObjectType>> table = new BoxedTablePanel<>(ID_TASK_ERRORS, provider, initColumns());
        table.setOutputMarkupId(true);
        add(table);

    }

    private List<IColumn<TaskErrorDto2<ObjectType>, String>> initColumns() {
        List<IColumn<TaskErrorDto2<ObjectType>, String>> columns = new ArrayList<>();
        columns.add(new PropertyColumn<TaskErrorDto2<ObjectType>, String>(createStringResource("pageTaskEdit.taskErros.objectName"), TaskErrorDto2.F_OBJECT_REF_NAME){
            @Override
            public String getSortProperty() {
                return "name";
            }
        });
        columns.add(new PropertyColumn<>(createStringResource("pageTaskEdit.taskErros.status"), TaskErrorDto2.F_STATUS));
        columns.add(new AbstractColumn<TaskErrorDto2<ObjectType>, String>(createStringResource("pageTaskEdit.taskErros.timestamp"), TaskErrorDto2.F_ERROR_TIMESTAMP){
            private static final long serialVersionUID = 1L;

            @Override
            public void populateItem(Item<ICellPopulator<TaskErrorDto2<ObjectType>>> cellItem, String componentId,
                                     IModel<TaskErrorDto2<ObjectType>> rowModel) {
        ...
        columns.add(new PropertyColumn<>(createStringResource("pageTaskEdit.taskErros.message"), TaskErrorDto2.F_MESSAGE));
        return columns;
    }
