...
/**
 * Created by Honchar Creates a panel with the list of focus type items with the
 * possibility to filter by user (show only assigned to the specified user
 * items), to search through the list and to reset the list to the initial state
 */
public class MultipleAssignmentSelector<F extends FocusType> extends BasePanel<List<AssignmentEditorDto>> {
	...
	public MultipleAssignmentSelector(String id, IModel<List<AssignmentEditorDto>> selectorModel,
			BaseSortableDataProvider provider, Class<F> targetFocusClass, Class type) {
	...
	private void initLayout() {
		...
		AjaxLink<String> filterByUserButton = new AjaxLink<String>(ID_FILTER_BY_USER_BUTTON) {
			...
			public void onClick(AjaxRequestTarget target) {
				...
				// FocusBrowserPanel<UserType> focusBrowser = new
				// FocusBrowserPanel<>(id, type, multiselect, parentPage)
				// if (showDialog) {
				// UserBrowserDialog window = (UserBrowserDialog)
				// MultipleAssignmentSelector.this.get(ID_USER_CHOOSER_DIALOG);
	...
	private Component createRowLink(String id, final IModel<SelectableBean<AssignmentEditorDto>> rowModel) {
		AjaxLink<SelectableBean<AssignmentEditorDto>> button = new AjaxLink<SelectableBean<AssignmentEditorDto>>(
				id, rowModel) {
	...
	private List<IColumn<SelectableBean<AssignmentEditorDto>, String>> initColumns() {
		...
		columns.add(new AbstractColumn<SelectableBean<AssignmentEditorDto>, String>(new Model()) {
			public void populateItem(Item<ICellPopulator<SelectableBean<AssignmentEditorDto>>> cellItem,
					String componentId, IModel<SelectableBean<AssignmentEditorDto>> rowModel) {
	...
	private void toggleRow(IModel<SelectableBean<AssignmentEditorDto>> rowModel) {
		...
		List<AssignmentEditorDto> providerList = ((BaseSortableDataProvider) getProvider())
				.getAvailableData();
		for (AssignmentEditorDto dto : providerList) {
			if (dto.getTargetRef().getOid()
					.equals(((AssignmentEditorDto) rowModel.getObject()).getTargetRef().getOid())) {
