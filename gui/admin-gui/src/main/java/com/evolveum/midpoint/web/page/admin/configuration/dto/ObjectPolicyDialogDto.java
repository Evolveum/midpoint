...
public class ObjectPolicyDialogDto implements Serializable{
    ...
    public ObjectPolicyDialogDto(ObjectPolicyConfigurationType config, PageBase page){
        ...
        if(config.getPropertyConstraint() != null){
            if(config.getPropertyConstraint().isEmpty()){
                config.getPropertyConstraint().add(new PropertyConstraintType());
            }

            for(PropertyConstraintType property: config.getPropertyConstraint()){
                propertyConstraintsList.add(new PropertyConstraintTypeDto(property));
            }
        } else {
