...
public class RealValueOfSingleValuePropertyFromSingleValueContainerWrapperModel<T,C extends Containerable> implements IModel<T> {
    ...
    public RealValueOfSingleValuePropertyFromSingleValueContainerWrapperModel(IModel<ContainerWrapper<C>> model, QName item) {
    	this.model = model;
    	this.item = item;
    }
	...
	public T getObject() {
		...
		if(model.getObject().getItemDefinition().isSingleValue()) {
			ContainerValueWrapper<C> containerValue = model.getObject().getValues().get(0);
		
			PropertyWrapperFromContainerValueWrapperModel<T,C> propertyModel = new PropertyWrapperFromContainerValueWrapperModel<T,C>(containerValue, item);
		...
		throw new IllegalStateException("ContainerWrapper  " + model.getObject() + " isn't single value");
	}
