...
public class PageSecurityQuestions extends PageBase {
	...
	public PageSecurityQuestions() {
		LOGGER.info("PageSecurityQuestions");
	...
	public void initLayout(){
		...
		if(model!=null){
			...
			try {
				config = getPageBase().getModelService().getObject(
				...
				securityPolicy = getModelService().getObject(SecurityPolicyType.class,config.asObjectable().getGlobalSecurityPolicyRef().getOid(), null, task, subResult);
	...
	private PrismObject<UserType> loadUser() {
		...
		PrismObject<UserType> user = WebModelUtils.loadObject(UserType.class,
				getSession().getAttribute(SESSION_ATTRIBUTE_POID).toString(), result, PageSecurityQuestions.this);
	...
	private void savePerformed(AjaxRequestTarget target) {
		...
		for (Iterator iterator = pqPanels.iterator(); iterator.hasNext();) {
			...
			List<SecurityQuestionAnswerDTO> userQuestionList= model.getObject().getSecurityAnswers();
			for (Iterator iterator2 = userQuestionList.iterator(); iterator2
					.hasNext();) {
	...
	private void resetPassword(UserType user,AjaxRequestTarget target){
		...
		try {
			...
			//		mailTransport.send(mailMessage, transportName, task, parentResult);
				...
				| SecurityViolationException | EncryptionException e) {
