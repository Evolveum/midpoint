...
public final class ImageSanitizationUtil {
     ...
     */
    private static byte[] writeImage(BufferedImage image, String outputFormatName)
            throws ImageSanitizationException {
        try {
            byte[] bytes = imageToBytes(() -> image, outputFormatName);
            if (bytes != null) {
                return bytes;
            }

            // try to handle PNG to JPG conversion (transparency must be removed first)
            if (getRawExtension(MIME_IMAGE_JPEG).equals(outputFormatName)) {
                bytes = imageToBytes(() -> handleTransparency(image), outputFormatName);
                if (bytes != null) {
                    return bytes;
                }
            }

            throw new ImageSanitizationException("No " + outputFormatName + " writer available.");
        } catch (IOException e) {
            throw new ImageSanitizationException("Failed to write " + outputFormatName + " image for sanitization.", e);
        }
    }

    private static byte[] imageToBytes(Supplier<BufferedImage> imageSupplier, String outputFormatName) throws IOException {
        try (ByteArrayOutputStream bos = new ByteArrayOutputStream()) {
            if (ImageIO.write(imageSupplier.get(), outputFormatName, bos)) {
    ...
    private static String getRawExtension(String mimeType) {
        String extension = MimeTypeUtil.getExtension(mimeType);
        return extension != null && extension.startsWith(".") ? extension.substring(1) : extension;
    }
}
