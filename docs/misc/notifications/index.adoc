...
For example, a user (or user's boss, or the person who requested the operation, the security manager, etc) may be notified when one of user's accounts is created, modified, or removed.
...
. *Task notifications.* These are created when a task is started or finished.
They are useful e.g. to notify in cases when a recurring task (like a reconciliation or live sync) ends with a failure.
...
Configuration of notifications is currently done within SystemConfiguration object.
...
The description of this example can be found on the next wiki page - wiki:General+notification+-+role+assignment+example[General notification - role assignment example].
...
Or a filter can be a standalone part of a handler chain (see below).
...
| Handler chain (chained)
| Contains sequentially ordered event handlers (notifiers, filters, forks, other chains).
Processing within a chain continues until an event is filtered out.
(Please note that filtering is done only by filters; all notifiers simply pass all events along.)


| Fork (forked)
| Splits processing of an event into more handlers "in parallel".
(We expect this kind of handler will be used only occassionally, however, we have implemented in for completeness.)
...
This can used to reference image attachments from your email HTML.
...
For instructions how to configure notifiers, filters, and transports please see wiki:Configuring+notifications[administrator's guide].
...
These are briefly described in the wiki:Sending+custom+notifications+HOWTO[Sending custom notifications HOWTO].


== See Also

* wiki:Configuring+notifications[Configuring notifications]

* wiki:Using+Velocity+Templates+for+Notifications[Using Velocity Templates for Notifications]