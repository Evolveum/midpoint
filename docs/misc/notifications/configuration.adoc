...
For the time being, here we show a few sample configurations, hopefully self-explanatory enough:
...
<messageTransportConfiguration>
    <mail>
        ...
        <server>
            ...
            <port>587</port>
            <!--
            Specify transport security: none, starttlsEnabled, starttlsRequired
            If undeclared, defaults to (none) for basic auth and (starttlsRequired) for oauth2.
            -->
...
Please note that although the OAuth 2 configuration should work with all smtp providers that have support for client credentials flow, it has been tested only on Microsoft.
...
[TIP]
====
For Microsoft Exchange, we've successfully tested the configuration with port=587 and OAuth2 as described in link:https://learn.microsoft.com/en-us/exchange/client-developer/legacy-protocols/how-to-authenticate-an-imap-pop-smtp-application-by-using-oauth[Microsoft's tutorial].
====
...
| ExpressionType
| 0..1
| Recipients that should get the notifications.
Expression should preferably return a focus object that can be later used by a transport
to extract the addresses and also to determine the language for multi-language message templates.
...
| ExpressionType
| 0..1
| The sender of the notification e-mail.
Overrides the `defaultFrom` option set in the `messageTransportConfiguration` object.
Only for mail transport.
...
See a xref:/midpoint/reference/misc/notifications/#general-notifier-example[general notifier example].
