...
All the existing midPoint function are available (wiki:Script+Expression+Functions[Basic, midpoint, report]).
...
Reports often use wiki:Mappings+and+Expressions[expressions]. Expressions allow to customize midPoint behavior and they are essential for the success of midPoint deployments.
...
wiki:Auditing[Audit records] are *not*  midPoint objects, they are just rows in ordinary relational table.
...
The wiki:Auditing[structure of an audit table is documented] and it can be used for integration with data warehouse and/or SIEM systems.
...
Since version 4.2, midPoint supports 'import report'/'reverse report'. This feature is *wiki:Experimental+Functionality[experimental]*.
