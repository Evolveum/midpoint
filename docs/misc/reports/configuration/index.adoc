...
This feature is available only in midPoint 4.4 and later. For configuration before 4.4 please see xref:../configuration-before-4-4/[Report Configuration before midPoint version 4.4].
...
One of the requirements for new Native report was supporting multithreading and bucketing. Now multitreading is supported for all possible kinds of reported objects such as containers, objects and audit records. Bucketing is supported for objects and audit records, because currently bucketing of task  don't support containers.

Every report output is stored in ‚export‘ directory in MidPoint_home. It is possible to send generated reports to mail using notifier handler.

Every report object contains three basic attributes for  behavior, engine and file format.

Behavior represent direction of report. Midpoint knows two types of behavior. Export is common report like the previous version 4.4, which generated file with reported objects. Import has an opposite  way, file with objects will be on input and Midpoint create object base-on row of file. Import behaviour is now experimental. Default behavior is export.

Engine define object, which will be used for generating of report. We have two choices dashboard and object collection.

And finally, element file format represent format of generated file. Default file format for dashboard engine is HTML and for object collection engine is CSV.
...
Collection reports generate reports based on Object collection configuration. Collection reports support audit records, containers and objects for example users, roles, organizations and so on. Basic part of configuration for Collection reports are query, columns, parameters, subreports and condition. We look at this on the next slides.
...
Now we can take a closer look at the engine Collection object. First important part of collection object report configuration is query for reported objects. Query represent attribute 'collection'.

We have three variants at the disposal. We can use reference to existing object collection. We can write filter directly in collection attribute without base collection or we write filter and use reference to existing collection as base collection for filter.
...
If we use filter with base collection, Midpoint has two filters for report, one from report and one from base collection. Midpoint concat this filters with logical and. On the slide we can see the example: First filter say that we want to see users with membership in Organization Evolveum. Second is for users with role End user. In the result we will see users that concurrently have memberships in organization ‚Evolveum‘ and role 'End user'.

image::collection-query.png[]

==== Columns

Next part of collection report configuration is columns. Definitions for columns contains attribute _view_. View contains definition for type of reported objects and definition of columns. Configuration of view is used on more places in Midpoint for example in adminGui configuration.

Columns for generated report are merged from two views, view defined in report and view defined in object collection used for generated of report. In example on the slide we can see that view in report has configuration for columns name and email and view in object collection has configuration for columns given and family names. The resulting report will have columns for name, email, given and family names.
...
We can skip definition of columns, then midPoint use default view for current type of object. MidPoint contains default view for audit records and for native objects, for example UserType, RoleType, OrgType and so on. But when we want to create report of containers, then we have to define columns configuration.
...
which define resource in filter and we set it before run of report.
...
Type of subreport is collection, but in this case we know that contains only one value so we directly use it.
...
On screenshot we can see example of generated HTML report of all users in midPoint. I chose HTML format because of better graphic view. Report contains columns for Name, Full Name, Administrative status, Roles, Organizations and Accounts of every user.

image::collection-example.png[]

=== Dashboard Report

Dashboard engine generates reports by dashboard object. Dashboard object can be shown in GUI. Dashboard contains widgets, which showed actual state of midpoint. Documentation for dashboard configuration can be found on xref:/midpoint/reference/admin-gui/dashboards/configuration/[Dashboard configuration].

Dashboards support both actual supported file formats, but there is a difference. CSV contains only table of widget with basic message and status, unlike HTML, which as default showing table for widgets and one table for every widget with objects which meet the condition in widget. We can turn off this default behaviour via attribute _showOnlyWidgetTable_ and show only table of widgets.
...
Configuration for dashboard report contains definition for _view_, _view_ attribute in dashboard report is multivalue. We can configure view for every type of objects, which are showed via widgets. View defines columns of table in report.
...
Exports any report in a "classical" way. Supports multithreading but not bucketing (multi-node mode). Support objects, containers and audit reports.
Contains two parameter 'reportRef' and 'reportParam'. 'reportRef' represent reference to report object. 'reportParam' represent parameters for report, which are defined via GUI.

=== ClassicReportImportWorkDefinitionType 'reportImport'

Imports a report in a "classical" way. Supports multithreading but not bucketing (multi-node mode).
