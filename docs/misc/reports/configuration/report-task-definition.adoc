...
A CSV or XLSX file is uploaded into midPoint in a similar format as a report, based on this midPoint will create or update objects.
The format is taken from the uploaded file name (`.xlsx` means XLSX, anything else CSV); the report's `fileFormat` is used when the report data object does not say.
