...
The usage and the structure for the ServiceType object is almost the same as xref:/midpoint/reference/roles-policies/rbac/[midPoint roles] (wiki:RoleType[RoleType]). It means, you can define rules for provisioning, you can assign roles, wiki:Roles,+Metaroles+and+Generic+Synchronization[meta-roles, meta-meta-roles] to them etc.
