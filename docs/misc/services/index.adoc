...
Services in midPoint can be expressed using midPoints wiki:ServiceType[ServiceType] objects.
...
If the standard attributes defined in the schema for ServiceType are not satisfactory, wiki:Custom+Schema+Extension[extension schema] can be used.
...
There exists a lot of connector which can be used with midPoint (wiki:Identity+Connectors[listed here]). After the resource is created for the service, provisioning for users and their access right for the service can start.
