...
The same applies to JVM arguments listed on the About page in the administration GUI.
To avoid this, values are replaced by a placeholder string.
A value is considered sensitive when its key contains a word like `password`, `passwd`, `pwd`, `secret`,
`token`, `credential` or `apiKey` (case-insensitive), or when it is one of the known keys
that may contain credentials (e.g. `midpoint.repository.jdbcUrl` or `midpoint.repository.dataSource`).
