...
The only difference is, that after mail nonce module was evaluated, the authentication sequence continues with `employeeNumberVerification` module (of `attributeVerification` type).
This additional module runs apart from the result of the mail nonce module.
So it doesn't matter is the module was successful or failed, `employeeNumberVerification` module will be evaluated.
If all modules are successful, the change password panel is shown (Figure 5).
An example of `attributeVerification` module (this time, checking the nickname) is shown in the Figure 7.
...
See xref:#_sample_notification_configuration[] for an example.
