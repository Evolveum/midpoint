...
:page-description: This describes how to enable and configure the password reset functionality in midPoint.
:page-keywords: password reset, reset password, forgotten password, credentials reset, self-service
:experimental:
...
[WARNING]
====
The password recovery process needs to be finalized in the same browser session from which it was started.
This means that once a user clicks the btn:[Reset password] button, they must NOT shut down their browser until the whole process is finished.
The link that users are sent in their <<nonce,password recovery email>> will not work in a different browser session.

If a user fails to complete the password reset process in one browser session, and an email nonce that had been issued for them is not invalidated automatically, as midPoint administrator, you can remove the nonce manually:

. Go to the [.nowrap]#icon:user[] btn:[Users]# > [.nowrap]#icon:user[] btn:[All users]# > _user_.
...
. The `credentialsReset` item.
. Authentication modules, their sequence, and credentials used.
. xref:/midpoint/reference/misc/notifications/[Notifications] (if using email or SMS to communicate with users).
. The `#resetPassword` UI authorization.
    Users need to be granted this authorization in order to have the access to the Password reset page.
...
<3> Normally, a xref:/midpoint/reference/misc/notifications/configure-smtp/[mail server configuration] should be present here.
