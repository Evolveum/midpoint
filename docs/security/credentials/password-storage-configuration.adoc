...
We take care to select a good cipher (AES), generate random key for every midPoint deployment, store the key in the wiki:Keystore+Configuration[keystore] and not in the database and so on.
...
Accounts that were created and were not yet initialized are marked in midPoint by using the wiki:Object+Lifecycle[lifecycle state property]. The uninitialized accounts are in the `proposed` lifecycle state.
...
* wiki:Encryption+and+Keys[Encryption and Keys]
