...
The link carries a one-time nonce and leads to a dedicated authentication sequence (see below).
The user authenticates by the nonce from the link and by the current midPoint password.
...
.Authentication sequence for account activation
The activation link is created only when the security policy contains an authentication sequence bound to the
`accountActivation` channel.
The sequence has to contain a mail nonce module (it verifies the token from the link) followed by a login form module
(it verifies the password and provides its cleartext to the activation page).
Without such sequence the notification is sent without the link and the accounts have to be activated by other means,
e.g. by changing the user password.
...
The activation page operates only on the accounts of the authenticated user.
No other authorizations are needed, the channel grants the necessary ones.
