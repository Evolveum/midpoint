...
[NOTE]
Column *Event DB ID* is relevant only for the older xref:/midpoint/reference/repository/generic/[Generic Repository].
The new xref:/midpoint/reference/repository/native-postgrsql/[Native PostgreSQL Repository] uses custom types displaying the name in the first column.
...
see xref:/midpoint/reference/repository/native-postgresql/native-audit/[Native PostgreSQL Audit Trail]
