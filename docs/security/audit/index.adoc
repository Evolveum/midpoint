...
* If the deprecated xref:/midpoint/reference/repository/generic/[Generic SQL Repository] is used,
...
When recording audit events, changed item paths are stored in a way that allows midPoint to index them and use for search.

By default, audit service doesn't store changed item paths for add object deltas, since there's no item path in add delta, just new full object.
For modification midPoint currently stores and indexes only item paths from item delta (modification),
not all available item paths from the changed value (new or old).

This behavior can be changed via system configuration using `systemConfiguration/audit/changedItemPaths`.
...
<systemConfiguration xmlns="http://midpoint.evolveum.com/xml/ns/public/common/common-3">
    ...
    <audit>
        <changedItemPaths>
            <!--
                This option enables storing and indexing item paths for add object deltas.

                With this option enabled, ADD object deltas will be "indexed" using first-level
                items from the new object. E.g. when adding User with name and assignment
                (containing targetRef, etc.) we'll index "name" and "assignment" as changed
                item paths.
            -->
            <indexAddObjectDeltaOperation>true</indexAddObjectChanges>
            <!--
                We can also index additional item paths found in add/modification deltas.
                `all` flag indicates whether to index all items that are children of the given path.
            -->
            <indexAdditionalItemPath>
                <path>extension</path>
                <all>true</all>
            </indexChangedValuesItemPaths>
        </changedItemPaths>
    </audit>
</systemConfiguration>
----
