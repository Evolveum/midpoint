...
* GUI authorization statement for action `http://midpoint.evolveum.com/xml/ns/public/security/authorization-ui-3#dashboard` allows access to the "Dashboard" GUI page.
...
(Optionally, with expressions.). Derefencing (`@`), `referencedBy` and `matches` with `@` subfilter are not supported in authorization filters.
...
Order of one or more means inducement (see xref:/midpoint/reference/roles-policies/policies/metaroles/gensync/['Roles, Metaroles and Generic Synchronization'] page for more details).
