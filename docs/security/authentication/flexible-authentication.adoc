...
:page-nav-title: Flexible Authentication
:page-wiki-name: Flexible Authentication Configuration
:page-since: "4.1"
:page-toc: top


Basic idea of flexible authentication is described on wiki:Flexible+Authentication[Flexible Authentication] page.
...
Midpoint obtains suffix 'auth' from URL, this suffix define using of specific authentication sequence.
...
Flexible authentication is configured in wiki:Security+Policy+Configuration[Security Policy], which is used as global security policy in System Configuration.
...
Only following modules are supported now: `formLogin, saml2, ``httpHeader, httpBasic, httpSecQ, securityQuestionsForm, mailNonce, ldap.` Each element contains common attributes:
...
Possible values are RSA\_SHA1, RSA\_SHA256, RSA\_SHA512 and RSA\_RIPEMD160.
| false
| _enum_
| RSA\_SHA256
...
Supported values are: EMAIL, TRANSIENT, PERSISTENT, UNSPECIFIED and X509\_SUBJECT.
...
[%autowidth]
|===
| Name | Description | Required | Type | Defrault
...
The rest of the functionality is considered to be wiki:Experimental+Functionality[experimental.]
...
REST support for flexible authentication is wiki:Experimental+Functionality[experimental].
...
Please see wiki:Flexible+Authentication+Improvements[Flexible Authentication Improvements] for the details.
