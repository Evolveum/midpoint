...
| xref:/midpoint/reference/security/authentication/flexible-authentication/modules/totp[totp]
| Module is used for MFA authentication of a user by using Time-based One-Time Password (TOTP) algorithm.
...
| /activate/accounts
| http://midpoint.evolveum.com/xml/ns/public/common/channels-3#accountActivation
| Account activation for users with hashed passwords, see xref:/midpoint/reference/security/credentials/password-storage-configuration/[Password Storage Configuration].
