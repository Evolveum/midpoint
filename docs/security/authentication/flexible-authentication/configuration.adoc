...
We can enable/disable updating of focus authentication behavior during every login, or we can use option for updating behaviour only when login failed and during success login after fails login. Default value is 'everyLogin'. Possible values are:
