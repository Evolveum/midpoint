...
MidPoint wiki:Support+Guidelines[support process] is prioritizing issues of the xref:/support/subscription-sponsoring/[subscribers], which means that a non-subscribers usually has to wait a long time for their issue to get fixed.
...
In case that a xref:/support/subscription-sponsoring/[subscriber] needs official release to deploy in their environment, such subscriber should contact Evolveum and a wiki:Development+Process[patch release] will be created in that case.
...
One of the most powerful parts of midPoint are xref:/midpoint/reference/expressions/introduction/[mappings and expressions]. Expressions allow to customize midPoint behavior and they are essential for the success of midPoint deployments.
...
Please consider using xref:/support/subscription-sponsoring/[platform subscription] to  fund wiki:Expression+Profiles:+Full+Implementation[full implementation of expression profiles.]
