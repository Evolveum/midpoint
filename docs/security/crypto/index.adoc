...
First stop the midPoint server.

[NOTE]
.Generating a new key
====
Currently, there is a limitation when setting up the "keypass" (-keypass) parameter. The parameter has to have the value "midpoint" with any new generated key.
The limitation is mentioned in the following work package bug:MID-9595.
====
...
Start the midPoint server again.
