...
MidPoint uses the `keystore.jceks` file as both keystore and truststore because the JCEKS file is just a container;
the difference between keystore and truststore lies in how the application uses it, not in the file format.
...
By default, the JVM uses separate stores for its keystore and truststore (see <<keystore-vs-truststore>>).
However, since midPoint `keystore.jceks` typically serves as both, the simplest option is to override the default JVM keystore and truststore locations and point them both at the midPoint keystore file.
You can do it with JVM options like this:

[source]
----
-Djavax.net.ssl.trustStore=/var/opt/midpoint/keystore.jceks
-Djavax.net.ssl.trustStoreType=jceks
-Djavax.net.ssl.keyStore=/var/opt/midpoint/keystore.jceks
