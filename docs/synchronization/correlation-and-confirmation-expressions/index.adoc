...
Correlation expression is in fact just a (parametric) search query, i.e. correlation expression is just a wiki:XML+Object+Query[search filter] with an xref:/midpoint/reference/expressions/expressions/[expression] inside.
