...
However, even this wiki:Initial+Logging+Setup+HOWTO[early initialization logging can be reconfigured]. To route even the initialization logging to the syslog server create a logback.xml file in wiki:MidPoint+Home+Directory[MidPoint Home Directory]. This file is supposed to be in the native LogBack format.
...
* wiki:Initial+Logging+Setup+HOWTO[Initial Logging Setup HOWTO]
