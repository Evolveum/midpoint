...
Verification is a process that reads objects from midPoint repository and checks whether they are compatible with the new version of midPoint.
...
Different categories for verification can be set via switch `--verification-category`.
Allowed values are:
...
Refer to xref:/midpoint/reference/deployment/ninja#how-multithreading-works[How multithreading works] for more information.
...
* Upgrade information (multiple columns see <<Upgrade information>> below)
