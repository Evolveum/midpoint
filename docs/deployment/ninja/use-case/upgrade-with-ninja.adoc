...
Running the command manually is almost equivalent to `upgrade-distribution` command described bellow, but split into separate commands.
...
For more information refer to: xref:../command/pre-upgrade-check.adoc[] command documentation.
