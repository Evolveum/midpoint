...
Since midPoint 4.11 the built-in manual connector can optionally manage the tickets on a real ITSM system, using a dedicated xref:/midpoint/reference/resources/manual/ticketing/[ticketing resource].
See the xref:/midpoint/reference/resources/manual/ticketing/[ITSM Ticketing] page for details and the ticketing resource setup.

The manual connector is configured by the following configuration items (in the `connectorConfiguration` of the resource):
...
If not set, the cases are managed internally by midPoint.

| `ticketingObjectKind`
| `generic`
| Kind of the ticket object type on the ticketing resource (`work` in the standard setup).
...
When `ticketingResourceOid` is set, the manual connector creates the provisioning cases with a ticket projection on the ITSM system.
The ticket state is tracked by reading it back from the ITSM system (via the xref:/midpoint/reference/tasks/shadow-refresh-task/[shadow refresh task] and the inbound mappings on the ticketing resource).

== ITSM Plugin

Both manual and semi-manual resources are often used with ITSM integration.
See the xref:/midpoint/reference/resources/manual/ticketing/[ITSM Ticketing] page for the built-in integration (no custom development needed) and the xref:/midpoint/reference/resources/manual/itsm-plugin/[ITSM Plugin] page for the custom plugin approach.
