...
Especially when entitlements are combined with the power of xref:/midpoint/reference/roles-policies/roles/assignment/[assignments] and xref:/midpoint/reference/synchronization/generic-synchronization/[generic synchronization].
====
...
| Whether you require midPoint to provide referential integrity for this reference type.

Used for resources that do not provide referential integrity by themselves
(e.g., OpenDJ with the referential integrity plugin turned off).
For resources that support referential integrity (e.g., Active Directory), set this parameter to `false`.
...
The following example shows how to define `groupMembership` reference type that binds together accounts (as subjects) and groups (as objects).
