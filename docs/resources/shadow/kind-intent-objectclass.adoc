...
Accounts can be _xref:/midpoint/reference/roles-policies/roles/assignment/assigning-vs-linking/[linked]_ to the xref:/midpoint/architecture/archive/data-model/midpoint-common-schema/usertype/[User] objects.
...
* *Work* resource objects represent work items, such as issues, tickets, cases or work packages.
Work objects are typically managed by an ITSM system.
midPoint uses the `work` kind e.g. for the xref:/midpoint/reference/resources/manual/ticketing/[ITSM ticketing] integration, where the ticket of a midPoint case is managed on a remote ITSM system as a work object.

* *Association* resource objects are used to represent associations between resource objects.

* *Generic* resource objects are the "catch-all" kind for resource objects that do not fit any of the other kinds.

* *Unknown* is a fallback kind for resource objects whose kind cannot be determined.
...
| ticket
| ri:ticket
| A ticket on an ITSM system (e.g. Jira or ServiceNow), managed through a xref:/midpoint/reference/resources/manual/ticketing/[ticketing resource].
The ticket is a projection of a midPoint case.
