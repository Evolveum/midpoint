...
MidPoint is using xref:/midpoint/reference/roles-policies/roles/assignment/assigning-vs-linking/[links] and xref:/midpoint/reference/correlation/[correlation] to determine the situation.
...
E.g., if this account was deleted we may want to leave it deleted and synchronize the repository state, we may want to re-create the account, we may want to disable or even delete the user that was linked to that account or we may want to do a bunch of other things.
