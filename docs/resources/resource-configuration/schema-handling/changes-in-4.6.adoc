...
The most prominent situations are searches done as part of import or reconciliation activities.
Also, `projectionDiscriminator` in `associationFromLink` is considered as a search as well.
...
However, this functionality can be achieved - perhaps more clearly - also using `classificationOrder` property of object type delineation.
...
Overall, it is better to avoid `defaultForObjectClass` setting.
