...
Type of every xref:/midpoint/reference/resources/shadow/[resource objects] that midPoint deals is defined by its wiki:Kind,+Intent+and+ObjectClass[object class]. The object class defines the basic structure of the object: whether it is account, group or organizational unit.
