...
Limitations for the generic repository implementation (Oracle, SQL Server):
...
Please turn on "use cached" setting with care.
...
That way or another, please make sure the system behaves as expected with caching enabled.
...
If set to `none`, then the default behavior is to have caching turned off.
If not set, or set to `standard`, the default is to have caching turned on.
