...
:page-toc: top
:page-keywords: shadow, delta, resource, attribute, provision, modify attribute directly, fire-and-forget, fire and forget
:page-description: Construct a delta for an attribute in a Groovy script and change the attribute value directly on a resource by updating the resource object shadow
...
. Write the Groovy code to handle the specified attribute value update:
    ...
    ** Compare the existing value of the attribute with the new value and create a xref:/midpoint/projects/midprivacy/phases/01-data-provenance-prototype/processing/model/delta/[delta].
...
In the example task below, we query all objects of the selected class on the specified resource.
...
<4> Use the link:#groovy-update-attribute-directly[Groovy code from above].
