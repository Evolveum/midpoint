...
MidPoint 3.6 has a basic support for wiki:Manual+Resource+and+ITSM+Integration[manual and semi-manual resources]. In these cases the operation is executed manual by a human operator.
...
Let's have a big LDAP server with "NIS" schema (link:https://www.ietf.org/rfc/rfc2307.txt[RFC2307]). MidPoint is an wiki:Unix+Story+Test[excellent choice] to manage such a system.
