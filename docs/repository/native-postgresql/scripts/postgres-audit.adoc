...
Stores audit payloads attached to audit events.
...
| `BIGINT`
| primary key, required
| Identifier of the audit event this payload belongs to.

| `timestamp`
| `TIMESTAMPTZ`
| primary key, required
| Timestamp of the audit event this payload belongs to. Also used as partitioning key.

| `ordinal`
| `INTEGER`
| primary key, required
| Order of this payload within the owning audit event.
...
| `TEXT`
| 
| Media type of the payload content.

| `content`
| `JSONB`
| 
| Payload content stored as JSON.

| `searchableText`
| `TEXT`
| 
| Text representation used for later full-text search.
...
| Speeds up later full-text-like audit payload searches. Used for: full-text-like audit payload searches
...
Default partition for audit payloads that do not match a monthly audit payload partition.

No primary key is defined for this table.

This table is a partition of xref:#ma_audit_payload[`ma_audit_payload`]. Columns are inherited from the partitioned table.
...
| `GET_OBJECT`, `ADD_OBJECT`, `MODIFY_OBJECT`, `DELETE_OBJECT`, `EXECUTE_CHANGES_RAW`, `SYNCHRONIZATION`, `CREATE_SESSION`, `TERMINATE_SESSION`, `WORK_ITEM`, `WORKFLOW_PROCESS_INSTANCE`, `RECONCILIATION`, `SUSPEND_TASK`, `RESUME_TASK`, `RUN_TASK_IMMEDIATELY`, `DISCOVER_OBJECT`, `INFORMATION_DISCLOSURE`, `SMART_SERVICE_CALL`
...
Creates monthly audit table partitions for audit events, payloads, deltas, and references.
