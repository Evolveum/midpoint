...
Add the new value `MESSAGE_TEMPLATE` to the `ObjectType` enum in `postgres-new.sql`.
This value will be later added in the `MObjectType` Java enum, but that requires some other classes we don't have yet.

. Also, prepare and test upgrade command to the same effect before the bottom comment in `postgres-new-upgrade.sql`.
We will wrap this inside `apply_change` so it is executed only if not applied yet.
Still, we will write it in an idempotent fashion (`IF NOTE EXISTS`), because we can and it is always more flexible:
+
[source,sql]
----
