...
The process can be done in a single export/import for some audit sizes, or iteratively for lager audits.
...
Check xref:/midpoint/reference/repository/native-audit/#audit-migration-from-other-database[Audit migration from other databases] section from xref:xref:/midpoint/reference/repository/native-audit/[Native PostgreSQL Audit Trail] document for more details.
