...
:page-toc: top
:page-description: This article describes the history, design, and implementation of the midPoint repository.
:page-keywords: repository, database, postgresql, native repository, generic repository, relational database
...
Finally, the generic database support was removed from midPoint in 4.10.

== The Future Of Database Engine Support

Few important facts for the future of database engine support in midPoint:

* *PostgreSQL is required* for all new midPoint deployments.
...
Demo, proof of concept or evaluation deployments should use docker compose with native PostgreSQL repository.
For more information see xref:/midpoint/get-started/install/containers/docker/[Docker Compose].

* Support for other commercial database engines (Oracle, Microsoft SQL) using generic repository implementation was dropped in midPoint 4.10.
...
Using xref:native-postgresql/[native PostgreSQL repository implementation] is currently the only option.

== Existing Deployments: Migration to Native PostgreSQL Repository

// todo rewrite migration part
