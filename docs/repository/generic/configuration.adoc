---
midpoint-feature: identity-repository-generic
doc-type: config
---
...
The configuration file *config.xml* will be generated if it does not exist and it will be populated with placeholders to use generic repository implementation.
If there's are no values populated in `config.xml` or via JVM `-D` options, repository initialization will fail.
...
Possible values are *oracle*, *sqlserver*, *postgresql*.
This option will set defaults for other options, for example *hibernateHbm2ddl, hibernateDialect* and *driverClassName* based on the selected database.
...
It is enabled by setting `hibernateHbm2ddl` parameter to `none`, which is now the default.
...
* `false` (i.e. "do not skip") otherwise (e.g. if it is `none` which is the default)
...
For production environments it is strongly recommended setting it to `validate`.
