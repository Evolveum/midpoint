...
:page-toc: top
:toclevels: 1
:page-description: This guide explains how to set up a scheduled task to run a certification campaign automatically at scheduled times.
...
Scheduled certification campaigns are meaningful if you, for example, want to run access certifications quarterly.
...
== Define certification campaign

Firstly, define an access certification campaign.
Refer to xref:/midpoint/reference/certification/certification-campaign-configuration/[] regarding the campaign configuration options.
See xref:/midpoint/reference/certification/tutorial/[] if you need help getting started with campaign definition.

For testing purposes, we suggest starting with something small; just one stage affecting low number of people willing to help you debug the setup.

== Set up task launching the campaign

Once your campaign is ready, define a task that will launch it at defined times.
Set the schedule to short intervals (minutes, hours, then days) to get meaningful results faster, or create an instance and run it manually.
...
=== Get started with examples from samples repository
...
== Manage and review scheduled campaigns in GUI

You can check the current status of a campaign in midPoint graphical user interface (GUI) under [.nowrap]#icon:certificate[] *Certification*# > [.nowrap]#icon:circle[] *Campaign scheduling*#.
