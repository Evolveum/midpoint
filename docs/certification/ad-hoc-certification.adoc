...
This guide shows you how to create ad-hoc certification campaigns.
This can be useful in specific cases for which regular scheduled certification campaigns are not a good fit, such as when a new user is added to an organization and its manager should review and certify all of the user's assignments (roles, etc.).
The regular campaigns are not the best fit for this scenario because this _single-person certification_ needs to be done right away and cannot wait several months until the next scheduled "full" certification campaign.
