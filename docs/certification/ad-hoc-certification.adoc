...
:page-description: This guide instructs on how to create ad-hoc certifications in midPoint.
:page-keywords: ad-hoc certification, policy rule, certification campaign
:page-toc: top

This guide shows how to create ad-hoc certification campaigns.

Ad-hoc certification campaigns are useful for situations that need to be taken care of right away, rather than waiting for the next scheduled regular certification campaign.
For example, when an employee changes a department, they are unassigned from one organization unit in midPoint and assigned to another.
The organization units may contain policies giving users accesses appropriate for the particular unit.
These accesses are granted and revoked automatically when users move across the units, no further action needed.

However, any manually assigned roles the user may have requested remain untouched.
Those may have been relevant for the employee's previous job assignment, but are they still relevant after the move?
The ad-hoc certification campaign is triggered to have that reviewed and certified by a human reviewer.
...
.Certification template vs. instance
[NOTE]
====
An ad-hoc certification campaign template may be defined for multiple objects and targets, same as the standard campaign template can, but every instance of an ad-hoc certification campaign is always single-object and single-target, unlike the standard campaigns.
====
