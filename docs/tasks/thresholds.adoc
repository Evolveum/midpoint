...
[NOTE]
====
Since midPoint 4.11, policy rules with thresholds can also be defined directly on individual task activities,
without assigning a policy-bearing role to the whole task.
See xref:./focus-policies-in-activities[Focus policies in activities] and xref:./activity-policies[Activity policies].
====
...
In general, the policy rules are enforced after the changes for an object are computed, but before they are executed, so the object that trips the threshold is not processed.
