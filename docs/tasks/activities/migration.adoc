...
after the upgraded system is started for the first time.
 - Download (export) all tasks you want to preserve - at least ones that contain `workManagement`
item (i.e. clustered and partitioned tasks). This item will be inaccessible after the upgrade.
Download or export can be done either via midPoint Studio or directly via midPoint GUI.
...
5. Migrate the tasks.
+
Tasks can be migrated using `Other actions` -> `Upgrade task to activity (4.4)` action. You can migrate individual files,
