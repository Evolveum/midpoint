...
(For 4.6 and above, please see xref:/midpoint/reference/resources/resource-configuration/schema-handling/changes-in-4.6.adoc#_defaults_for_kind_and_intent[].)
. If `kind` is not specified but `objectclass` and `intent` are, then the behavior is undefined.
We suggest avoiding this case.
. If only `objectclass` is specified (i.e. both `kind` and `intent` are not present), then it is used.footnote:[There are some issues with this setting present before midPoint 4.6, see bug:MID-7470[].]
