...
. `cronLikePattern`: Quartz cron-like pattern specifying time(s) when the task is to be run.
...
// There is also latestFinishTime but it is not implemented and used;
// it was a proposal which was never done.
// Something similar was implemented in 4.10 as part of (sponsored) task activity policies, where you can specify maximum execution time for given activity.
// But it's specified as a duration, not as specific (absolute) moment in time.
// See Pavol's comment in https://support.evolveum.com/wp/10767 for details.
...
* Quartz cron-like scheduling provides the ability to specify starting times using an expression.
    ** For example, to schedule a task to run every Saturday at 03:45, you would use the following pattern: `0 45 03 ? * 7` (second: 0, minute: 45, hour: 3, day: any, month: any, week day: 7 (Saturday)).
...
// `forget` option removed as per info in https://support.evolveum.com/wp/10774
