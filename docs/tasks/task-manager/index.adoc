...
[[threadstopaction]]
== What Happens to Tasks When Their Node Goes Down

This section covers two task types and their behavior when the node they run on shuts down before they finish, as well as your options to control the action they take.
...
=== What Can Happen with Tasks When They Are Interrupted

The task behavior is controlled by *`threadStopAction` attribute*, which can have the following values:
...
If you set task as non-resilient using `threadStopAction`, it will suspend or close when its node shuts down.
...
Making tasks non-resilient overrides this behavior, which can lead to task termination or suspension instead of automatic recovery.
...
you should avoid setting unless you have a strong reason for it (e.g., a manual synchronization task you want to inspect after an interruption).
...
//TODO: Is it even possible to use in-memory job store still? Tracked in https://support.evolveum.com/wp/10775 @dakle 2025-07-23
