...
When using such a query to drive e.g. focus recomputation or arbitrary action execution, it may happen that some objects are processed more than once.
...
And now let us consider the following action.
...
Therefore, if such a query is used in action task or recomputation task or similar one, each user having all of the mentioned 5 role assignments is processed 2101 times (!).
...
The first one consists of additional filtering of search results in action interpreter or iterative search task handler.
...
Here we show the use within actions and recomputation tasks.


=== Distinct option in actions

.Action with 'distinct' query option
