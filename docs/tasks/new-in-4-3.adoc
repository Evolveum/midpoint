...
Tasks are one of key areas of the link:/midpoint/projects/midscale/[midScale project] aimed
...
Formally, these features are marked as experimental in 4.3. They should work, but details will
be most probably fine-tuned based on experiences and user feedback.
====
...
This aspect is rather complex, so it is covered in a link:task-error-handling.adoc[separate document].
...
but its basic features can be seen in the link:/midpoint/projects/midscale/design/tasks/[preliminary design document].
