...
=== Reconcile all default accounts
...
`intent`, and _appends_ the given query (containing only `paging` element here) to it.
The resulting query is then used
...
=== Import all accounts starting with "b"
...
=== Reload and import unlinked shadows

This example reloads and imports unlinked shadows.
See a https://github.com/Evolveum/midpoint-samples/blob/master/samples/tasks/bulk-actions/reload_import-new.xml[separate example page].
