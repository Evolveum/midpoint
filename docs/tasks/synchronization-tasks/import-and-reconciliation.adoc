...
The operations are executed even if their retry time has not come yet.footnote:[Technically, the operation
invoked on the shadows found is `provisioning.refreshShadow` with the option of `forceRetry`.]
...
* internal changes (deltas) used are different: "add" for import, no delta for reconciliation.footnote:[The real effects
...
1. `resourceRef` and `objectclass` match the values derived from the `resourceObjects` part of the work definition,footnote:[Technically,
