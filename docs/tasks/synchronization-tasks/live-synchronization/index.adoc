...
* *Synchronization policy must exist* - Without `synchronization` reactions (such as `linkSource`, `addFocus`, and `deleteShadow`) defined on the resource, live sync detects changes but does nothing with them.
