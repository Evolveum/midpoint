...
Inline activity policies may be enabled or disabled via the user interface in task details.
...
Policies contributed via `policyRef` or virtual assignments are not affected by this action and remain enabled.

The task details UI also allows users to clear activity policy triggers and counters, while keeping the policies and activity state untouched.
...
To resume, first disable the policy or clear its state via the <<User interface>>.
