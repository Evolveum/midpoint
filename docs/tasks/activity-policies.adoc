...
* Current architecture and ConnId Connector Framework API doesn't allow handling a number of situations (exceptions) via activity policies.
+
Such exceptions are reported from ConnId as usual and can be seen in midPoint (GUI, task errors).
+
** Exceptions thrown during search operation when executing the search query or creating connector object.
*** Error handling for these situations happens outside of midPoint control, directly in ConnId framework and search operation is interrupted.
