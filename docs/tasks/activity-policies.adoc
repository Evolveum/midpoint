...
Activity-based constraints (such as `executionTime`) and focus-based constraints (see xref:./focus-policies-in-activities[Focus policies in activities]) cannot be combined within a single constraint.
They are evaluated in different places, so each policy must use only one of the two kinds.
