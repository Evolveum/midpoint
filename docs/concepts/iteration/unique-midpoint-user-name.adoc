...
It increments on every iteration.
The default starting value is `0` when `maxIterations` is configured and `1` otherwise (unless `start` is explicitly set).
This ensures backward compatibility for deployments that previously had `maxIterations` defined (and may now define `end`), so that new records continue to be iterated starting from `0` consistently with existing records.
When configuring iteration without `maxIterations`, the numbering starts naturally from `1`.
Variable `attempt` is an alias for `iteration`.

* Variable `iterationToken`: String variable that contains the portion of the identifier that is changed in each iteration.
It can be derived from the iteration number using a token expression.
If no token expression is specified, the default expression is `iteration`.
By default (with `useTokenOnlyOnConflict` set to `true`), the token expression is not evaluated for the first (non-conflicting) attempt.
...
[NOTE]
====
The default token expression is `iteration`.
With the default `useTokenOnlyOnConflict` value of `true`, the token expression is not evaluated for the first (non-conflicting) attempt, therefore `iterationToken` is empty on that attempt.
On subsequent conflict attempts the token expression is evaluated and the result is the string value of the `iteration` (or `attempt`) variable.

The default `start` value is `0` when `maxIterations` is configured and `1` otherwise.
The default of `0` when `maxIterations` is present preserves backward compatibility for existing deployments where users had `maxIterations` defined (and now might define `end`), ensuring that new records continue to be iterated from `0`.
====
...
. . .
    <iterationSpecification>
        ...
        <useTokenOnlyOnConflict>true</useTokenOnlyOnConflict>
        <tokenExpression>
            <script>
                <code>
