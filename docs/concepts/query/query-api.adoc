...
(like assignment in the example above), `NOT EXIST` behavior works properly only with the new Native repository.
...
[IMPORTANT]
Works only since midPoint 4.6.

Since version 4.6, `REF` filter can optionally contain a nested *target filter* which is applied to the target of the reference.
...
// TODO how to create filters, move to
