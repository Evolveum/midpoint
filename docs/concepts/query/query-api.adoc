...
These structures are called *prism objects*.
They are stored in various places, namely:
...
`EXISTS` for "old repo" was implemented using `LEFT JOIN` which works fine for positive conditions,
...
Good news is that with the new (Sqale) repository coming with midPoint 4.4, `EXISTS` is translated
...
** This works for single-value paths, polystrings (albeit strict matching using two serparate
``IN``s is not totally correct) - see
...
** TODO: refs?
** This works multi-vs-multi for cases like `subtype` or `policySituations`, see
...
** This works for multi-value attribute (LHS) with single value in the filter as usual, that is
if any of the attribute values are equal.
