...
xref:/midpoint/reference/concepts/query/axiom-query-language[Axiom query language] or XML.
...
* Filters can be also written in xref:/midpoint/reference/concepts/query/axiom-query-language[Axiom query language]
...
[IMPORTANT]
Axiom query language is experimentally available since 4.3 and fully supported since 4.5.
...
// TODO Axiom syntax described? Or link to axiom-query-language page?
...
** All the `ref` filters must use "self" path represented by `.` in the Axiom query language.
...
There is no Axiom example, because the ordering is not part of the Axiom query language, only filter is.
