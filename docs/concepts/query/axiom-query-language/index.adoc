...
you could click on down arrow in search fields to select `Advanced` (or `Axiom query`) as one of the search options.

==== midPoint 4.5, 4.6, 4.7

You could click on down arrow in search fields to select `Axiom query` as one of the search options.

==== midPoint 4.4

Query language is experimental feature in midPoint 4.4, so it needs to be explicitly enabled in GUI.
