...
[IMPORTANT]
====
xref:/midpoint/reference/repository/native-postgresql/[Native repository] always uses the SQL `EXISTS` clause both in the Exists filter, and when traversing to multi-value containers.
