...
You can use this method for both inbound and outbound reconciliation.
However, note that marked objects stay in quarantine until you remove the mark.
This means that if an object is marked during reconciliation, it will not be processed in any subsequent reconciliation (inbound or outbound) until the mark is removed.
