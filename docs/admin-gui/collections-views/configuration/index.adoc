...
To display all types at once as a default option, please, use the parent object type class which is suitable for the concrete panel (e.g. use AbstractRoleType for such panels as assignments or member; or use ObjectType on the panels where all midPoint object type can be present).
