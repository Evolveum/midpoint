...
Use collections to gather audit log events in order to, for example, display them in a xref:/midpoint/reference/admin-gui/dashboards/configuration/#audit-log-events-as-widget-data-source[dashboard].
...
To display all types at once as a default option, please, use the parent object type class which is suitable for the concrete panel (e.g., use AssignmentHolderType for members panel of the org. unit; or FocusType for members panel of the role or service; or use ObjectType on the panels where all midPoint object type can be present).
...
Search item is presented with a SearchItemType type, there is a possibility to configure search item path, filter, description, display order and display name (pls, see the following example for more info)
|
...
| Configuration for the Type search item on the Search panel.
Contains 2 attributes:
1. _defaultValue_ is used to set the default type value for the Type search item.
E.g., UserType or RoleType.
To display all types at once as a default option, please, use the parent object type class which is suitable for the concrete panel (e.g., use AssignmentHolderType for members panel of the org. unit; or FocusType for members panel of the role or service; or use ObjectType on the panels where all midPoint object type can be present).
2. multivalue _supportedTypes_ attribute defines a list of available object types for the list/view search.
...
`ScopeSearchItemConfigurationType`, `ObjectTypeSearchItemConfigurationType`, `RelationSearchItemConfigurationType` and `IndirectSearchItemConfigurationType` are extension of `UserInterfaceFeatureType`, so we can configure _visibility_, _displayOrder_, _display/label_ and _display/help_. Also, these types contain _defaultValue_ element and `ObjectTypeSearchItemConfigurationType` and `RelationSearchItemConfigurationType` contains element for supported values.

By default, search box items are displayed in alphabetical order. If no _searchBoxConfiguration_ is specified for an _objectCollectionView_ (e.g., default collections), only the default search items of that type are displayed by default (e.g., the default search items for UserType are Full name and Name). New search items can be added using the More button.

If a custom _searchBoxConfiguration_ is specified for an _objectCollectionView_, the search box only contains search items from that _searchBoxConfiguration_. The display order of search items is the same as their order in the _searchBoxConfiguration_ by default. You can specify a custom display order by configuring the _displayOrder_ property of the search item. New search items added using the More button are added after the custom search items and displayed in alphabetical order.

The example of search panel configuration for custom System Users page:

[source,xml]
----
<objectCollectionView>
    <identifier>system-users-view</identifier>
    <display>
    ...
    <type>UserType</type>
    <collection>
        <collectionRef oid="00000000-0000-0000-0000-000000000300" relation="org:default" type="c:ArchetypeType">
            <!-- System user -->
        </collectionRef>
    </collection>
</objectCollectionView>
...
image::SysUsers.png[100%]

The example of search panel configuration for custom System Users page where we specified _displayOrder_ with equivalent output as above:
...
You can also see in the image the new search items added using the More button. They are added after the custom search items and displayed in alphabetical order.

image::SysUsersAdded.png[100%]
...
image::AllUsersTelEmail.png[100%]
...
Use case for such search configuration is described more detailed xref:../../admin-gui-config/admin-gui-config-examples.adoc/#configure-fulltext-search-item-on-the-members-basic-search-panel[here].
