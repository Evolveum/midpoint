...
Collections and views are two concepts that are related, yet distinct:
...
The plan is that collections can be used in the entire midPoint system, e.g., in xref:/midpoint/reference/security/authorization/[authorizations], xref:/midpoint/reference/roles-policies/policies/policy-rules/[policy rules] may be applied to them, and so on.
...
Views can be adjusted and overridden in xref:/midpoint/reference/admin-gui/admin-gui-config/[admin GUI configuration].
...
<objectCollection
    ...
    <description>Counts users modified directly in GUI and in raw edit mode</description>
...
----
<1> Query filtering events by type; in this case, object modification using the GUI—both the form and raw XML editor.
