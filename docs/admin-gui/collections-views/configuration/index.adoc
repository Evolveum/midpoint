...
If no _searchBoxConfiguration_ is specified for an _objectCollectionView_ (e.g., default collections), only the default search items of that type are displayed by default (e.g., the default search items for UserType are Full name and Name). New search items can be added using the More button.

If a custom _searchBoxConfiguration_ is specified for an _objectCollectionView_, the search box only contains search items from that _searchBoxConfiguration_. The display order of search items is the same as their order in the _searchBoxConfiguration_ by default. You can specify a custom display order by configuring the _displayOrder_ property of the search item. New search items added using the More button are added after the custom search items on the end of the list in the order as they was added.
...
You can also see in the image the new search items added using the More button. They are added after the custom search items on the end of the list in the order as they was added.
