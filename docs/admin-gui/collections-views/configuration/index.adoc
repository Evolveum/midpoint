...
Default settings for all list views can be defined in the `objectCollectionViews` element.
For example, paging configuration can be set up for all list views in the system.
You can learn more about default list view settings in the xref:/midpoint/reference/admin-gui/admin-gui-config/index.adoc#_default_gui_object_list_panel_configuration[Default GUI object list panel configuration] chapter.
Also, you can see some examples by following the xref:/midpoint/reference/admin-gui/admin-gui-config/admin-gui-config-examples/#default-settings-for-object-collection-views[link].
