...
Solution to this in the xref:/midpoint/reference/security/authorization/admin-gui/[midPoint GUI are authorizations] combined with xref:/midpoint/reference/admin-gui/collections-views/[object collection and views].
...
This you can do using xref:/midpoint/reference/security/authorization/[authorizations]—do not let users of the _Employee_ archetype view the all users list but let them view the object collections.
