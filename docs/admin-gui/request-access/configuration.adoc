...
|
|Specification of an object collection that will be used for data presentation in the role catalog
(for all available collection views including Roles of teammate and Role catalog organization tree).
Role catalog has several sources of the object collection configuration (e.g.
`collectionRef` or `viewConfiguration`), it's possible to
prioritize the configuration of the current collection (taken from Object collection object
referenced in `collectionRef`) by setting `preferCollectionView` to true. If it is not specified,
the default false value is used, which means that `viewConfiguration` configuration will
be used if it is defined.
|===

.Role collection view type description
[options="header", cols=4]
|===
|Element
|Type
