...
* Roles listed in the role catalog are also filtered by the authorization mechanism, and to compute authorizations, we only take the first user from the selected person of interest (first step) into account.
