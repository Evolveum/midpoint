...
*Subject* tile allows <<Subject,detailed association subject configuration>>.
...
* *Strength* allows the association xref:/midpoint/reference/expressions/mappings/#_mapping_strength[] to be set
...
In this step we create a new mapping using btn:[Add inbound] that defines the transformation of association data from resource to midPoint data (inbound):
...
If associations correspond to assignments, you typically want to use `targetRef` property (of the assignment) as correlator.
...
In this step we create a new mapping using btn:[Add outbound] that defines the transformation of midPoint data to association data (outbound).
