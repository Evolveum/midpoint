...
As a general rule, define source and target in credential mappings
...
* For [.nowrap]#icon:arrow-right-to-bracket[] *Inbound*# mappings:
    ** *Source* specifies the attribute on the resource side from which to take the credentials.
        This is typically defined in the xref:/midpoint/reference/resources/resource-configuration/[resource schema] and needless to specify manually here.
        However, if your resource objects (e.g., users) have multiple passwords, such as a password _and_ and a PIN,
        you would specify the PIN as an additional credentials source and map it to its dedicated target in midPoint.
        The value to use in case of a CSV resource could be, for instance, `$shadow/attributes/ri:someColumnName`.
    ** *Target* specifies the attribute of the resource object type in midPoint to which you wish to put the credentials, e.g. `credentials/password`.
        The use case for this option is when you have a special object type attribute for the password or other credential type.
...
* For [.nowrap]#icon:arrow-right-from-bracket[] *Outbound*# mappings:
    ...
    ** If you cannot rely on the resource schema definition for password target mapping because your users have two passwords, for instance,
        you need to map the secondary password to the appropriate resource attribute using the general outbound xref:/midpoint/reference/admin-gui/resource-wizard/object-type/mapping/[mapping rules].
        In other words, you need to handle secondary passwords the same as other attributes, such as _surname_.
