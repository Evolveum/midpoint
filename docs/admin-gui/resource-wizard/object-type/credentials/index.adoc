...
Set up mappings for credentials, such as passwords, in the graphical user interface with the resource wizard for credential mappings.

Using the credential mapping configuration, you can manage various types of credentials, such as passwords, link:https://en.wikipedia.org/wiki/Secure_Shell#Authentication:_OpenSSH_key_management[SSH keys], link:https://en.wikipedia.org/wiki/X.509[X.509] certificates for link:https://en.wikipedia.org/wiki/Transport_Layer_Security[TLS], link:https://en.wikipedia.org/wiki/Time-based_one-time_password[TOTPs] for link:https://en.wikipedia.org/wiki/Multi-factor_authentication[MFA], etc.
You can use the mapping to transfer the credential to or from the resource, generate new keys that follow your security policy, or run a script to perform any advanced actions with the credential.
...
* Set *strength* based on the mapping purpose:
    ** See the xref:/midpoint/reference/admin-gui/resource-wizard/object-type/mapping/advanced-mappings/#mapping-strength[explanation of the mapping strength options].
...
* In *Expression*, you define how to handle the credential.
    ** _As is_ is the simplest option that just passes an existing password from midPoint to the resource or vice versa.
...
* *Exclusive* option ensures only one mapping can target the particular attribute.
    ** Refer to xref:/midpoint/reference/expressions/mappings/#exclusive[] for details.
* *Channel* option provides a way to restrict the mapping applicability only to the context of certain channels.
    ** Refer to xref:/midpoint/reference/expressions/mappings/#channel-constraints[] for details.
...
Let us have a target resource to which you provision accounts as they are creates in another source resource.
Some systems, such as LDAP, do not allow _passwordless_ accounts, meaning you have to create _some_ password,
even if users have to reset it by means of a help desk, for instance, before actually using the account.

This is the situation to have midPoint generate a random password for the new account.
...
but leaves it be in case the source stays the same but the target value is updated from elsewhere.
The expression _As is_ simply copies the password from midPoint to the resource.
