...
However, in case you exchange passwords with a custom resource
or handle accounts with multiple credential types per account, for example,
...
* For [.nowrap]#icon:arrow-right-to-bracket[] *Inbound*# mappings:
    ** *Source* specifies an attribute on the resource side from which to take credentials.
        ...
        However, if your resource objects (e.g., users) have multiple passwords, such as a password _and_ a PIN,
    ...
    ** *Target* specifies an attribute in the midPoint resource object type to which you wish to put authentication tokens.
        You would use this option to copy passwords to an additional custom attribute, for example.
