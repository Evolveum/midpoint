...
[NOTE]
====
You can modify the selected or create a new archetype outside the resource wizard later.
====

.See also the following pages for more information:
* xref:/midpoint/reference/schema/archetypes/[Archetypes overview]
* Details on the built-in xref:/midpoint/reference/schema/archetypes/person/[Person archetype]

image::step-1-object-type-midpoint-data.webp[link=step-1-object-type-midpoint-data.webp, 100%, title=Midpoint data]

=== Select or Create an Archetype

Select *Use existing archetype* and click btn:[Select archetype] to pick an existing archetype.

Select *Create new archetype* to create a new archetype with basic configuration options.
The created archetype will be saved after you save the object type and will be added to its configuration as a reference.

In case of a new archetype, you have the following options:

* *Inherit settings from archetype*: Select archetype which becomes superarchetype for the one you are creating. +
	For example, you can create your own archetype for roles which inherits the `Application role` archetype.

* *Name*: The new archetype name.
	The resource wizard generates it using the resource name and intent, but you can change it.

* *Description*: Short description of the new archetype.

* *Create inducement*: Set to `True` to create an inducement in the new archetype to construct the resource object defined in currently edited object type for all focus object with this archetype.
...
* When you expand the *Display* item, you can set a few human-friendly attributes:
	...
	** *Icon*: The new archetype icon using Font Awesome icon names.
For example `fa fa-briefcase` corresponds to `briefcase` icon in Font Awesome in the default (solid) icon set
...
After you confirm whether your settings produce expected results, you can choose your next steps to configure other parts of your object type configuration:
