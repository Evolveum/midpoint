...
Before configuring correlation rules for a resource, make sure you have configured xref:/midpoint/reference/admin-gui/resource-wizard/object-type/mapping[inbound mapping].
This step is a prerequisite for correlation, and it also helps you identify which attributes are suitable for correlation,i.e. attributes that are stable and sufficiently unique for reliable matching.
...
. Open the list of correlation rules in your resource via [.nowrap]#icon:male[] *Accounts*# > [.nowrap]#icon:cog[] btn:[Configure]# > [.nowrap]#icon:code-branch[] *Correlation*#.
...
+
image::correlations-regenerate.webp[title="Re-generate correlation rules suggestions if you are not happy with the current ones. This will not affect any of the rules you have already accepted."]
+
...
+
TIP: You can always review the simulation results in your correlation rules by clicking the dropdown menu for [.nowrap]#icon:flask[] btn:[Simulate]#.
...
* Check that your resource is connected by going to [.nowrap]#icon:database[] *Resources*# > [.nowrap]#icon:database[] *All Resources*# > resource, and clicking [.nowrap]#icon:question[] btn:[Test connection]#.
