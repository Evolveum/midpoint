...
* `*objectCollectionViews*`: Specifies the set of pages that are used for displaying lists of objects such as Users, Roles, Orgs,...
Influences a list of links in sidebar menu for particular object.
E.g. defining objectCollection for _Employees_ will create new link in sidebar menu under the _Users_, after clicking on the link, filtered list of the _Employees_ is shown.
For more information, please refer to xref:/midpoint/reference/admin-gui/collections-views/configuration/#views[Views] documentation.
...
* `*imageUploadProcessing*`: Defines options for image upload processing.

* `*fileUploadConfiguration*`: Defines options for xref:/midpoint/reference/admin-gui/file-upload[file upload validation].
...
include::./default-list-view-panel-configuration.adoc[]


=== Panel configuration

`ContainerWithStatusWidgetPanel` configuration contains information about two things:
...
| `indirectAssignments`
| Panel showing direct and also indirect assignments.
In details menu present as a submenu of `assignments`.
Limited to use only default "in memory" assignment data provider without a possibility to configure Repository assignment data provider.
|
...
Labels for individual items in the form can be overridden using the item `display/label` definition
(for example, to provide a custom label or translation key for a specific `path`).

NOTE: Currently, only `display/label` overrides are supported at the item level.
Other display settings, such as `icon` or `cssClass`, are not supported for item-level configuration.
            ...
            <container>
                <identifier>basic</identifier>
                    ...
                    <item>
                        ...
                        <display>
                            <label>
                                <orig>Custom display name</orig>
                                <translation>
                                    <key>PageResource.wizard.step.objectType.custom.displayName</key>
                                </translation>
                            </label>
                        </display>
...
Also, it's impossible to configure Repository Search Provider for 'All direct/indirect assignments'.
...
== Customization for audit events list and history panel

Table displaying audit events can be customized in the same way as other tables.
By default, it contains columns:
...
Changed item column is visible only when searching for specific item paths in search panel.
By default, column displays _new values_ of changed items.

You can customize the table by adding or removing columns, changing their order, and changing their visibility.

.Example of additional changed item column displaying estimated old values and new values that's always visible in audit events list
[source, xml]
----
<adminGuiConfiguration>
    <objectCollectionViews>
        <objectCollectionView>
            <!--
                Identifier for audit event list collection.
                Another identifier `history` can be used for object details history panel.
            -->
            <identifier>allAudits</identifier>
            <includeDefaultColumns>true</includeDefaultColumns>

            <column>
                <name>changedItemColumnOldNew</name>
                <path>changedItem</path>
                <!--
                    If visibility is not set to `visible` column will be visible only when search
                    query contains changedItem predicate (search item).
                -->
                <visibility>visible</visibility>
                <!--
                    Possible values for changedItem column are: `oldValue`, `newValue` and `oldAndNewValue`.
                    Default value: `newValue`.
                -->
                <displayValue>oldAndNewValue</displayValue>
            </column>
...
NOTE: If predefined column have to be replaced, it's necessary to use `includeDefaultColumns=false` and enumerate + configure all columns that should be visible in the view.
...
Image Upload Processing configures how uploaded images are processed.
Contains elements 'processing', 'format' and 'stripExifData'.

=== Processing and Format

'processing' attribute specifies how to process uploaded images.
Relevant values are 'preserveOriginalFormat' and 'fixedFormat'. Default value is 'preserveOriginalFormat'.

If the 'preserveOriginalFormat' value is set, the original format of the image is preserved.

If the 'fixedFormat' value is, the midpoint converts the uploaded images to the fixed format specified in the "format" attribute. This conversion removes any EXIF data from the image, if present. The removal of the EXIF data from the image is also performed in the case of conversion to the same format.

"format" attribute specifies which fixed format we want to convert the uploaded images to. It is relevant and is only used if the "fixedFormat" value of the "processing" attribute is set.
Relevant values are 'jpg' and 'png'. Default value is 'jpg'.

[source,xml]
----
<adminGuiConfiguration>
   <imageUploadProcessing>
        <processing>fixedFormat</processing> 
        <format>png</format>
    </imageUploadProcessing>
</adminGuiConfiguration>
----
...
'stripExifData' attribute specifies whether to strip the EXIF data of uploaded images. The attribute value is ignored if the "processing" attribute's "fixedFormat" value is set. In this case, the image format conversion will strip all EXIF data from the image, if present.
Relevant values are 'true' and 'false'. Default value is 'false'.

[source,xml]
----
<adminGuiConfiguration>
   <imageUploadProcessing>
        <stripExifData>true</stripExifData>
    </imageUploadProcessing>
</adminGuiConfiguration>
----
...
The variable can also be set in `setenv.sh` file (see xref:/midpoint/get-started/install/bare-installation/distribution/#post-installation[this] for more).
...
* xref:/midpoint/reference/concepts/system-configuration-object/[]

* xref:/midpoint/reference/security/authorization/[]

* xref:/midpoint/reference/admin-gui/collections-views/use-cases/[]
