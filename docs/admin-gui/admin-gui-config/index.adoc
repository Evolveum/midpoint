...
`fileUploadConfiguration` configures validation and optional processing of uploaded files.
Configuration is defined for individual item paths.

The following options are available:

* `path` - Identifies the item for which the upload configuration applies.
* `checkContentType` - If set to `true`, midPoint detects the actual content type of the uploaded file and verifies that it matches the declared content type and, if configured, one of the values specified by `allowedContentType`.
If set to `false`, content type validation is skipped.
* `allowedContentType` - Specifies an allowed MIME content type.
The element can be specified multiple times, and wildcard values such as `image/*` can be used.
* `convertImageTo` - Converts an uploaded image to the specified format.
Supported values are `jpg` and `png`.
* `stripMetadata` - If set to `true`, metadata is removed from the uploaded image.

For detailed information, default behavior, and configuration examples, see xref:/midpoint/reference/admin-gui/file-upload/[File upload configuration].
...
<adminGuiConfiguration>
    <fileUploadConfiguration>
        <item>
            ...
            <convertImageTo>png</convertImageTo>
