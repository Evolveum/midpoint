...
The label is used for the graphical user interface (GUI).
...
You will see how to create an asynchronous widget in the example for the second widget.
...
. xref:/midpoint/tools/studio/usage/#uploading-midpoint-objects[Upload] both the collection and the dashboard objects to your midPoint instance.

The only step that remains is to add the dashboard to the midPoint graphical user interface.
...
Firstly, modify the widget configuration to include the `displaySourceType` element, allowing the widget to utilize the data that the report has stored in it.
