...
The example above contains an specification definition of the xref:/midpoint/reference/admin-gui/collections-views/#collection-domain[collection domain].
...
The policy rule then computes the ratio of `up` resources vs. all resources and fires a situation in case the ratio drops beneath the `highWaterMark` value.
...
Both watermark objects are of the `WaterMarkType` type and can contain the variables `count` and `percentage`.
