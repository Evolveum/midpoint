...
Use the configuration options described here to customize your dashboards and dashboard widgets in particular.
...
| Use this element in asynchronous widgets to select the data source the widget uses for displaying data.
    ...
    and the widget can then display the saved data without querying midPoint objects through the collection de novo.
...
=== Synchronous widget using object collection

Here is the simplest possible example of a widget with an object collection as its data source:
...
<widget>
    ...
    <data>
        ...
        <collection>
            <collectionRef oid="15de186e-1d8c-11e9-a469-8f5d9cfc0259" type="ObjectCollectionType"/>
        </collection>
    </data>
</widget>
----

=== Asynchronous widget using object collection

Below is a bare-bones asynchronous widget that uses an object collection as its data source.
See xref:/midpoint/reference/admin-gui/dashboards/dashboard-configuration-intro/#async-widget[Create asynchronous widget to reduce system load] for the required configuration regarding asynchronous widgets.
...
<widget>
    ...
    <data>
        ...
        <collection>
            <collectionRef oid="15de186e-1d8c-11e9-a469-8f5d9cfc0259" type="ObjectCollectionType"/> <!--3-->
        </collection>
...
<4> The string here displays only before the connected report runs for the first time.
    The report overwrites this value.
...
=== Specific object as widget data source
...
=== Audit log events as widget data source

To create a dashboard widget that uses audit log entries as a source, you need to create a xref:/midpoint/reference/admin-gui/collections-views/configuration/#collection-with-events-from-audit-log[collection for the audit log records].
Unlike in other collection-based widgets that use `objectCollection` as their source type, in a widget for displaying audit log records, you need to specify the `auditSearch` option as its `sourceType`.

.Dashboard widget enumerating audit log entries filtered by a collection
[source,xml]
----
<widget>
	<identifier>removed-users</identifier>
	<data>
		<sourceType>auditSearch</sourceType> <1>
		<collection>
			<collectionRef oid="0a22293e-5b0a-4e86-83b5-68319edcfb74" relation="org:default" type="c:ObjectCollectionType"> <2>
				<!-- Removed users -->
			</collectionRef>
		</collection>
	</data>
...
----
<1> The `auditSearch` source type signifies that the collection contains audit log events instead of the "usual" midPoint objects.
<2> A reference to a xref:/midpoint/reference/admin-gui/collections-views/configuration/#collection-with-events-from-audit-log[collection that filters the audit log events] for the widget to display.

Should you create object collections for `deleteObject`, `addObject`, and `modifyObject` audit log events, such as the one in xref:/midpoint/reference/admin-gui/collections-views/configuration/#collection-with-events-from-audit-log[], you can create a dashboard for quick overview of new, retired, and modified users.
Such a dashboard can then serve as a simplified overview of the joiner-leaver-mover process.

.Dashboard with widgets for users who were added, modified, and deleted recently
image::audit-log-users-join-move-leave.webp["Dashboard with widgets for users who were added, modified, and deleted"]

When you click the [.nowrap]#btn:[More info] icon:arrow-circle-right[]# button, you get redirected to the filtered audit log where you can inspect the records in more detail.

.Audit log filtered by collection query
image::audit-log-users-modified-list.webp["Audit log filtered by collection query"]
...
* The `presentation` element: Use to select the data presentation style or condition the appearance based on the data.
    Furthermore, the `view` element within the `presentation` element serves to configure the report that can be created based on the dashboard configuration.
...
* In the case of `unit`-type field, the `expression` element holds the `value` element that specifies the string shown after the number (e.g., _users_, _tasks_, _roles_).
...
* A number of objects in the collection and no information about the xref:/midpoint/reference/admin-gui/collections-views/configuration/#collection-domain[collection domain]
    ** For example, _8_
    ** `<style>value-only</style>`
...
[%collapsible]
====
link:https://github.com/Evolveum/midpoint-samples/blob/master/samples/dashboard/howto/dashboard-widget-data-presentation-styles.xml[View on GitHub]
...
| `display`
| `DisplayType`
| Display properties to be applied when the condition evaluates to true.
...
[%collapsible]
====
link:https://github.com/Evolveum/midpoint-samples/blob/master/samples/dashboard/object-collection-resource-up.xml[View on GitHub]

sampleRef::samples/dashboard/object-collection-resource-up.xml[]
====

The example above contains an explicit definition of the xref:/midpoint/reference/admin-gui/collections-views/configuration/#collection-domain[collection domain].
...
When the `policyThreshold` is met, the `policySituation` from `policyRule` can trigger the widget variation.
...
* `lowWaterMark` is the lower bound of the threshold, representing the lowest value for which the policy rule is activated.
    The policy rule triggers for all values above this value up to the `highWaterMark` value (closed interval).
...
* `highWaterMark` is the upper bound of the threshold, representing the highest value for which the policy rule is activated.
    The policy rule triggers for all values between `lowWaterMark` and `highWaterMark` (closed interval).
