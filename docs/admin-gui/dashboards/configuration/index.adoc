...
You need to use the `auditSearch` option as the `sourceType` of the widget instead of `objectCollection`.

.Dashboard widget enumerating audit log entries filtered by a collection
[source,xml]
----
<widget>
    <identifier>modified-users</identifier>
    <display>
        <label>during the past week</label>
        <color>#265DA6</color>
        <icon>
            <cssClass>fa fa-user-pen</cssClass>
        </icon>
    </display>
    <data>
        <sourceType>auditSearch</sourceType> <1>
        <collection>
            <collectionRef oid="e45dc8a1-5782-4fd0-a4b4-952749030a9f" relation="org:default" type="c:ObjectCollectionType"> <2>
                <!-- Audit log - modified users - last 7 days -->
            </collectionRef>
        </collection>
    </data>
    <presentation>
...
You can create a simple user statistics dashboard like the one below by creating object collections for various events happening to user objects and connecting them to one dashboard.
See xref:/midpoint/reference/admin-gui/collections-views/configuration/#collection-with-events-from-audit-log[the guide on audit log collection configuration] for details.
