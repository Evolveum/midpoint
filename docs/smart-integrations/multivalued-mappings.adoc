...
* **Target attribute has data (As-Is check):**
  ...
  ** **Mismatches / Incompatible cardinality:** midPoint cannot match a multivalued attribute (List) with a single-value attribute (String) or collections with different sizes. The As-Is check fails and midPoint proceeds to script suggestion.

* **Script suggestion for multiple values:**
  If values differ and require transformation:
  ** The AI microservice fails when parsing multi-item lists against single-value expectations (raises a 500 error).
