...
The client has to provide the type of the web resource used in the request "{web_resource_type}"
and the oid of the specific object for which we execute the operation "{oid}".

.Generate operation
[source, http]
----
POST http://localhost:8080/midpoint/ws/rest/{web_resource_type}/{oid}/generate
----

.Validate operation
[source, http]
----
POST http://localhost:8080/midpoint/ws/rest/{web_resource_type}/{oid}/validate
----
