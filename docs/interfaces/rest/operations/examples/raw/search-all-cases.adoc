...
[source,bash]
----
curl --user administrator:y0uR_P455woR*d -H "Accept: application/json" -H "Content-Type: application/json" -X POST http://localhost:8080/midpoint/ws/rest/cases/search?options=resolveNames --data-binary @pathToMidpointGit\samples\rest\query-all.json -v
----
