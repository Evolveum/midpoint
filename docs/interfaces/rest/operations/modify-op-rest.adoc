...
[source, http]
----
POST http://localhost:8080/midpoint/ws/rest/{web_resource_type}/{oid}
----

.Modify operation using HTTP PATCH
[source, http]
----
PATCH http://localhost:8080/midpoint/ws/rest/{web_resource_type}/
----
...
In case of the modify operation you need to specify the provided content type of the
payload in the body:
