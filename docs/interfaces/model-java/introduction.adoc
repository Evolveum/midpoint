...
This interface mostly work with midPoint objects as described in our xref:/midpoint/reference/schema/[Data Model]. Please see the wiki:Basic+Data+Model[Basic Data Model] page for a quick introduction to the data model.
...
Therefore it is obvious that also midPoint interfaces will work with wiki:Deltas[deltas]. Similarly to the objects the Java interfaces to midPoint are using deltas provided by xref:/midpoint/devel/prism/[prism framework]. Prism deltas are designed especially to work with prism objects and together they work a complete and elegant data representation and management solution.
...
More sophisticated error handling is possible by inspecting the wiki:OperationResult[Operation Result]. E.g. if a user is modified, the modification is reflected to several accounts using xref:/midpoint/reference/expressions/mappings/[mappings], some of account modification succeed and one of the account modification fails then an exception is *not* thrown.
...
A modify delta can contain several modifications of various modify types (see wiki:Deltas[Deltas]). There may also be more than one object delta in the collection, e.g. to modify user and its accounts in a single midPoint operation (but see limitations below).
