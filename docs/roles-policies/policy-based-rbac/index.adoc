...
However, it is much better to use algorithmic, _policy-based_ assignment of roles as much as possible.
...
This passion lead to the creation of policy-based RBAC in the first place.
...
xref:/midpoint/reference/schema/archetypes/[Archetypes] also act as roles, providing direct and easily-manageable _birthright_ access to various types of objetcs.
...
Benefits of dynamic, policy-based RBAC over a traditional static RBAC are quite clear.
However, policy-based RBAC has significant advantages over other approaches as well, such as policy-based access control (PBAC) or attribute-based access control (ABAC).

In policy-based RBAC the overall *policy can be divided to manageable parts*.
...
Policy-based RBAC naturally divides the policy to roles - small objects that can be easily managed.
...
Perhaps the most significant benefit of policy-based RBAC is its practicality.
Policy-based RBAC is still an RBAC - it is *compatible with static RBAC*.
...
This provides a nice starting point to build up a dynamic policy-based system from the bottom up.
Dynamic RBAC can co-exist with static RBAC in midPoint, providing a feasible evolutionary path.
Policy-based approach can be implemented using an *incremental approach*, without a need for disruptive big bang.
...
Knowledge of the policy is amalgamation of many guidelines, processes, rule books, regulations and management decisions, many of which are not documented at all.
...
These can be incrementally refined, reviewed and optimized.
...
Role definitions form first part of our policy-based RBAC system.
...
MidPoint policy-based RBAC mechanism may look impressive.
Yet, midPoint goes one step further.
...
However, interesting thing happens when archetypes are applied to roles.
...
However, archetypes can be used as _meta-roles_, applying RBAC principles to roles themselves.
