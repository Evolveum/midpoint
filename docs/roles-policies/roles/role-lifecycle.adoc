...
MidPoint contains mechanisms that can be used to manage lifecycle of the roles and role-like objects (xref:/midpoint/architecture/archive/data-model/midpoint-common-schema/roletype/[roles], xref:/midpoint/architecture/archive/data-model/midpoint-common-schema/orgtype/[orgs], xref:/midpoint/reference/misc/services/[services] and even xref:/midpoint/architecture/archive/data-model/midpoint-common-schema/usertype/[users]). Lifecycle management means that midPoint can apply constraints and xref:/midpoint/reference/roles-policies/policies/policy-rules/[rules] to various lifecycle events such as creation of a new role, change of role definition, deletion and so on.
MidPoint can prohibit certain lifecycle transitions, require approval and so on.

The role lifecycle is implemented by combining xref:/midpoint/reference/cases/approval/[approval] process with xref:/midpoint/reference/roles-policies/policies/policy-rules/[policy rules]. The policy rules are triggered when the role is created, modified or deleted.
