...
They are used to assign roles, organizational units, accounts, entitlements and other things to xref:/midpoint/reference/schema/focus-and-projections/[focal objects]. Both assignment and inducement has the same internal structure that is described in xref:/midpoint/reference/roles-policies/roles/assignment/configuration/[Assignment Configuration] page.
