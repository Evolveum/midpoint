...
The wiki:Object+Template[object template] is the right place to implement such automatic assignment.
...
This is easy to implement using an object template wiki:Mapping[mapping] with a static (literal) value:
...
When the condition becomes `false` the role is unassgined (remebmer, midPoint is wiki:Relativity[relativistic] and that also applies to conditions).
...
MidPoint has special `assignmentTargetSearch`wiki:Expression[expression evaluator] designed especially for this purpose.
