...
In fact the role definitions are using wiki:Mapping[mappings] to determine attribute values.
It is the same mechanism that is used in wiki:Assignment[assignments] and wiki:Resource+Schema+Handling[resource schema handling section] therefore the same features and limitations apply here.
Following diagram provides more detailed illustration of use of mappings in the roles.
Each mapping has three parts: source, value constructor and target (see wiki:Mapping[Mapping]). However some of these parts can be determined by the context in which the mapping is used.
...
Please see the wiki:Mappings+and+Expressions[Mappings and Expressions] page for explanation of basic principles of mapping mechanism.
...
Main article: wiki:Assignment[Assignment], wiki:Assignment+Configuration[Assignment Configuration]
...
See wiki:Assignment[Assignment] and wiki:Assignment+Configuration[Assignment Configuration] pages for more details about assignments.
