...
* xref:/midpoint/reference/roles-policies/policies/identity-governance-rules/[Compliance reporting]: Policy rules may be used to set feature:object-mark[marks] in the affected objects.
...
[small]#_Note that on the Policy Rule Examples slide, the presentation incorrectly shows the `report` action instead of the `record` action._#
...
| `exclusion`
| Prohibits an object from being assigned together with another object.
...
| xref:/midpoint/reference/roles-policies/policies/policy-rule-constraints-examples/#modification[Details and example]
...
| `situation`
| Triggers when a policy triggering situation occurs within an object or an assignment.
For example, when a user's role changes.
| xref:/midpoint/reference/roles-policies/policies/policy-rule-constraints-examples/#situation[Example]

| `custom`
| A custom way of evaluating a policy constraint using a script. This is typically used for transitional constraints.
...
| `transition`
| Triggers if the state of an object changes.
    See <<policy-transitions>>.
...
Policy transitions are a means to define _when_ during the operation in question is the policy rule evaluated.
This is useful when you need to evaluate a value which may not exist before or after the operation (usually create and delete operations),
or in case you need to evaluate a change of an attribute value from one value to another.

There are two kinds of policy constraints in midPoint:

* State-related constraints
* Transition-related constraints

A policy with a state-related constraint can evaluate an object at any time and fire because the object is not in the state prescribed by the policy.
A transition-constrained policy rule, on the other hand, can trigger only in the context of an operation.

Furthermore, midPoint distinguishes the states _before_ and _after_ operations:

* Mappings generally consider both states.
* Policy rules consider the _after_ state but you can override it with a policy transition statement.

For example, if your operation deletes an object:

* The focus is defined _before_ the operation (the object still exists).
* The focus is `null` _after_ the operation (there is no object to refer to).

You can create a custom transition-related policy rule constraint by combining two state-related constraints.
For example:

* Before operation: _cost center_ attribute is not null
* After operation: _cost center_ attribute is null.

This policy will trigger only after operations that clear the _cost center_ attribute value;
not for operations that change the value, fill the empty value, or leave the attribute empty.

Policy transition is defined as follows:
...
<policyRule>
    <policyConstraints>
        <transition>
            <stateBefore>true</stateBefore> <1>
            <stateAfter>false</stateAfter> <2>
        <transition>
    <policyConstraints>
</policyRule>
----
<1> Forces evaluation of the policy rule _before_ the operation.
<2> Disables evaluation of the policy rule _after_ the operation.

See an example in xref:/midpoint/reference/roles-policies/policies/policy-rule-constraints-examples/#transition[].
...
* <<activity_pr,In task activities>>
...
[[activity_pr]]
=== Rule Definitions in Task Activities

Since midPoint 4.11, policy rules can also be attached to individual xref:/midpoint/reference/tasks/activities/[task activities].
Such rules apply only to the objects processed by the given activity - either defined inline, taken from a referenced policy object, or applied through virtual assignments.
A typical use case is a threshold that suspends an import or reconciliation activity after a certain number of changes.

See xref:/midpoint/reference/tasks/focus-policies-in-activities/[Focus policies in activities] for details.
Note that this is different from xref:/midpoint/reference/tasks/activity-policies/[activity policies], which use a separate set of constraints reacting to the activity run itself (execution time, number of attempts, item processing errors).
...
* xref:/midpoint/reference/tasks/focus-policies-in-activities/[Focus policies in activities]

* xref:/midpoint/reference/tasks/activity-policies/[Activity policies]
