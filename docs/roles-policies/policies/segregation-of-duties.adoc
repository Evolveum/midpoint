...
In its simplest form, the Segregation of Duties is implemented at the RBAC level by _role exclusions_. It means that roles that _exclude_ each other cannot be assigned to the same user at the same time, or such a situation is followed by another action (e.g., an extra approval workflow).
...
In midPoint 3.5 and later, the SoD is defined by using the xref:/midpoint/reference/roles-policies/policies/policy-rules/[policy rules] mechanism.
...
The `policyActions` part defines what to do when such an exclusion violation is detected.
There are various xref:/midpoint/reference/roles-policies/policies/policy-rules/#policy_actions[policy actions] available.
Following options are the most common for implementing the SoD use cases:

* *Enforcement* means that the role conflict is not allowed.
Any attempt to assign conflicting roles will end up with an error.
...
[NOTE]
====
In the example above, the role Financial Auditor excludes the role Accountant.
...
----
<role>
    <name>Product Student Licence</name>
    <assignment>
        <policyRule>
            <name>Product Employee Licence Exclusion Policy Rule</name>
            <policyConstraints>
                <exclusion>
                    <targetRef relation="org:default" type="c:RoleType">
                        <filter>
                            <q:text>name = 'Product Employee Licence'</q:text>
                        </filter>
                        <resolutionTime>run</resolutionTime>
                    </targetRef>
                </exclusion>
...
[NOTE]
====
In the example above, the Product Employee Licence excludes the Product Student Licence.
To not waste resources, the company does not want to assign both types of licenses to the same person for the same product.
However, the policy may be that the licence that was assigned (or requested by a user) later is the preferred one we want to keep.
With the _prune_ policy action, an attempt to assign the employee licence when the student licence is already assigned will be successful, but the student licence will be unassigned during the process.
In the opposite situation, the employee's licence would be unassigned.
====
...
Thus, a new policy was created, mutually excluding the roles Developer and Tester.
However, as such a change will require updating employees' contracts, we cannot take immediate action but need to choose a more subtle approach.
...
This information can be later used in dashboards, reports, or in the certification campaigns to review the current status and continuously enforce the newly created policy.
====
...
However, this configuration can easily be created and maintained by using xref:/midpoint/reference/roles-policies/roles/rbac/[role hierarchies] and xref:/midpoint/reference/roles-policies/policies/metaroles/gensync/[metaroles].
