...
The role lifecycle is implemented by combining wiki:Approval[approval] process with xref:/midpoint/reference/roles-policies/policy-rules/[policy rules]. The policy rules are triggered when the role is created, modified or deleted.
