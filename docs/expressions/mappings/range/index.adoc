...
New codes are continually introduces, while the legacy codes are slowly removed - without any notice.
...
Such mappings are evaluated in diffirent phases of xref:/midpoint/reference/concepts/clockwork/clockwork-and-projector/[projector computation], which may lead to incorrect results.
