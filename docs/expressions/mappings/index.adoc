...
The domain is very useful especially for automatic xref:/midpoint/reference/roles-policies/roles/assignment/[assignment] expressions in xref:/midpoint/reference/expressions/object-template/[object template].
...
`$projection` +
 +
Note that the attribute, for which this inbound mapping si applied, is automatically added to sources and is presented as `input` variable in the mapping expression.
| N/A
| [.nowrap]#`$focus/givenName`# +
[.nowrap]#`$projection/attributes/firstName`#
...
| Projection (Shadow)
| N/A +
(target is fixed to projection attribute)
| N/A
|
...
Mapping strength defines how aggressively a mapping is applied.
_Strong_ mappings are applied all the time (regardless of the consequences), while _Weak_ mappings are applied only if nothing else can be applied.
The following table summarizes mapping strengths.
...
The mapping value is merged with any other deltas for the same target.
This may cause errors, e.g., if a user requests setting a value different than the one the mapping sets for a single-value attribute.
| The value produced by the mapping must be present in the target property.
If it is not, then reconciliation will add it.


| Normal
| Mapping is applied unless there is a more specific change.
| Mapping is used unless a user has requested a more specific change.
For example, a mapping from user `fullName` to an account will be normally used to set the account `cn` attribute.
But this mapping will not be applied if an administrator explicitly specifies a different value for the `cn` attribute.
| The behavior of normal mappings during reconciliation depends on the direction of the mappings.
See the <<normal_mappings,normal mappings>> section below for more details.


| Weak
| Mapping is applied only if there is a no other change on target property and the target property does not have any value.
...
If not specified, the default strength of a mapping is `normal`.

Mappings created in the xref:/midpoint/reference/admin-gui/resource-wizard/object-type/mapping/[midPoint GUI resource/role wizard] are `strong` by default.
...
While `weak` mappings only apply when no value is present, and `strong` mappings are always enforced, regardless of changes, `normal` mappings are applied in a more sophisticated way.
They are not a simple middle ground between `weak` and `strong` mappings.
Instead, they can be best understood as "relative" or "change-driven".
They are evaluated when a change is detected on their input side.

The behavior of `normal` mappings depends on their direction:

* In *inbound mappings*, `normal` strength behaves similarly to `strong`.
Such mappings are:
** Evaluated whenever there is a change on the source side.
** Evaluated whenever a shadow (account) is loaded into memory during processing.
** Applied during reconciliation.

+
This means that, in practice, inbound `normal` mappings are not skipped during reconciliation.
This is because midPoint lacks precise delta information from the resource during import and live synchronization, and it cannot reliably determine whether input values have changed.
That is why it defaults to evaluating the mappings whenever data is present.

* In *outbound mappings*, evaluation is conditional.
A `normal` mapping is skipped only if it has no sources, or if it has sources but there are no changes in their values. +
Therefore, reconciliation does not universally skip normal mappings.
It only skips them when there is no detected input change.
...
Check xref:/midpoint/reference/concepts/object-lifecycle/[] for details.
...
NOTE: When both setting a `mappingAlias` or an `additionalMappingSpecification`, the value metadata computed previously (i.e., with the original mapping specification) are _not_ automatically migrated.
That means that the mapping recognizes them as its own, but does not convert them into the up-to-date form.
See bug:MID-10084[].
