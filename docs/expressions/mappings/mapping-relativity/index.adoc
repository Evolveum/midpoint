...
TODO: Relativity mechanism is not perfect.

* Target values may be inconsistent, e.g. there may be extra value.
Simple add/delete of values will not fix that.

* One value may be produced by several mappings.
Executing mappings in isolation may destroy values.
