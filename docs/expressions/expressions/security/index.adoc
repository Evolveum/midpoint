...
* Expression profiles may not be completely enforced in midPoint user interface (GUI).
Therefore, security of expressions used in xref:/midpoint/reference/admin-gui/admin-gui-config/[admin GUI configuration (`adminGuiConfig`)] cannot be guaranteed.
Make sure that only trusted top-tier system administrators have ability to modify `adminGuiConfig` configuration in any midPoint object.
