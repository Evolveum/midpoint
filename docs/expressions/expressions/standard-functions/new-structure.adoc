...
These `createEmptyObject` and `createEmptyObjectWithName` functions are used to construct Java instances of an object before adding them to the repository.
Note that these functions construct the objects *in memory only*.
// TODO: add anchor links
If you intend to create an object in the repository or on a resource, you need to populate this object with data and then invoke the `addObject()` or `executeChanges()` function.
...
Returns approver references from the given object or assignment.
...
Returns the collection of modification approver references for the given assignment.

// TODO Move approver ref fns together
