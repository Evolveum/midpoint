...
The `cel.bind` macro binds result of an expression to a local variable name:

`cel.bind(_variable_, _value expression_, _result expression_)`

This macro can be used to avoid duplicating an expensive computation.
For example:
...
Returns type of the object in QName form.
The type corresponds to types as specified in midPoint schema, (e.g. `UserType`, `RoleType`).
...
xref:/midpoint/reference/concepts/item-path/[Item path] is a simple syntax for addressing properties, containers, references, attributes and similar items in data structures used in midPoint.
Simply speaking, item path is a sequence of item names (QNames) and optionally container identifiers.

In MEL, item path is part of some data structures (e.g. item delta), or it can be used as a parameter to some functions (e.g. `object.findItem()`).
Item path can be constructed using the `itemPath()` function:

[source,mel]
----
itemPath(['activation', 'effectiveStatus'])
----

Segments of the item path can be accessed as `segments` field using the _dot notation_:
...
Item path can be used to access specific part of midPoint objects using the `findItem()` function:

[source,mel]
----
object.findItem[itemPath(['activation', 'effectiveStatus'])]
----

Item path can be compared with strings and QNames.

[source,mel]
----
itemPath(['activation', 'effectiveStatus']) == 'activation/effectiveStatus'    // Returns true
----

=== itemPath

`itemPath(list(any))` -> itemPath +
`itemPath(string)` -> itemPath

Creates an item path value.

When list parameter is specified, item path is created from the list of segments provided as parameter.
The segments representing item names may be specified as strings or QNames, the container identifier segments are specified as integers.

When string parameter is used, the item path is constructed from its string representation.
...
MEL brings support for several midPoint-specific data structures, such as assignments and deltas.
These data structures are represented by structured objects in MEL.

MidPoint objects and containers defined by xref:/midpoint/reference/schema/[midPoint schema] are represented in MEL in the same form as they are defined in the schema (see xref:/midpoint/reference/schema/schemadoc/[SchemaDoc]).

=== Object Reference

Object reference MEL data structure corresponds to the `ObjectReferenceType` of midPoint schema.
It is used to reference one object from another object.
Following table summarizes the most frequently used fields.

.Object Reference Data Structure Fields
|===
| Field | Type | Description

| `oid`
| string
| OID of an object which is the target of the reference.

| `type`
| qname
| Type of an object which is the target of the reference.

| `relation`
| qname
| The relation or a "role" of this reference.
It may further specify the meaning of the reference.
See xref:/midpoint/reference/concepts/relation/[relation page] for more details.
...
Fields of _assignment_ data structure in MEL are given by `AssignmentType` midPoint type, as specified in midPoint schema.
Following table summarizes the most frequently used fields.

.Assignment Data Structure Fields
|===
| Field | Type | Description

| `identifier`
| string
| Identifier of assignment, unique to the object.
...
`assignment.hasDefaultRelation()` -> boolean

Returns true is the assignment has default xref:/midpoint/reference/concepts/relation/[relation].

This function is null-safe, it returns `false` in case that assignment has no `targetRef`.
...
`assignment.hasRelation(any)` -> boolean

Returns true if `targetRef` of the assignment has specified xref:/midpoint/reference/concepts/relation/[relation].
String or QName representation of the relation can be used.
...
`assignment.hasApproverRelation()` -> boolean +
`assignment.hasConsentRelation()` -> boolean +
`assignment.hasDeputyRelation()` -> boolean +
`assignment.hasMetaRelation()` -> boolean +
`assignment.hasOwnerRelation()` -> boolean +
`assignment.hasRelatedRelation()` -> boolean
...
`assignment.isTarget(type, archetypeOid)` -> bool

Returns true if `targetRef` of the assignment is of specified type.
When archetype OID is specified, the function also checks for presence of specified archetype in target object.
...
// Checks whether the target is business role.
assignment.isTarget('RoleType', '00000000-0000-0000-0000-000000000321')
----

===== isTarget*

`assignment.isTargetRole()` -> bool +
`assignment.isTargetRole(archetypeOid)` -> bool +
`assignment.isTargetOrg()` -> bool +
`assignment.isTargetOrg(archetypeOid)` -> bool +
`assignment.isTargetService()` -> bool +
`assignment.isTargetService(archetypeOid)` -> bool +
`assignment.isTargetPolicy()` -> bool +
`assignment.isTargetPolicy(archetypeOid)` -> bool +
`assignment.isTargetUser()` -> bool +
`assignment.isTargetUser(archetypeOid)` -> bool


Returns true if `targetRef` of the assignment is of specified type.
When archetype OID is specified, the function also checks for presence of specified archetype in target object.

When archetype parameter is specified, invocation of this function may include retrieval of the target object from the repository, which may have performance impact if used too often.
...
// Checks whether the target is business role.
assignment.isTargetRole('00000000-0000-0000-0000-000000000321')
----
...
Returns object that the targetRef of the assignment refers to.

Returns `null` in case that the assignment is `null`, or if it does not contain `targetRef`, or if the reference cannot be resolved.

Invocation of this function may include retrieval of the target object from the repository, which may have performance impact if used too often.
...
`assignment.targetName()` -> polystring

Returns name of the object that the targetRef of the assignment refers to.
...
[source,mel]
----
'Target:' + assignment.targetName()
----


===== targetOid

`assignment.targetOid()` -> string

Returns OID of targetRef of the assignment.

This function is null-safe, it returns `null` in case that assignment has no `targetRef`.

.Example
[source,mel]
----
assignment.targetOid()   // Returns '847ab384-8a63-11f1-ad15-9c6b00360713'
...
Returns relation of targetRef of the assignment in QName form.

As QNames can be safely compared with strings, string representation of relations can be used in comparisons.
This function is null-safe, it returns `null` in case that assignment has no `targetRef`.
...
As QNames can be safely compared with strings, string representation of the type can be used in comparisons.
This function is null-safe, it returns `null` in case that assignment has no `targetRef`.
...
xref:/midpoint/devel/prism/concepts/deltas/[Object delta] data structure represents a change of an midPoint object.
The MEL _object delta_ data structure represents xref:/midpoint/devel/prism/concepts/deltas/[internal representation of object delta] used by midPoint (or more exactly the xref:/midpoint/devel/prism/[Prism] data representation library).
It roughly corresponds to `ObjectDeltaType` from Prism schema.

.Object Delta Structure Fields
|===
| Field | Type | Description

| `oid`
| string
| OID of object which is modified or deleted by the delta.

| `objectType`
| qname
| Type of object which is modified or deleted by the delta.

| `changeType`
| string
| Type of change described by the delta:

* `add`: new object is added
* `modify`: existing object is modified
* `delete`: existing object is deleted

| `objectToAdd`
| object
| Object being added.

The value is provided only if `changeType` of the delta is `add`.

| `itemDelta`
| list(Item Delta)
| List of item deltas, specifying changes of individual items of the object.

The value is provided only if `changeType` of the delta is `modify`.

|===

==== Object Delta Functions

===== estimateAddedValuesFor

`objectDelta.estimateAddedValuesFor(path)` -> list(any)
...
List of values is returned if the values can be determined.
Empty list is returned if the function have determined that there are no added values (value is known to be empty).
Null is returned if the estimation method cannot determine the value (value is unknown).

This function is null-safe, it returns `null` in case that the delta or path are `null`.

.Example
[source,mel]
----
delta.estimateAddedValuesFor('assignment')
----

===== estimateChangedValuesFor

`objectDelta.estimateChangedValuesFor(path)` -> list(any)
...
List of values is returned if the values can be determined.
Empty list is returned if the function have determined that there are no changed values (value is known to be empty).
Null is returned if the estimation method cannot determine the value (value is unknown).
...
[source,mel]
----
delta.estimateChangedValuesFor('fullName')
----


===== estimateDeletedValuesFor

`objectDelta.estimateDeletedValuesFor(path)` -> list(any)
...
List of values is returned if the values can be determined.
Empty list is returned if the function have determined that there are no deleted values (value is known to be empty).
Null is returned if the estimation method cannot determine the value (value is unknown).
...
[source,mel]
----
delta.estimateDeletedValuesFor('assignment')
----

===== estimateModifiedValuesFor

`objectDelta.estimateModifiedValuesFor(path)` -> list(any)
...
i.e. the values that were present before the operation and are present after the operation, however their content is different.
...
List of values is returned if the values can be determined.
Empty list is returned if the function have determined that there are no modified values (value is known to be empty).
Null is returned if the estimation method cannot determine the value (value is unknown).
...
[source,mel]
----
delta.estimateModifiedValuesFor('assignment')
----

===== estimateNewValuesFor

`objectDelta.estimateNewValuesFor(path)` -> list(any)
...
List of values is returned if the values can be determined.
Empty list is returned if the function have determined that the item will have no values when the delta is applied (value is known to be empty).
Null is returned if the estimation method cannot determine the value (value is unknown).

.Example
[source,mel]
----
delta.estimateNewValuesFor('fullName')
----

===== hasDeltaFor

`objectDelta.hasDeltaFor(path)` -> boolean
...
[source,mel]
----
delta.hasDeltaFor('fullName')
----

===== findItemDelta

`objectDelta.findItemDelta(path)` -> itemDelta
...
[source,mel]
----
delta.findItemDelta('fullName')
----

===== isItemChanged

`objectDelta.isItemChanged(path)` -> boolean
...
therefore it may not be entirely reliable in some situations.
...
[source,mel]
----
delta.isItemChanged('fullName')
----


=== Item Delta

xref:/midpoint/devel/prism/concepts/deltas/[Item delta] data structure represents a change of one individual item of an midPoint object.
The MEL _item delta_ data structure represents xref:/midpoint/devel/prism/concepts/deltas/[internal representation of item delta] used by midPoint (or more exactly the xref:/midpoint/devel/prism/[Prism] data representation library).
It very roughly corresponds to `ItemDeltaType` from Prism schema, although the MEL data structure has a slightly different fields.

.Item Delta Structure Fields
|===
| Field | Type | Description

| `path`
| Item Path
| Path of the item (container of property) being changed.

| `valuesToAdd`
| list(any)
| List of values that are added to this item.
New values are added to the existing values of the object.
Existing values of the object are not affected.

Contains `null` value if no values are added.

| `valuesToDelete`
| list(any)
| List of values that are deleted from this item.
Existing values not specified by this field are not affected.

Contains `null` value if no values are deleted.

| `valuesToReplace`
| list(any)
| List of values that are set to this item.
Existing values of the item are discarded, and replaced by the values provided in this field.

Contains `null` value if no values are replaced.

| `estimatedOldValues`
| list(any)
| List of values that are estimated to be present in the item before the delta was applied.

Contains empty list if it is estimated that the item had no value.
Contains `null` value if the old values are unknown.

|===

The item delta can contain either added/deleted values, or replaced values, but not both.
...
_Audit event record_ data structure describes a single event (usually data change) in a format suitable for xref:/midpoint/reference/security/audit/[auditing].

It roughly corresponds to `AuditEventRecordType` from MidPoint schema, which is a representation of one record in midPoint xref:/midpoint/reference/security/audit/[audit log].

.Audit Event Record Structure Fields
|===
| Field | Type | Description
...
This field is an identifier of the task.
...
used to evaluate access to the operation.
...
| structured(EffectivePrivilegesModificationType)
| resent if the effective privileges used to execute the operation differ or may differ from the regular
(declared) privileges of the "effectivePrincipalRef".
...
* `modifyObject`: todo
* `deleteObject`: todo
* `executeChangesRaw`: todo
* `synchronization`: todo
* `createSession`: todo
* `terminateSession`: todo
* `workItem`: todo
* `workflowProcessInstance`: todo
* `reconciliation`: todo
* `suspendTask`: todo
* `resumeTask`: todo
* `runTaskImmediately`: todo
* `discoverObject`: todo
* `informationDisclosure`: todo


| `delta`
| Object Delta Operation
| Data structure describing the changes (delta) and results of application of the changes (operation result).
...
| `outcome`
| string
| Type of result of the operation:

* `success`: Operation was executed successfully.
* `warning`: Operation was executed successfully, but there were warnings.
* `partial_error`: Operation was executed with partial error. Some parts of the operation were successful, other parts have failed.
* `fatal_error`: Operation was not executed, it ended up in an error that prohibited completion of operation.
* `handled_error`: Execution of an operation caused an error, which was successfully handled. Effects of the operation are equivalent to successful operation.
* `not_applicable`: Result does not make any sense for the operation. E.g. unsupported operation which was skipped.
* `in_progress`: The operation is still being executed.
* `unknown`: No information about operation is present, which usually means programming bug.

| `result`
| string
| Result of the operation (e.g. number of entries, returned object, business result of workflow task or process instance - approved, rejected).
...
| list(AuditEventRecordCustomColumnPropertyType)
| Field of custom column for audit record table.

|===

// ==== Audit Event Record Functions
...
_Object delta operation_ data structure represents an executed change of an midPoint object.
Simply speaking, it describes an xref:/midpoint/devel/prism/concepts/deltas/[object delta] that was already applied, including results of the operation that applied the delta.
_Object delta operation_ data structure is mostly used in midPoint xref:/midpoint/reference/security/audit/[audit records].
It roughly corresponds to `ObjectDeltaOperationType` from MidPoint schema.

.Object Delta Operation Structure Fields
|===
| Field | Type | Description

| `objectOid`
| string
| OID of object which was added, modified or deleted by the delta.

| `objectDelta`
| Object Delta
| Delta that was applied to the object.

| `executionResult`
| Operation Result
| Result of the operation that applied the delta.
...
| `shadowIntent`
| string
| Shadow intent of the shadow, that is related to this operation.
Useful for informational purposes, typically in audit records.

|===

// ==== Object Delta Operation Functions
...
xref:/midpoint/architecture/concepts/operation-result/[_Operation result_] data structure provides rich information about an operation being executed, primarily for recording, error reporting and troubleshooting.
It is also used in xref:/midpoint/reference/security/audit/[audit records] as part of _object delta operation_ data structure.
It roughly corresponds to `OperationResultType` from MidPoint schema.

.Operation Result Structure Fields
|===
| Field | Type | Description
...
This is optional, as some statuses do not need a message (e.g. success).

|===

// ==== Operation Result Functions
