...
`assignment.isTarget(any)` -> boolean

Returns true if `targetRef` of the assignment is of specified type.
String or QName representation of the type can be used.
...
`assignment.isTargetRole()` -> boolean +
`assignment.isTargetOrg()` -> boolean +
`assignment.isTargetService()` -> boolean +
`assignment.isTargetPolicy()` -> boolean +
`assignment.isTargetUser()` -> boolean
