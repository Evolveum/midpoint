...
isNil('foo')    // Returns false
----

Function `isNil()` can be used with structured data as well.
...
It is strongly recommended to use `isNil()` function instead of `null` equality check (e.g. `foo == null`) in all cases.
...
It is recommended to use `isNil()` and `isPresent()` functions instead (see below).
...
Therefore, it is recommended to use functions `isPresent()` and `isNil()` for checks, and `format()` for string formatting when dealing with optional values.
