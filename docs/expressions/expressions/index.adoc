...
// TODO: missing evalutors: function, referenceSearch, shadowOwnerReferenceSearch, assignmentFromAssociation, assignmentFromAssociation, assignmentFromAssociation, proportional
...
Each value construction needs to have at least one evaluator to be useful.
...
Following evaluator is referencing the `name` property in the object stored in `focus` variable.

.Path expression evaluator referencing user's name property
[source,xml]
----
<expression>
...
[TIP]
The path evaluator is following the path through xref:/midpoint/devel/prism/[prism objects] directly which is much faster.
This also allows the path evaluator to follow the modification mode (delta) of the source property which is very useful in some situations.
Therefore, as a rule of the thumb the path evaluator should always be preferred if it is possible to use it.

=== Script

Script expression evaluator is the most flexible of all the evaluators as it allows to invoke an expression written in an expression or scripting language.
...
.Expression evaluator using Groovy script
[source,xml]
----
<expression>
    <script>
...
.Expression evaluator using ECMAScript (JavaScript) script
[source,xml]
----
<expression>
    <script>
...
[#_java_method_reference]
=== Java Method Reference

#TODO provide reasoning here#

.Expression evaluator using Java method reference
[source,xml]
----
<expression>
    <javaMethodReference>
        <className>com.example.scripting.extensions.BasicFunctions</className>
        <methodName>generateName</methodName>
    </javaMethodReference>
</expression>
----

#TODO example Java code and things from XSD#
...
See xref:/midpoint/reference/expressions/expressions/filter/[Filter Expression Evaluator] page for more details.


=== Generate

The _generate_ evaluator is used to generate a random value.
...
Mappings and expressions are often used to create xref:/midpoint/reference/roles-policies/roles/assignment/[assignments]. Therefore there is a special-purpose expression evaluator that simplifies the way how assignments are created.
...
When such object is found the evaluator creates an xref:/midpoint/reference/roles-policies/roles/assignment/[assignment] for that target.
...
If such an object is found than an appropriate xref:/midpoint/reference/roles-policies/roles/assignment/[assignment] structure is created, the xref:/midpoint/devel/prism/concepts/object-identifier/[OID] of the org object is placed inside it.

[[include-null-inputs]]
==== Force null input processing

Search expression evaluators do not process null inputs by default.
The rationale is that null inputs are typically irrelevant to searching, and skipping them reduces the number of search operations.

If you need null inputs to be processed—for example, when the evaluator should produce a default value even when the input is absent—use `<includeNullInputs>true</includeNullInputs>`.

If the evaluator is not producing a value you expect, enabling `includeNullInputs` is the first thing to check.
It will make midPoint slightly slower, but it may resolve the issue.

[[default-target-reference]]
==== Default target reference

In case you work with a filter that may not return any value, you can specify a fallback target reference that would be returned in such a case.

.Default target reference example usage
[source,xml]
----
<assignmentTargetSearch>
   <c:targetType>RoleType</c:targetType>
   <filter>
       <q:text>identifier = $input</q:text>
   </filter>
   <defaultTargetRef oid="6def1235-d25b-4068-a436-5e6f3cb78756" type="RoleType"/> <1>
</assignmentTargetSearch>
----
<1> The role object returned by the search in case the filter does not match any object.

Note that if the `$input` is null and the link:#include-null-inputs[`includeNullInputs`] is not used to force processing of null inputs, the whole search is skipped and the default target reference is not used.
...
[NOTE]
====
You can use the link:#default-target-reference[default target reference] and link:#include-null-inputs[force null input processing] for association target search the same way as for assignment target search.
====
...
.Expression evaluator using explicit variable
[source,xml]
----
<expression>
    <path>$focus/name</path>
...
.Expression evaluator using root node
[source,xml]
----
<expression>
    <path>name</path>
