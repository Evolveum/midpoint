...
In case you work with a filter that may not return any value, you can specify a fallback target reference that would be used in such a case.
...
[NOTE]
====
You can use both the link:#default-target-reference[default target reference] and link:#include-null-inputs[includeNullInputs] options for association target search the same way as for assignment target search.
====
