...
[[null-focus]]
== Resolving Focus Is Null Situation
...
The usual cause for this error is because you evaluate a variable (e.g., focus) when it does not exist _yet_ or it does not exist _anymore_ in the particular context.

When focus is null, e.g., in policy rules during delete operations, and you need to access the focus OID in a script used in the policy rule triggering the delete operation, you can access it via the focus context:
...
Another option to avoid the "focus is null" situation is to leverage the fact that midPoint distinguishes the states _before_ and _after_ operations.
In policy rules, if focus is undefined _after_ an operation (e.g., delete object), it may help to evaluate it _before_ the operation when it still exists.
It works both ways (i.e., evaluate it _after_ a create operation).

Setting the phase in which variables are evaluated is done using *policy transitions*.
Refer to xref:/midpoint/reference/roles-policies/policies/policy-rules/#policy-transitions[] for details.
