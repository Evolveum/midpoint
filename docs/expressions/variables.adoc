...
[[null-focus]]
== Situations When Focus Is Null

In some execution contexts, the focus variable is undefined (`null`).
This is expected.
In this section, we explain when and why this happens and how you may work around it.

MidPoint distinguishes the states _before_ and _after_ operations:

* Mappings generally consider both states.
* Policy rules consider the _after_ state.
    ** With the exception of explicitly defined policy transition (`TransitionPolicyConstraintType`, see below).

For example, if your operation (e.g., a policy rule) deletes an object:
...
This works for create operations as well, only inversely.

=== Circumventing Null Focus OID

When focus is `null` (such as in policy rules during delete operations) and you need to access the focus OID in a script used in the policy rule triggering the delete operation, you can access it via the focus context:

[source,groovy]
----
def deletedObjectOid = midpoint.getFocusContext()?.oid
----

For policy rules that must evaluate the object before it is deleted, you can configure a transition policy rule with `stateBefore` set to `true`:
...
<transition>
    ...
    <constraints>
        <!-- constraints evaluated before the operation -->
    </constraints>
</transition>
----
...
The projection is usually legal if there is an xref:/midpoint/reference/roles-policies/roles/assignment/[assignment] for it.
...
E.g. the variables specific to assignment processing are described in the xref:/midpoint/reference/roles-policies/roles/assignment/configuration/[Assignment Configuration] page.
