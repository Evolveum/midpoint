...
As described in <<Limitations>>, the object template must be connected to the resource object type via the archetype declared in the object type definition.footnote:[The main reason is that midPoint has to know the archetype _before_ the correlation-time mappings are evaluated.
...
As a general rule, when referencing a configuration in an object template, it must be bound to the resource object type in question via statically-defined archetype (see Listing 3 and 4 in <<Example 3: Object Template Based Correlation Definition>>).

Other limitations are mentioned on pages that describe individual features in more details:
