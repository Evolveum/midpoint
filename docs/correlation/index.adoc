...
Correlation (also known as _smart correlation_) is a mechanism to correlate identity data to existing focus objects in the repository.
...
The goal for _correlation_ is to provide a configurable correlation mechanism that can provide approximate matching.
...
|===
| Type | Meaning
| `items` | xref:/midpoint/reference/correlation/items-correlator/[Item-based] correlation rule. The suggested one.
...
Correlation relies on inbound mapping converting resource's attribute to a property of midPoint object.
