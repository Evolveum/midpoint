...
MidPoint has several built-in object types such as xref:/midpoint/reference/roles-policies/rbac/[roles], xref:/midpoint/reference/org/organizational-structure/[orgs], and wiki:Services[services]. Those data types are built to be extensible and in fact, they are quite generic.
...
* There is also a big benefit from xref:/midpoint/features/planned/lawful-bases-for-data-processing/[data protection perspective]. As the entitlements are placed directly into job categories and qualifications it is easy to find the reason or "basis" why the user has such an entitlement.
