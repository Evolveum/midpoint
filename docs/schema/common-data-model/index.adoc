...
MidPoint is an wiki:IDM+Means+Integration[identity integration] system.
...
MidPoint provides a concept of an wiki:MidPoint+Common+Schema[User] that acts as a central point in the data model.
...
The User object is one of the objects of midPoint wiki:Data+Model[Data Model] that is built on top of wiki:Prism+Objects[Prism Objects]. MidPoint data model is a kind of _common denominator_ model.
...
The support for ordering may substantially complicate the system especially while considering our wiki:Consistency+Model[consistency model]. It is not sufficient to simply add a value to the ordered field.
...
MidPoint can access such objects (using wiki:Connectors[Connectors]) and work with them.
But working with such objects is difficult.
E.g. they do not have wiki:Object+ID[OID] and therefore cannot be directly linked to other objects in midPoint repository.
...
Account shadow are then linked to the user by ordinary midPoint linking using wiki:Object+ID[OID]. This indicates that a user _owns_ an account.

image::synchronization-user-account.png[]



See wiki:Shadow+Objects[Shadow Objects] for more information.
...
See wiki:Junkyard[junkyard page] for more details.
...
* wiki:IDM+Means+Integration[IDM Means Integration]