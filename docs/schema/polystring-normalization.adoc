...
The basic idea is that is the user is searching for user `naiveboy123` then the users `naiveBOY123` and `NaiveBoy123` are found.
The other effect is that if someone already registered `naiveboy123` username then other user cannot register `naiveBOY123`, `NaiveBoy123` and `NAIVEboy123` usernames.
...
The _orig_ form is used for the vast majority of purposes: displaying the value, editing the value and so on.
...
. *Decomposition (_nfkd_)*: composed characters (such as é) are decomposed to the constituent parts (e and '). Unicode Normalization Form Compatibility Decomposition (NFKD) is used for this purpose.
