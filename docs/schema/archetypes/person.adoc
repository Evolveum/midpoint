...
image::PersonArchetype.png[Example of using the Person archetype]
...
* `generate-name-jsmith-8-2`: Mapping to generate the midPoint username from user's `givenName` and `familyName` properties using the `jsmith` naming convention.
At most 8 characters are used.
To achieve username uniqueness, an iteration token is appended (2-99).
    ...
    ** This mapping has the lifecycle state set to `draft` by default, i.e., it is effectively disabled by default.
