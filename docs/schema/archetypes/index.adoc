...
A _project_ is only a special case of an xref:/midpoint/reference/org/organizational-structure/[organizational unit], just as much as _division_, _section_, _company_, _workgroup_, or any other concept that groups identities.
An _application_ is a special case of a xref:/midpoint/reference/misc/services/[service], just as much as _mobile device_, _printer_, _cloud service_, _web API endpoint_, and so on.
All those concepts are just specializations of the object types that midPoint already has.

There is an enormous advantage to reusing existing midPoint concepts.
...
As a result, _projects_ can have managers, they may be used for delegated administration, they can grant privileges to members and managers, and so on.
Whatever is already implemented for organizational units, can instantly be reused for _projects_.

In a similar fashion, an _application_ can inherit existing functionality from a _service_ (e.g., the ability to grant privileges).
This kind of reuse is very convenient and it is also an extremely powerful mechanism.
...
Each object of that archetype will contain an archetype definition OID.
...
Archetype definition are xref:/midpoint/architecture/concepts/abstract-role/[abstract roles] which means they essentially work as roles.
...
That means archetypes may work as xref:/midpoint/reference/roles-policies/policies/metaroles/policy/[metaroles].
However, this mechanism can also be reversed, and archetypes may be assigned from ordinary roles or orgs.
...
Subtype will still work in midPoint 4.x, but it is no longer a recommended mechanism - except for the assignment subtype, which is still useful.
Subtypes will be most likely deprecated sometime during the lifetime of midPoint 4.x, and the plan is to remove it completely in midPoint 5.0.
...
The list of all archetyped objects can be available right from midPoint menu.
