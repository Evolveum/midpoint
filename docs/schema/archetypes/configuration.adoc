...
Being a midPoint object, archetype has a natural persistent identifier: xref:/midpoint/devel/prism/concepts/object-identifier/[OID]. OID of the archetype definition is also an archetype identifier.
Each object of that archetype will contain the archetype definition OID in the `archetypeRef` reference:
...
From that point on, the archetype applies to that object.
...
Such a view can automatically render buttons to create new objects.
...
Therefore an archetype definition can have assignments pointing to it, such as an owner of an archetype definition.
...
E.g., an object policy for `UserType` inheriting from object policy for `ObjectType` - and user archetypes inheriting from the `UserType` policy.
====
...
Archetype definitions can specify the details of behavior for "archetyped" objects.
But there are also objects that do not have any archetype.
We may want to specify behavior for those objects as well.
And in fact, this has been possible in midPoint for a long time, even though it was not explicitly denoted as having anything to do with archetypes.

There is the `defaultObjectPolicyConfiguration` container in xref:/midpoint/reference/concepts/system-configuration-object/[system configuration object]. +
Its data structure of is almost identical to the structure of `archetypePolicy` in the archetype definition.
...
And parts of that definition may also apply to archetyped objects, as this definition is merged with `archetypePolicy`.
Of course, `archetypePolicy` will override any aspects of the default specification.
...
Therefore, midPoint has a few default archetypes that can be used as a starting point for further midPoint configurations
...
They are provided in the initial objects mostly to keep the terminology of midPoint deployments aligned.
This makes communication in the midPoint community smoother.
...
* Structural
** This represents the main class/type of an object (e.g., Person, Service account, Application) and define its properties (including visuals), behavior, birthrights, and so on.
** It is expected to be practically static during the lifetime of the object.
Typically, the object is created with the archetype and keeps it forever.
** Only one such archetype can be assigned to the object directly.
** It can be extended, and it can extend other structural or abstract archetypes.

* Auxiliary
** Can be assigned to an object with a structural archetype assigned.
...
It contains an archetype policy defining what the icon should look like, and two xref:/midpoint/reference/admin-gui/sections-object-details/[GUI virtual containers (sections)] used on task details page: *Advanced options* and *Operational attributes* (state).
...
For example, you can apply an `Employee` archetype to users, and a `Project` archetype to orgs, however, you cannot apply one archetype to both to users and orgs.
...
`assignmentRelation` is not supported in metaroles or other mechanisms that require higher-order inducement or inducement chaining.
...
E.g., the color of the "summary panel" on user details page will be red, regardless of the archetype, as red is currently the color associated with users.
