...
Please see wiki:Archetypes[Archetypes] page for a more in-depth introduction to archetype concepts.
...
That means archetypes may work as ordinary roles or wiki:Using+Metaroles+for+Policy+Configuration[metaroles]:
...
Archetypes can be added to menus and other places of midPoint interface by using wiki:Object+Collections+and+Views[object views]. All that is needed is simple definition of a view in wiki:Admin+GUI+Configuration[admin GUI configuration structure]:
...
There is `defaultObjectPolicyConfiguration` container in wiki:System+Configuration+Object[system configuration object]:
...
_
| Archetype for cases that describe wiki:Manual+Resource+and+ITSM+Integration[manual provisioning operations].
...
Archetype can be used as a criterion in wiki:Authorization+Configuration[authorizations]:
