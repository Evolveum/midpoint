...
| xsd:int (Integer)
a| stored in `ext` JSONB as numeric
...
a| Full support.
Native repository stores it in `ext` JSONB as numeric.
| xsd:decimal (BigDecimal)
a| Not fully supported by Prism API
.3+a| Full support in the Native repository, stored in `ext` JSONB as numeric.
| xsd:double (Double)
