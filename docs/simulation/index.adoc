...
They are accompanied by a simple xref:/midpoint/reference/simulation/tutorial/[tutorial].
...
MidPoint supports this by allowing us to avoid putting resource definition into the production configuration, and keeping it in development one only.
...
However, classification in development-mode resources can be made more flexible.
The performance is not crucial for them.
The stability is not important either, as the shadows on such resources should not be linked to any focal objects yet.

Hence, if a shadow on a development-mode resource is processed in a simulation-development mode, it is re-classified each time it is seen.
