...
* recomputation,
* iterative and non-iterative scripting (provided that model-level API is used to execute the actions, see <<Limitations>>),
* change execution.
...
Examples: direct modification through the repository or provisioning API; direct manipulation of outside objects (files, sockets, and so on).
