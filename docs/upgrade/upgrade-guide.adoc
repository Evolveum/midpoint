...
NOTE: Starting with midPoint version 4.8 ninja tool was extended.
...
The database does not only contain configuration, but also internal midPoint state and metadata that cannot be reconstructed directly from source systems.
This includes e.g. shadow objects and their links to identities, synchronization state, tokens and audit history.
...
You can upgrade from one major/minor version to the subsequent one (4.1 → 4.2)
Upgrade is only supported between consecutive major/minor versions.

* 4.1 → 4.2 is supported.
* 4.1 → 4.3 is *not supported*.

To get from 4.1 to 4.3, you will need to upgrade to 4.2 first.

It is possible to upgrade from one *LTS* version to the other.

* 4.0 → 4.4 is supported, because it is an upgrade from one LTS to the subsequent one.
...
This isolates the users from the system during upgrades and avoids any unnecessary user confusion.
...
To run those scripts, you can use the `ninja` command `run-sql`.
For more information see xref:/midpoint/reference/deployment/ninja/command/run-sql/[The detailed `run-sql` documentation].
...
TODO: Ad example of database update, update article database schema upgrade.
