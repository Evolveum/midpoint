...
:page-toc: top
:page-description: This guide describes how to upgrade midPoint native PostgreSQL repository.
:page-keywords: repository, database, postgresql, upgrade, native repository
...
IMPORTANT: Database schema upgrades often introduce new database columns values of which can be derived from the existing object data.
Although the required information is already present in the repository as part of the complete object representation, the newly introduced columns remain empty until the objects are reprocessed.
Running the Reindex Repository Task recomputes these values and populates the new columns.
...
[IMPORTANT]
====
If you xref:/midpoint/reference/repository/native-postgresql/usage/#installing-as-non-superuser[created the schema objects as non-superuser],
...
By default, the database username is the current operating system username, and the database name is the same as the selected database user.
If no host is specified, `psql` connects to the local PostgreSQL server.

Visit https://www.postgresql.org/docs/current/app-psql.html[the PostgreSQL documentation] to learn more about the default connection behavior.
====
...
Do not use them to run upgrade scripts!
====
...
As an alternative, upgrade scripts can be executed using the xref:/midpoint/reference/deployment/ninja/[Ninja tool].
...
To upgrade the database schema using Ninja, see xref:/midpoint/reference/deployment/ninja/command/run-sql/[Run SQL].
