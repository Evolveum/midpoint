...
If you have followed xref:/midpoint/reference/upgrade/upgrade-with-ninja/[] page, after the application is upgraded, the `lib/` directory contains both (old) `midpoint.war` and (new) `midpoint.jar`.
...
See also xref:/midpoint/get-started/install/bare-installation/systemd/[Running midPoint with systemd].
