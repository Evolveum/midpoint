...
Because those objects will have unpredicatable xref:/midpoint/devel/prism/concepts/object-identifier/[OIDs]. Therefore resource definitions (wiki:ResourceType[ResourceType]) cannot have fixed OIDs in their `connectorRef`s.
