...
=== 2026-09-24 Safe Velocity mode introduced

Task: https://support.evolveum.com/wp/12389[12389]
Description:

As part of providing safe execution mode for midPoint, where expressions and scripts can be safely entered by untrusted users, we are introducing `#safe-velocity` scripting language.
It is based on existing `#velocity` language, with some restrictions.
The existing `#velocity` scripts should not be affected.

However, because of limitations of the Velocity engine (in particular that it requires the configuration to be global) we had to limit the execution of regular scripts in minor ways:

. `#parse` and `#include` directives are forbidden
. inline definition of Velocity macros is disabled

If you need these features, please set `legacyVelocityEngine` to `true` in the `config.xml`.
The drawback is that you won't be able to use new `#safe-velocity` language.

Required Upgrade Steps:

- Check if you need above-mentioned features; if yes, set `legacyVelocityEngine` to `true` or migrate your templates.
- It is recommended to gradually migrate all your scripts to `#safe-velocity` language, and then disable the full (unsafe) `#velocity` language completely.
It is done by setting `safeVelocityExpressionsOnly` to `true` in `config.xml`.

Notes & References:

* Related commits: <this one>
* Docs: regular documentation and migration guide will be provided soon

== 4.11-RC1

=== 2026-09-20 Expression profiles are now fully enforced

Task: https://support.evolveum.com/wp/10948[10948]
Description:

Expression profiles are now enforced throughout the system.

The idea is that the expression profile for any expression present an object in midPoint repository is derived from the object type (with archetype and/or subtype, if present).
These profiles were partially enforced in the previous versions, but now their enforcement is to the full extent.

Although the default setting is still "allow all" - in order to provide smooth migration - there may be situations where expressions were set up but not enforced.
In such situations they may now start to fail. Hence, it is necessary to review and optionally adapt your existing expression profile setting to make sure the expressions are compliant with the declared expression profiles.

In rare cases when midPoint cannot determine the expression profile because of missing information about the origin of an expression, it will report them as "TrustDescriptor is required but not set in ..." exceptions.
This may indicate a bug in midPoint code. Please report it using the support portal.
...
As part of hardening the enforcement of expression profiles, the defaults for midPoint actions were changed.
...
* Related commits: https://github.com/Evolveum/midpoint/commit/cdb65600deae108be505d7b97e9f8e10748ef1ae[cdb65600]
...
As part of hardening the enforcement of expression profiles, clarification of concepts and refactoring of internal classes implementing script execution were done.
Specifically, the following classes were renamed:
...
It should be generally invisible to a midPoint engineer.
The only exceptions are if someone used these classes in scripts, in an overlay, or reference them for detailed logging.
...
As part of hardening the enforcement of expression profiles, evaluation of scripts in reports was unified with the evaluation of other scripts and expressions in midPoint.
This may affect deployments that depend on the subtleties of previous implementation, for example, that the order of values returned by the script is preserved, which is no longer the case.
(MidPoint generally does not guarantee ordering of values.)
...
Task: https://support.evolveum.com/wp/12111[12111]
Description: Password hints now stored as ProtectedStringType instead of plain strings. See xref:/midpoint/guides/migrating-plaintext-password-hints/[migration guide].
