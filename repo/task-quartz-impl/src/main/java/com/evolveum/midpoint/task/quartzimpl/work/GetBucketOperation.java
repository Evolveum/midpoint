...
import com.evolveum.midpoint.task.quartzimpl.work.segmentation.BucketAllocator.Response.NewBuckets;
import com.evolveum.midpoint.task.quartzimpl.work.segmentation.BucketAllocator.Response.NothingFound;
...
public class GetBucketOperation extends BucketOperation {
    ...
    private WorkBucketType getWorkBucketStandalone(OperationResult result)
        ...
        if (response instanceof FoundExisting) {
            ...
            return ((FoundExisting) response).bucket;
    ...
    private WorkBucketType getWorkBucketMultiNode(OperationResult result)
        ...
        for (;;) {
            ...
            if (response instanceof NewBuckets) {
                return recordNewBucketInWorkerTask((NewBuckets) response, result);
            } if (response instanceof FoundExisting) {
                return recordExistingBucketInWorkerTask((FoundExisting) response, result);
            } else if (response instanceof NothingFound) {
                if (!TaskWorkStateUtil.isScavenger(workerTask.getWorkState())) {
                    processNothingFoundForNonScavenger();
                    return null;
                } else if (((NothingFound) response).definite || options.freeBucketWaitTime == 0L) {
    ...
    private long getInitialDelay() {
        ...
        return ac != null && ac.getWorkAllocationInitialDelay() != null ?
                ac.getWorkAllocationInitialDelay() : workStateManager.getConfiguration().getWorkAllocationInitialDelay();
    }
    ...
    private long getFreeBucketWaitInterval(WorkBucketsManagementType bucketingConfig) {
        ...
        if (ac != null && ac.getWorkAllocationFreeBucketWaitInterval() != null) {
            ...
            return freeBucketWaitIntervalOverride != null ? freeBucketWaitIntervalOverride :
                    workStateManager.getConfiguration().getWorkAllocationDefaultFreeBucketWaitInterval();
        }
    }
    ...
    private WorkBucketType recordNewBucketInWorkerTask(NewBuckets newBucketsResult, OperationResult result)
    ...
    private WorkBucketType recordExistingBucketInWorkerTask(FoundExisting foundExistingResult,
    ...
    private Collection<? extends ItemDelta<?, ?>> computeCoordinatorModifications(TaskType coordinatorTask,
        ...
        if (response instanceof NewBuckets) {
            return computeCoordinatorModificationsForNewBuckets(workState, (NewBuckets) response);
        } else if (response instanceof FoundExisting) {
            return computeCoordinatorModificationsForExistingBucket((FoundExisting) response);
        } else if (response instanceof NothingFound) {
    ...
    private Collection<ItemDelta<?, ?>> computeCoordinatorModificationsForExistingBucket(FoundExisting response)
            throws SchemaException {
