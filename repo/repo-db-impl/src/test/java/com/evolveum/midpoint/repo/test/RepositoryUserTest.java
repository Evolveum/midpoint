...
public class RepositoryUserTest {
    ...
    public void testUser() throws Exception {
        ...
        try {
            ...
            assertEquals(user.getAdditionalNames(), ((UserType) (retrievedObjectContainer.getObject())).getAdditionalNames());
            assertEquals(user.getEMailAddress(), ((UserType) (retrievedObjectContainer.getObject())).getEMailAddress());
            assertEquals(1, ((UserType) (retrievedObjectContainer.getObject())).getEMailAddress().size());
            assertEquals(user.getEmployeeNumber(), ((UserType) (retrievedObjectContainer.getObject())).getEmployeeNumber());
            assertEquals(user.getEmployeeType(), ((UserType) (retrievedObjectContainer.getObject())).getEmployeeType());
            assertEquals(1, ((UserType) (retrievedObjectContainer.getObject())).getEmployeeType().size());
            assertEquals(user.getFamilyName(), ((UserType) (retrievedObjectContainer.getObject())).getFamilyName());
            assertEquals(user.getFullName(), ((UserType) (retrievedObjectContainer.getObject())).getFullName());
    ...
    public void testUserAddExtension() throws Exception {
        ...
        try {
            ObjectContainerType objectContainer = new ObjectContainerType();
            UserType user = ((JAXBElement<UserType>) JAXBUtil.unmarshal(new File("src/test/resources/user-without-extension.xml"))).getValue();
            objectContainer.setObject(user);
            repositoryService.addObject(objectContainer);
            ObjectContainerType retrievedObjectContainer = repositoryService.getObject(oid, new PropertyReferenceListType());
            assertEquals(user.getOid(), ((UserType) (retrievedObjectContainer.getObject())).getOid());

            ObjectModificationType objectModificationType = CalculateXmlDiff.calculateChanges(new File("src/test/resources/user-without-extension.xml"), new File("src/test/resources/user-added-extension.xml"));
    ...
    public void testUserAddWithoutOid() throws Exception {
        ...
        try {
            ObjectContainerType objectContainer = new ObjectContainerType();
            UserType user = ((JAXBElement<UserType>) JAXBUtil.unmarshal(new File("src/test/resources/user-without-oid.xml"))).getValue();
            objectContainer.setObject(user);
    ...
    public void testUserDeleteAccountRef() throws Exception {
        ...
        try {
            ObjectContainerType objectContainer = new ObjectContainerType();
            ResourceType resource = ((JAXBElement<ResourceType>) JAXBUtil.unmarshal(new File("src/test/resources/resource-modified-removed-tags.xml"))).getValue();
            objectContainer.setObject(resource);
            repositoryService.addObject(objectContainer);

            objectContainer = new ObjectContainerType();
            AccountShadowType accountToDelete = ((JAXBElement<AccountShadowType>) JAXBUtil.unmarshal(new File("src/test/resources/account-delete-account-ref.xml"))).getValue();
            objectContainer.setObject(accountToDelete);
            repositoryService.addObject(objectContainer);

            objectContainer = new ObjectContainerType();
            UserType user = ((JAXBElement<UserType>) JAXBUtil.unmarshal(new File("src/test/resources/user-delete-account-ref.xml"))).getValue();
...
//            doc.createElement();
            ...
            value.getAny().add((Element) DOMUtil.parseDocument("<i:accountRef xmlns:i='http://midpoint.evolveum.com/xml/ns/public/common/common-1.xsd' type=\"account\" oid=\"8254880d-6584-425a-af2e-58f8ca394bbb\"/>").getFirstChild());
