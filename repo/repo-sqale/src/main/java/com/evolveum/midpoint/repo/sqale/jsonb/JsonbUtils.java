...
public enum JsonbUtils {
    ...
    private static <T> void readProtectedData(Map<?,?> map, ProtectedDataType<T> ret, Class<T> clearType) {
        ...
        ret.setExternalData(readExternalData(map));

    }

    private static ExternalDataType readExternalData(Map<?,?> map) {
        var externalProvider = readFromMap(map, String.class, PROTECTED_EXTERNAL_PROVIDER_KEY);
        var externalKey = readFromMap(map, String.class, PROTECTED_EXTERNAL_KEY_KEY);

        if (externalKey != null || externalProvider != null) {
            var external = new ExternalDataType();
            external.setProvider(externalProvider);
            external.setKey(externalKey);
            return external;
        }
        return null;
    }
    ...
    public static Object protectedDataToMap(ProtectedDataType<?> data) {
        ...
        var external = data.getExternalData();
        if (external != null) {
            writeToMap(obj, external.getProvider(), PROTECTED_EXTERNAL_PROVIDER_KEY);
            writeToMap(obj, external.getKey(), PROTECTED_EXTERNAL_KEY_KEY);
        }
        return obj;
