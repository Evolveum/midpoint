...
public class SqaleRepositoryService extends SqaleServiceBase implements RepositoryService {
    ...
    /**
     *
     * Multiple Joins are ineffective ones
     *
     * @param spec Aggregate Query to execute
     * @return
     * @param <T>
     * @throws RepositoryException
     * @throws SchemaException
     */
    public <T extends Containerable> SearchResultList<PrismContainerValue<?>> aggregate(AggregateQuery<T> spec) throws RepositoryException, SchemaException {
        // Start search (lets get root definition)
        var type = spec.getRoot();
        SqaleQueryContext<T, FlexibleRelationalPathBase<Object>, Object> context =
                SqaleQueryContext.from(type, sqlRepoContext);

        var selectExpressions = new ArrayList<Expression<?>>();
        var groupExpressions = new ArrayList<Expression<?>>();
        var orderSpecifiers = new ArrayList<OrderSpecifier<?>>();
        var mappedItems = new ArrayList<ResultMapping>();
        for (var item : spec.getItems()) {
            ...
            check(mapped != null, RepositoryException::new, "Can not map item {}", item.getPath());
            // Special Handling
        ...
        for (var ord : spec.getOrdering()) {
            if (ord instanceof AggregateQuery.AggregateOrdering agg) {
                ...
                var mapped = mappedItems.stream().filter(i -> i.getItem().equals(agg.getItem())).findFirst().get();
        ...
        //for (var item : spec.getGroupBy()) {
        //    var path = context.resolvePath(item);
        //    context.sqlQuery().groupBy(expression);
        //}

        context.beforeQuery();
        try (JdbcSession jdbcSession = sqlRepoContext.newJdbcSession().startReadOnlyTransaction()) {
            var query = context.sqlQuery().clone(jdbcSession.connection())
    ...
    private static String pathName(ItemName name, String suffix) {
        return "_" + name.getLocalPart() +"_" + suffix;
    }

    // Migrate Aggregation mappings outside of SqaleRepositoryService
    ...
    private static class CountMapping extends ResultMapping {
        ...
        public CountMapping(AggregateQuery.Count item, SqaleQueryContext<?, ?, ?> context) {
            super(item);
            count = Expressions.numberPath(Long.class, pathName(item.getName(), "count"));
    ...
    private static class PolyStringMapping extends ResultMapping {
        ...
        public PolyStringMapping(AggregateQuery.ResultItem item, SqlQueryContext.ResolveResult<?, ?> resolved) {
            super(item);
            normPath = Expressions.stringPath(pathName(item.getName(),"norm"));
            origPath = Expressions.stringPath(pathName(item.getName(),"orig"));
    ...
    private class ReferenceMapping extends ResultMapping {
        ...
        private QObject derefPath;
        private QObjectMapping derefMapping;

        public ReferenceMapping(AggregateQuery.ResultItem item, SqlQueryContext.ResolveResult<?, ?> resolved, boolean dereference) {
            super(item);
            this.dereference = dereference;
            oidPath = new UuidPath(PathMetadataFactory.forVariable(pathName(item.getName(),"oid")));
            typePath = Expressions.enumPath(MObjectType.class, pathName(item.getName(),"type"));
            relationPath = Expressions.numberPath(Integer.class, pathName(item.getName(), "relation_id"));
            ...
            if (processor instanceof RefItemFilterProcessor ref) {
                ...
                if (dereference) {
                    ...
                    derefPath = (QObject) derefMapping.newAlias(pathName(item.getName(), "deref"));
        ...
        public Item<?, ?> prismItemFrom(Tuple partial, JdbcSession session) throws SchemaException {
            ...
            if (oid != null) {
                var typeClass = partial.get(typePath).getSchemaType();
                ...
                if (dereference) {
                    if (partial.get(derefPath.fullObject) != null) {
                        var objectable = (Objectable) derefMapping.toSchemaObject(partial, derefPath, session, null);
                        refValue.setObject(objectable.asPrismObject());
                    }
                }
