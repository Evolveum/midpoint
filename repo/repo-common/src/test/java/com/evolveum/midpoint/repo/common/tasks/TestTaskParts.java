...
public class TestTaskParts extends AbstractRepoCommonTest {
...
//
    ...
    private static final TestResource<TaskType> TASK_SINGLE_MOCK_PART = new TestResource<>(TEST_DIR, "task-single-mock-part.xml", "68da9812-45ed-4cd6-86eb-1006b3f7d9ea");
...
//
    ...
    public void test000Sanity() throws Exception {
        ...
        PrismObject<TaskType> task = prismContext.parserFor(TASK_SINGLE_MOCK_PART.file).parse();
        TaskPartDefinitionType work = task.asObjectable().getWork();

        then();

        List<TaskPartUtil.PartParameters> values = TaskPartUtil.getPartParametersCollection(work.getParameters());
        displayValue("Parameters found", values);

        Collection<QName> typesFound = TaskPartUtil.getPartParametersTypesFound(work.getParameters());
        displayValue("Parameters types found", typesFound);

        // TODO asserts
    }

    @Test
    public void test100RunSimpleTask() throws Exception {
        given();

        Task task = getTestTask();
        OperationResult result = task.getResult();

        Task task1 = taskAdd(TASK_SINGLE_MOCK_PART, result);
