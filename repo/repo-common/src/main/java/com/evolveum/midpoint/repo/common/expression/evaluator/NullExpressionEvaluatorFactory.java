 ...
 */
@Component
public class NullExpressionEvaluatorFactory extends AbstractAutowiredExpressionEvaluatorFactory {

    private static final QName ELEMENT_NAME = SchemaConstantsGenerated.C_NULL;

    @Autowired private Protector protector;

    @SuppressWarnings("unused") // Used by Spring
    public NullExpressionEvaluatorFactory() {
    }

    @VisibleForTesting
    public NullExpressionEvaluatorFactory(Protector protector) {
    ...
    public <V extends PrismValue, D extends ItemDefinition<?>> ExpressionEvaluator<V> createEvaluator(
        ...
        NullExpressionEvaluatorType evaluatorBean =
                getSingleEvaluatorBean(evaluatorElements, NullExpressionEvaluatorType.class, contextDescription);

        return new NullExpressionEvaluator<>(ELEMENT_NAME, evaluatorBean, outputDefinition, protector);
    }
}
