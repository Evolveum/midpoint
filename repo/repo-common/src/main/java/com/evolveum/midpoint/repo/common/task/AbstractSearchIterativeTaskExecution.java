...
public class AbstractSearchIterativeTaskExecution
    ...
    private final AtomicReference<AbstractSearchIterativeTaskPartExecution<?, ?, ?, ?, ?>> currentTaskPartExecution
            = new AtomicReference<>();

    public AbstractSearchIterativeTaskExecution(TH taskHandler,
    ...
    public TaskWorkBucketProcessingResult run() throws SchemaException, TaskException, CommunicationException,
            ...
            ObjectAlreadyExistsException, ThresholdPolicyViolationException {
        try {
            ...
            List<? extends AbstractSearchIterativeTaskPartExecution<?, ?, ?, ?, ?>> partExecutions = createPartExecutions();
            for (int i = 0, partExecutionsSize = partExecutions.size(); i < partExecutionsSize; i++) {
                AbstractSearchIterativeTaskPartExecution<?, ?, ?, ?, ?> partExecution = partExecutions.get(i);
    ...
    private List<AbstractSearchIterativeTaskPartExecution<?, ?, ?, ?, ?>> createPartExecutionsFromAnnotation() {
        try {
            ...
            return singletonList(InstantiationUtil.instantiate(annotation.value(), this, taskHandler));
        } catch (Throwable t) {
    ...
    public Long heartbeat() {
        AbstractSearchIterativeTaskPartExecution<?, ?, ?, ?, ?> currentTaskPartExecution = this.currentTaskPartExecution.get();
